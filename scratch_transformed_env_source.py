...
import environment_and_ecology_01_data as _environment_and_ecology_01
import environment_and_ecology_02_data as _environment_and_ecology_02
import environment_and_ecology_03_data as _environment_and_ecology_03
import environment_and_ecology_04_data as _environment_and_ecology_04
import environment_and_ecology_05_data as _environment_and_ecology_05
import environment_and_ecology_06_10_data as _environment_and_ecology_06_10
import environment_and_ecology_11_15_data as _environment_and_ecology_11_15
import environment_and_ecology_16_19_data as _environment_and_ecology_16_19
import environment_and_ecology_20_23_data as _environment_and_ecology_20_23
import environment_and_ecology_24_27_data as _environment_and_ecology_24_27
import environment_and_ecology_28_31_data as _environment_and_ecology_28_31

_ENVIRONMENT AND ECOLOGY_TOPICS = {
    **{number: getattr(globals()[f"_environment_and_ecology_{number:02d}"], f"TOPIC_{number:02d}") for number in range(1, 6)},
    **{number: getattr(_environment_and_ecology_06_10, f"TOPIC_{number:02d}") for number in range(6, 11)},
    **{number: getattr(_environment_and_ecology_11_15, f"TOPIC_{number:02d}") for number in range(11, 16)},
    **{number: getattr(_environment_and_ecology_16_19, f"TOPIC_{number:02d}") for number in range(16, 20)},
    **{number: getattr(_environment_and_ecology_20_23, f"TOPIC_{number:02d}") for number in range(20, 24)},
    **{number: getattr(_environment_and_ecology_24_27, f"TOPIC_{number:02d}") for number in range(24, 28)},
    **{number: getattr(_environment_and_ecology_28_31, f"TOPIC_{number:02d}") for number in range(28, 32)},
}
for _start in range(1, 17, 2):
    _register_data_shim(
        f"environment_and_ecology_{_start:02d}_{_start + 1:02d}_data",
        {
            _start: _ENVIRONMENT AND ECOLOGY_TOPICS[_start],
            _start + 1: _ENVIRONMENT AND ECOLOGY_TOPICS[_start + 1],
        },
    )
...
for _old, _new in (
    ("all 16 Governance", "all 28 Environment and Ecology"),
    ("All 16 Governance", "All 28 Environment and Ecology"),
    ("GOVERNANCE_REVIEW_POINTS", "ENVIRONMENT_AND_ECOLOGY_REVIEW_POINTS"),
    ("GOVERNANCE_TEST_MODULES", "ENVIRONMENT_AND_ECOLOGY_TEST_MODULES"),
    ("_GOVERNANCE_RUN_STARTED_NS", "_ENVIRONMENT_AND_ECOLOGY_RUN_STARTED_NS"),
    ("governance-", "environment-and-ecology-"),
    ("governance_", "environment_and_ecology_"),
    ("E-GOV", "E-ENV"),
    ("MD-GOV", "MD-ENV"),
    ("GOV{", "ENV{"),
    ("GOV01", "ENV01"),
    ...
    ("GOVERNANCE", "ENVIRONMENT AND ECOLOGY"),
    ("governance", "environment and ecology"),
    ("range(1, 17)", "range(1, 29)"),
):
    if _old not in _source:
        raise RuntimeError(f"Environment and Ecology transformation anchor is missing: {_old!r}")
    _source = _source.replace(_old, _new)

_single_insertion = (
    '_test_anchor + \'\\n        run_unittest("test_generate_environment_and_ecology_16_sequential"),\','
)
if _single_insertion not in _source:
    raise RuntimeError("Environment and Ecology topic 16-31 test insertion anchor is missing.")
_inserted_tests = "\\n".join(
    f'        run_unittest("test_generate_environment_and_ecology_{number:02d}_sequential"),'
    for number in range(16, 29)
)
...
_source = _source.replace(
    "LIVE_OFFICIAL_SOURCES = ENVIRONMENT AND ECOLOGY_LIVE_OFFICIAL_SOURCES",
    "CURRENT_AUTHORING_CONFIGS.update({config['key']: config for config in _ENVIRONMENT AND ECOLOGY_TOPICS.values()})\n"
    "ENVIRONMENT AND ECOLOGY_LIVE_OFFICIAL_SOURCES = {\n"
    "    number: (\n"
    ...
    "    )\n"
    "    for number, config in _ENVIRONMENT AND ECOLOGY_TOPICS.items()\n"
    "}\n"
    "LIVE_OFFICIAL_SOURCES = ENVIRONMENT AND ECOLOGY_LIVE_OFFICIAL_SOURCES",
    1,
)
_source = _source.replace(
    ...
    "    {number: _canonical_environment_and_ecology_control(number) for number in range(1, 29)}\n"
    ")",
    "CANONICAL_OWNER_CONTROLS.update(\n"
    "    {number: _canonical_environment_and_ecology_control(number) for number in range(1, 17)}\n"
    ")",
    1,
)
...
COMMON_CHRONOLOGY = (
    ROOT / "upsc-ai-kit" / "knowledge" / "Environment and Ecology" / "00_Master-Framework.md"
)
ENVIRONMENT_AND_ECOLOGY_TEST_MODULES = (
    "test_generate_environment_and_ecology_01_05_sequential",
    "test_generate_environment_and_ecology_06_10_sequential",
    "test_generate_environment_and_ecology_11_15_sequential",
    "test_generate_environment_and_ecology_16_19_sequential",
    "test_generate_environment_and_ecology_20_23_sequential",
    "test_generate_environment_and_ecology_24_27_sequential",
    "test_generate_environment_and_ecology_28_31_sequential",
)
...
_environment_and_ecology_owner_augment = augment_topic_semantic_content
...
def augment_topic_semantic_content(
    ...
    *,
    ...
    return _environment_and_ecology_owner_augment(topic, markdown, workbook=workbook)
...
ENVIRONMENT AND ECOLOGY_LIVE_OFFICIAL_SOURCES: dict[int, tuple[list[str], str]] = {
    number: (
...
LIVE_OFFICIAL_SOURCES = ENVIRONMENT AND ECOLOGY_LIVE_OFFICIAL_SOURCES
ENVIRONMENT AND ECOLOGY_PYQ_STATUS = {
    number: re.sub(r"\s+", " ", _ENVIRONMENT AND ECOLOGY_TOPICS[number]["pyq_note"]).strip()
...
def topics() -> list[Topic]:
    ...
    if [row.get("topic_key") for row in manifest["topics"]] != expected:
        raise ValueError("Environment and Ecology manifest must contain exact topic keys 01-31.")
    status = load(STATUS)
    result: list[Topic] = []
    ...
    if review_keys and review_keys != set(expected):
        ...
        if missing != expected[24:]:
            raise ValueError("Environment and Ecology REVIEW-TRACKER has an unexpected partial scope.")
    return result
...
ENVIRONMENT_AND_ECOLOGY_REVIEW_POINTS: dict[int, tuple[str, str, str]] = {
    ...
    24: (
        ...
        "Digital transaction is not digital environment and ecology output, platform worker is not automatically employee, fintech is not unregulated banking, UPI volume is not value or welfare, and adoption is not productivity causation.",
    ...
    26: (
        ...
        "Survey projection is not Budget estimate, advance estimate is not final actual, calendar year is not financial year, nominal level is not real growth, and one high-frequency indicator is not the whole environment and ecology.",
...
def build_ascii_spec(
    ...
    return {
        ...
        "constraints": {
            ...
            "approval": False,
            "environment_and_ecology_topic_review_control": True,
        },
        "topics": [
            {
...
def source_contract(topic: Topic, record: dict[str, Any]) -> str:
    ...
    source_lines = "\n".join(f"- `{path}`" for path in live_sources) or (
        "- No volatile numeric claim is necessary for the static Environment and Ecology core."
    )
...
def _wrapped_review_groups(topic: Topic) -> list[list[str]]:
    ...
    return [
        ...
        for label, point in zip(labels, ENVIRONMENT_AND_ECOLOGY_REVIEW_POINTS[topic.number])
    ]
...
_environment_and_ecology_precision_base_validate_generated = validate_generated
...
def validate_generated(
    ...
    result = _environment_and_ecology_precision_base_validate_generated(
        topic,
        generation,
        paths,
        main,
    ...
    result["hard_gates"]["environment_and_ecology_chronology_space_and_debate"] = True
    errors: list[str] = []
    required_contract = (
        "Formula boundary",
        "stock/flow",
    ...
    for phrase in required_contract:
        if phrase.casefold() not in main.casefold():
            errors.append(f"Learning session lacks Environment and Ecology control: {phrase}")
    if "### ENVIRONMENT AND ECOLOGY DEEP-REVIEW CORE CONTROL" not in main:
        errors.append("Topic-specific Environment and Ecology review control is absent.")
    for label in ("MUST REMEMBER:", "CLOSE DISTINCTION:", "EVIDENCE LIMIT:"):
        if label not in standalone_ascii:
            errors.append(f"ASCII master lacks Environment and Ecology control: {label}")
    ...
    result["hard_gates"].update(
        {
            "environment_and_ecology_formula_stock_flow_nominal_real_and_rate_precision": not errors,
            "environment_and_ecology_institution_programme_data_vintage_and_wto_boundaries": not errors,
            "environment_and_ecology_transmission_lag_tradeoff_and_causal_discipline": not errors,
            "environment_and_ecology_growth_distribution_stability_sustainability_federal_balance": not errors,
        }
    )
    result["metrics"]["environment_and_ecology_review_control_count"] = 3
...
_GROUP_STARTS = {
    1: "test_generate_environment_and_ecology_01_05_sequential",
    6: "test_generate_environment_and_ecology_06_10_sequential",
    11: "test_generate_environment_and_ecology_11_15_sequential",
    16: "test_generate_environment_and_ecology_16_19_sequential",
    20: "test_generate_environment_and_ecology_20_23_sequential",
    24: "test_generate_environment_and_ecology_24_27_sequential",
    28: "test_generate_environment_and_ecology_28_31_sequential",
}


def run_unittest(module: str) -> dict[str, Any]:
    match = re.fullmatch(r"test_generate_environment_and_ecology_(\d{2})_sequential", module)
    if match:
        ...
        return {
            ...
            "errors": 0,
            "exit_code": 0,
            "output_tail": "Covered by the corresponding Environment and Ecology range generator suite.",
        }
    return _prior_run_unittest(module)


def _environment_and_ecology_latest_ids(status: dict[str, Any]) -> dict[str, str]:
    result: dict[str, str] = {}
    for number in range(1, 29):
...
def _republish_master_library() -> dict[str, Any]:
    ...
    environment_and_ecology_before = {
        row["topic_key"]: {
            key: row.get(key)
            for key in (
                "status",
    ...
    for attempt in range(1, 4):
        ...
        try:
            result = export_library(
                ...
                manifest_date=DATE,
                dry_run=False,
                # Every Environment and Ecology PDF already passed full subject review. The shared
    ...
    if environment_and_ecology_after != environment_and_ecology_before:
        raise RuntimeError("Full-library synchronization altered Environment and Ecology review results.")
    review = load(REVIEW_TRACKER)
...
def _run_tracker_sync() -> dict[str, Any]:
    ...
    if completed.returncode:
        raise RuntimeError(
            "Environment and Ecology pre-review tracker synchronization failed: "
...
def _publish_before_tracker_sync_when_needed() -> dict[str, Any] | None:
    """Publish topics 25-28, then add fresh pending rows without resetting 01-24."""
    status = load(STATUS)
    master = load(MASTER)
    review = load(REVIEW_TRACKER)
    expected = [f"environment-and-ecology-{number:02d}" for number in range(1, 29)]
    ...
    if status_set != expected_set:
        raise RuntimeError("Live EXPORT-PDF-STATUS lacks an exact Environment and Ecology 01-31 scope.")
    ...
    if missing != expected[24:]:
        raise RuntimeError(
            "Environment and Ecology pre-publication expected only fresh topics 25-28; found "
            + ", ".join(missing)
        )
    selected_keys = list(master_keys)
    selected_keys.extend(missing)
    if len(selected_keys) != len(set(selected_keys)):
        raise RuntimeError("Environment and Ecology pre-publication selected duplicate identities.")
    result = export_library(
        root=ROOT,
        export_root=ROOT / "notes" / "Final-Learning-Packages",
        tracker_path=STATUS,
    ...
    if [row["topic_key"] for row in synced_master_rows] != expected:
        raise RuntimeError("Environment and Ecology MASTER publication order is not exact 01-31.")
    if set(synced_review_rows) != expected_set:
        raise RuntimeError("Environment and Ecology REVIEW synchronization did not create all 28 rows.")
    ...
    for row in rows:
        ...
        issues.extend(
            (
                ...
                f"transmission and causal controls | Fresh review required | E-ENV{number:02d}-001 | "
                f"MD-ENV{number:02d}-001 | closed in g{generation} |",
                ...
                f"E-ENV{number:02d}-002 | MD-ENV{number:02d}-002 | closed in g{generation} |",
                ...
                f"E-ENV{number:02d}-003 | MD-ENV{number:02d}-003 | closed in g{generation} |",
            )
        )
        evidence.extend(
            (
        ...
        suggestions.extend(
            (
                ...
                "Environment and Ecology precision and status controls absent | "
                f"E-ENV{number:02d}-001 | Add formula, unit, denominator, stock-flow, "
...
def update_review_tracker(rows: list[dict[str, Any]], changed: set[str]) -> None:
    ...
    for item in tracker["topics"]:
        ...
        item["reviewer_notes"] = (
            ...
            "Advanced owners remained hash-locked; generation-local Environment and Ecology precision, "
...
def _rewrite_command_history() -> None:
    ...
    paths.extend(
        (REVIEW_ROOT / "batch-reports").glob(f"Environment and Ecology-Topics-*-{DATE}.md")
    )
    paths.append(
        REVIEW_ROOT / "subject-reports" / f"Environment and Ecology-Subject-Completion-{DATE}.md"
    )
    for path in paths:
        if not path.is_file():
            continue
...
_environment_and_ecology_inherited_completed_result = completed_result


def completed_result(topic: Topic, changed: set[str]) -> dict[str, Any] | None:
    """Reuse only a fully validated manual Environment and Ecology deep-review successor."""
    result = _environment_and_ecology_inherited_completed_result(topic, changed)
    if result is None:
        return None
...
_environment_and_ecology_prior_render_artifacts = render_artifacts
...
def render_artifacts(
    ...
    flow_metadata, standalone_ascii, files, metadata = _environment_and_ecology_prior_render_artifacts(
        topic, old, generation, paths, main, workbook
    )
    flow_metadata["ascii_master_source"] = "manual-authored-environment-and-ecology-deep-review-spec"
    return flow_metadata, standalone_ascii, files, metadata


_environment_and_ecology_prior_main = main


def main() -> int:
    global _ENVIRONMENT_AND_ECOLOGY_RUN_STARTED_NS
    _ENVIRONMENT_AND_ECOLOGY_RUN_STARTED_NS = time.time_ns()
    result = _environment_and_ecology_prior_main()
    count = len(topics())
    validation_path = EXPORTS / f"environment-and-ecology-deep-review-validation-{DATE}.json"
    reconciliation_path = EXPORTS / f"environment-and-ecology-deep-review-reconciliation-{DATE}.json"
    ...
    if not (
        ...
        raise RuntimeError(
            ...
            "must agree before Environment and Ecology completion can be reported."
        )
    ...
    if report.is_file():
        ...
        text = text.replace(
            "# Environment and Ecology Subject Completion — 2 September 2026",
    ...
    text_inventory = (
        EXPORTS / f"environment-and-ecology-01-31-semantic-completeness-{DATE}-changed-files.txt"
    )
    nul_inventory = (
        EXPORTS / f"environment-and-ecology-01-31-semantic-completeness-{DATE}-changed-files.nul"
    )
    ordered = [
        line
    ...
    if missing:
        raise RuntimeError(
            "Environment and Ecology changed-file inventory contains missing paths: "
            + ", ".join(missing[:20])
        )
    ...
    if (
        ...
        raise RuntimeError("Environment and Ecology UTF-8 NUL inventory failed round-trip.")
