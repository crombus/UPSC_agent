"""Run one strictly sequential hostile semantic-completeness review for Ethics."""
...
from ethics_semantic_runtime import REPORT_DATE, load_runtime, topic_slug
...
runner.DEEP_REVIEW_TEST_MODULE = "test_regenerate_ethics_deep_review"
runner.REPORT_DIR = (
    deep.ROOT / "upsc-ai-kit" / "manifests" / "reviews" / "ethics"
)
runner.DRIVER_FILES = {
...
LIVE_AUDIT = (
    ...
    / "manifests"
    / "exports"
    / f"ethics-authoritative-live-source-audit-{REPORT_DATE}.json"
)


def _load_live_audit() -> dict[str, Any]:
    if not LIVE_AUDIT.is_file():
        raise FileNotFoundError("Run verify_ethics_live_sources.py first.")
    payload = runner.load(LIVE_AUDIT)
    if payload.get("result") != "passed":
        raise RuntimeError("Authoritative Ethics live-source audit has failures.")
    return payload
...
def _live_sources() -> dict[int, tuple[list[str], str]]:
    ...
    for number in range(1, 24):
        key = f"ethics-{number:02d}"
        row = by_key[key]
        sources = [
            item["url"] for item in row["sources"] if item.get("substantive")
        ]
        result[number] = (
            sources,
            f"Rechecked {REPORT_DATE} through {row['attempted']} authoritative "
            f"or primary-academic source attempts, including "
            f"{row['substantive_retrievals']} substantive live retrievals. "
            "Failed or thin attempts remain preserved and support no claim. "
            "Legal/institutional fact, ethical analysis and inference are separate.",
        )
    return result
...
runner.PYQ_STATUS = {
    number: (
        f"{len(topic.data.PYQS)} verified/routed GS-IV questions retained with "
        "year, marks, source note and examiner-grade solution; no unavailable "
        "official key, quotation provenance or case fact was invented"
    )
    ...
    row["findings"] = [
        {
            "severity": "closed",
            "finding": (
                "Hostile GS-IV audit closed: literal syllabus, prerequisites, "
                "standard ethical taxonomy, thinker and quotation integrity, "
                "verified PYQs/case studies, canonical ownership and boundaries, "
                "public-law/current-source status, examiner-grade answer contracts, "
                "applied decision architecture and both twelve-panel flow masters pass."
            ),
...
def report_text(
    ...
    return f"""# Ethics Semantic-Completeness Review {topic.number:02d} - {topic.title}
...
Only this topic was active. The literal UPSC GS-IV syllabus, indispensable
prerequisites, standard ethical taxonomy, canonical Basic and Optional Advanced
owners, cross-topic boundaries, verified PYQs and case studies, hostile
retrieval searches, thinker/quotation integrity and authoritative live sources
were reconciled.

The immutable successor preserves precise distinctions, Indian and Western
ethical thought where relevant, administrative application, competing
principles, objections and replies, India-centric verified evidence, and
case-study reasoning through fact/value separation, stakeholder and vulnerable-
group mapping, legality/constitutional morality, duty-consequence-virtue-
justice-care-public-interest tests, short/long-term effects, reasoned choice,
implementation risk and preventive institutional reform. Basic precedes
    ...
    record.setdefault("provenance", {}).update(
        {
            ...
            "verification_scope": _load_live_audit()["topics"][topic.number - 1][
                "verification_scope"
            ],
...
def run(topic_number: int) -> dict[str, object]:
    if topic_number not in runner.TOPIC_CHOICES:
        raise ValueError("Ethics runner accepts topics 1 through 23.")
