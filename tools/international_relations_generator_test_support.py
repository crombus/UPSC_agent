"""Shared assertions for International Relations sequential generator tests."""
...
def assert_batch_contract(
    ...
    case.assertIn(
        "QP-CSM19-GeneralStudies-II.pdf",
        [path.name for path in generator.LOCAL_BOOKS],
    )
    ...
    case.assertEqual("International-Relations", manifest["subject"]["key"])
    case.assertEqual(
    ...
    for config in generator.TOPICS:
        ...
        case.assertEqual("International-Relations", graphical["subject"], key)
...
def assert_live_source_attempt_log(
    ...
    for phrase in (
        ...
        "attempted 2026-09-03",
        "returned HTTP 403",
        "browser-requirement stub",
        "the Ministry's own error page",
    ):
        case.assertIn(phrase, text, key)
    for config in generator.TOPICS:
