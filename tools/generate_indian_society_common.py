"""Shared authoring-only engine for Indian Society learner-v2 batches."""
...
# OCR-searchable official General Studies Paper-I question papers held locally.
# They are used only to verify routed PYQ wording, never to invent a question,
# a marking key or an official answer.
LOCAL_BOOKS = [
    ...
    ROOT / "books" / "more_previous_papers" / "QP-CSM19-GeneralStudies-I.pdf",
    ROOT / "books" / "more_previous_papers" / "Gen_St_P1.pdf",
    ROOT
    / "books"
    / "more_previous_papers"
    / "QP-CSM-21-GENSTUDIESPAPER-I-110122.pdf",
    ROOT
    / "books"
    / "more_previous_papers"
...
COMMON_CROSS = [
    ...
    KNOWLEDGE / "REVISION-CHART_Structures-Change-and-Distinctive-Features.md",
...
def _session_fragment(
    ...
    plain = (
        ...
        f"{joined} fit into one examinable social relationship."
    )
    technical = (
        f"In sociological analysis, {title} denotes the source-bounded "
        f"relationship among {joined}, described by unit of analysis, "
        "boundary rule, mechanism of reproduction, direction of change and "
        "the evidence that may legitimately be claimed for it."
    )
    ...
    fragment = (
        ...
        "as a list of adjectives about Indian society.\n\n"
        "#### MUST-WRITE KEYWORDS\n\n"
        ...
        + f"- {caution}\n\n"
        + "#### EXAM LINK\n\n"
        + f"- **Prelims:** Retain the exact unit, named scholar, official "
        f"category, dated statistic or legal boundary attached to {title}.\n"
        + f"- **Mains:** {exam_use}\n\n"
        + "#### MINI RECAP\n\n"
        ...
        + f"MECHANISM / ARGUMENT: relate {joined} through unit, boundary rule and direction of change\n"
        ...
        + f"ANSWER-GRABBING FORMULATION: {title} converts named social evidence into a qualified argument\n"
        + "```"
    )
    return re.sub(r"[ \t]+\n", "\n", fragment).strip()
...
def topic(
    ...
    """Create one Indian Society topic configuration."""

    return {
        "number": number,
        "key": f"indian-society-{number:02d}",
        "title": title,
        ...
        or (
            ...
            "The package invents no caste, tribe, demographic or agrarian "
            "statistic, no legal or constitutional claim and no policy status."
        ),
        "ocr_note": (
            "Repository Markdown was primary. The OCR-searchable official "
            "General Studies Paper-I question papers held under books\\mains and "
            "books\\more_previous_papers were used only to confirm the wording of "
            "routed Mains PYQs. No official answer key, marking scheme, page "
            "precision or unsupported quotation was imported from them."
        ),
        "pyq_note": pyq_note,
...
def ensure_section_manifest() -> Path:
    ...
    target = next(
        ...
        if item.get("topic_key") == "indian-society-01"
    )
    ...
    if path != SECTION_MANIFEST:
        raise ValueError(f"Unexpected Indian Society manifest path: {path}")
    return path
...
def validate_catalog(topics: list[dict[str, object]]) -> None:
    ...
    if missing:
        raise ValueError(f"Indian Society topics missing from catalog: {missing}")
...
def _register_notes(config: dict[str, object]) -> str:
    """Topic-specific consolidated register notes for Indian Society."""
    ...
    return (
        f"### {config['title']}: RAPID CONCEPT AND EVIDENCE MAP\n\n"
        f"{facts}\n\n"
        ...
        "```\n\n"
        f"### {config['title']}: CURRENT-LINK AND DATA BOUNDARY\n\n"
