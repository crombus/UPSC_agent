"""Reusable authoring-only engine for Internal Security learner-v2 topics.

The engine follows the current Science and Technology single-writer
workflow while enforcing internal-security legal, institutional and status
discipline. It never invents mandates, attribution, operational detail,
implementation outcomes, security metrics, PYQs or objective answers.
"""
...
COMMON_CROSS = [
    ...
    KNOWLEDGE / "REVISION-CHART_Threats-Responses-and-Distinctive-Features.md",
...
_KEYWORD_EXCLUSIONS = {
    ...
    "owner", "programme", "question", "security", "source", "status",
    "threat", "topic", "which", "while", "with", "without",
}
REGISTER_HEADINGS = (
    "THREAT, GOVERNANCE AND INSTITUTION MAP",
    "LAW, INSTRUMENT AND STATUS FIREWALLS",
    "ANSWER-WRITING SPINE",
    "LIVE-SOURCE AND OPERATIONAL-CLAIM BOUNDARY",
)
...
def make_plans(
    ...
    if len(titles) != 15:
        raise ValueError("Exactly fifteen Internal Security sessions are required.")
    return [
    ...
    for marks, prompt, indexes in prompts:
        ...
        model = " ".join(
            (
                ...
                "institutional mandate and verified evidentiary rung before "
                "drawing the policy implication. **Qualification:** The conclusion "
                "retains the owner's date, institution, legal character and "
...
def topic(
    ...
    official_domains = (
        "mha.gov.in", "pib.gov.in", "indiacode.nic.in",
        "legislative.gov.in", "nia.gov.in",
    )
    ...
    return {
        ...
        "ocr_note": (
            ...
            "answer key, attribution, operational method, notification effect, "
            "implementation outcome or security metric was inferred."
        ),
        "pyq_note": pyq_note,
...
def _session_keywords(title: str, selected: list[tuple[str, str]]) -> list[str]:
    ...
    for word in re.findall(r"[A-Za-z][A-Za-z0-9'-]{3,}", source):
        ...
        if len(result) == 6:
            break
    if len(result) < 4:
        raise ValueError(f"{title}: fewer than four internal-security-specific keywords.")
    return result
...
def _session_fragment(
    ...
    return (
        ...
        "examinable internal-security governance system.\n\n"
        f"**Technical definition:** The answer fixes the threat mechanism, "
        "governance or response layer, responsible institution, legal basis "
        "and verified evidentiary rung before comparing impact.\n\n"
        "#### ANSWER-GRABBING OPENING — WRITE/ADAPT IN THE EXAM\n\n"
        f"> {title} must be analysed through {joined}, while preserving the "
        "difference between law, notification, listing, scheme, agreement, "
        "ceasefire, implementation and verified outcome.\n\n"
        "#### MUST-WRITE KEYWORDS\n\n"
        ...
        f"institution, legal instrument and status. Qualification: {caution}\n\n"
        "#### VISUAL FIRST\n\n```text\n"
        + "\n".join(visual)
        ...
        "correct legal category, institutional mandate and evidentiary status."
        + "\n\n#### NAMED EVIDENCE AND MECHANISM\n\n"
        + "\n".join(f"- {statement}" for _, statement in selected)
        + f"\n\n#### EXAMINER CAUTION\n\n- {caution}\n\n#### EXAM LINK\n\n"
        "- **Prelims:** Keep institution, law, notification, ban/listing, "
        "scheme, agreement, ceasefire and outcome in their exact category.\n"
        f"- **Mains:** {exam_use}\n\n#### MINI RECAP\n\n"
        ...
        f"MECHANISM / ARGUMENT: connect {joined} without changing the legal or evidentiary status\n"
        f"CONSEQUENCE / CONTRAST: {exam_use}\n"
        f"UPSC TRAP / ANSWER-USE: {caution}\n"
        "ANSWER-GRABBING FORMULATION: a notification, operation or agreement "
        "proves only its own officially verified rung\n```"
    )
...
def _build_mcqs(config: dict[str, object]) -> str:
    ...
    variants = [
        ...
        "Which option preserves the legal or institutional boundary of {label}?",
        "Which statement uses {label} without changing its institution, law or status?",
    ...
    for fact_index, (label, statement) in enumerate(facts):
        for variant_index, template in enumerate(variants):
            ...
            blocks.append(
                ...
                "threat category, institutional owner, legal instrument, "
...
def _assemble(
    ...
    workbook = (
        ...
        "internal-security distinctions and strict A-B-C-D rotation.\n\n"
        "## BASIC MCQS / REMEDIATION\n\n" + mcqs
        + "\n\n## PYQS AND ANSWER PRACTICE\n\n" + practice + "\n"
    )
...
def ensure_section_manifest() -> Path:
    ...
    target = next(
        ...
        if item.get("topic_key") == "internal-security-01"
    )
    path = section_indexes.materialize_catalog_section_manifest(ROOT, catalog, target)
    if path != SECTION_MANIFEST:
        raise ValueError(f"Unexpected Internal Security manifest: {path}")
    return path
...
def validate_catalog(topics: list[dict[str, object]]) -> None:
    ...
    if missing:
        raise ValueError(f"Internal Security topics missing from catalogue: {missing}")
...
def _write_generation_spec(
    ...
    if missing:
        raise FileNotFoundError("Missing Internal Security sources: " + ", ".join(missing))
    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    catalog_topic = next(
    ...
    payload = {
        ...
        "supersedes": (
            "internal-security-01:legacy-v1:g1"
            if config["key"] == "internal-security-01" else None
        ),
