"""Prove and record completion manifests for one Governance learner-v2 topic.
...
import generate_governance_common as governance  # noqa: E402
...
SHARED_FILES = [
    ...
    "upsc-ai-kit\\manifests\\v2\\governance--subject-wide-syllabus.json",
    "upsc-ai-kit\\knowledge\\Governance\\LEARNING-SESSION-COMMAND-INDEX.md",
    f"notes\\Governance\\learning-session-v2\\{SECTION_KEY}\\indexes\\TOPIC-COVERAGE-INDEX.md",
    f"notes\\Governance\\learning-session-v2\\{SECTION_KEY}\\indexes\\NOTES-PDF-INDEX.md",
    f"notes\\Governance\\learning-session-v2\\{SECTION_KEY}\\indexes\\WORKBOOK-PDF-INDEX.md",
]
...
def prove(topic_key: str, record: dict[str, object]) -> dict[str, object]:
    ...
    register_headings = list(governance.REGISTER_HEADINGS)
...
def changed_files(
    ...
    *,
    ...
    Basic/Advanced owners, the PYQ routing ledgers and the topic catalogue are
    recorded as provenance inside the tracker record, but they are never written
    by an export, so they are excluded here. Only paths under the learner-v2
    output roots, the shared tracker/index state, the authored specs and the
    export manifests are reported.
    """
