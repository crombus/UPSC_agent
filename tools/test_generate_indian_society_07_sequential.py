"""Regression tests for Indian Society learner-v2 Topic 07."""

from __future__ import annotations

import sys
import unittest
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "tools"))

import generate_indian_society_06_sequential as previous
import generate_indian_society_07_sequential as generator
import validate_v2_export as validator
from indian_society_generator_test_support import (
    assert_batch_contract,
    assert_no_publish_side_effects,
    session_markdown,
    workbook_markdown,
)


class IndianSociety07GeneratorTests(unittest.TestCase):
    def test_complete_batch_contract(self) -> None:
        assert_batch_contract(
            self,
            generator,
            ["indian-society-07"],
            ["Women and Women's Organisations"],
        )

    def test_previous_batch_identity_is_unchanged(self) -> None:
        self.assertEqual(
            ["indian-society-06"],
            [item["key"] for item in previous.TOPICS],
        )

    def test_organisational_forms_and_trichotomy_are_preserved(self) -> None:
        text = session_markdown(generator, "indian-society-07")
        for phrase in (
            "Women's Indian Association",
            "All India Women's Conference",
            "1927",
            "National Federation of Indian Women",
            "Self Employed Women's Association",
            "self-help group",
            "gender equity",
            "intersectionality",
            "National Suicide Prevention Strategy",
        ):
            self.assertIn(phrase, text)

    def test_access_indicators_are_dated_and_not_treated_as_agency(self) -> None:
        text = session_markdown(generator, "indian-society-07")
        self.assertIn("78.6", text)
        self.assertIn("33.3", text)
        self.assertIn("29 May 2026", text)
        self.assertIn("rather than control", text)

    def test_routed_pyq_status_is_transparent_and_unfabricated(self) -> None:
        text = session_markdown(generator, "indian-society-07")
        workbook = workbook_markdown(generator, "indian-society-07")
        self.assertIn("VERIFIED PYQ OWNERSHIP AUDIT", text)
        self.assertIn("VERIFIED PYQ OWNERSHIP AUDIT", workbook)
        self.assertEqual(5, text.count("### PYQ DEMAND CARD"))
        self.assertEqual(5, len(generator.TOPICS[0]["pyq_solutions"]))
        self.assertIn("no verbatim wording is claimed", text)
        self.assertEqual(6, text.count("### ORIGINAL MAINS"))

    def test_advanced_owner_is_preserved_in_the_optional_block(self) -> None:
        text = session_markdown(generator, "indian-society-07")
        advanced = text.split(
            "## OPTIONAL ADVANCED DEPTH — NOT REQUIRED FOR A CORE ANSWER",
            1,
        )[1].split("## CONSOLIDATED REGISTER NOTES", 1)[0]
        for phrase in (
            "Intersectionality vs universal sisterhood framing",
            "Organisational-layering chain",
            "Membership-versus-voice trade-off",
        ):
            self.assertIn(phrase, advanced)
        self.assertNotIn("\n## 6.", advanced)

    def test_session_definitions_pass_semantic_quality_audit(self) -> None:
        audit = validator.deep_content_quality_audit_text(
            session_markdown(generator, "indian-society-07"),
            topic_key="indian-society-07",
        )
        high = [
            item
            for item in audit["defects"]
            if item["severity"] in {"high", "blocker"}
        ]
        self.assertEqual([], high)

    def test_generator_has_no_publish_side_effects(self) -> None:
        assert_no_publish_side_effects(self, generator)


if __name__ == "__main__":
    unittest.main()
