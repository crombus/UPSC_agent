"""Remove partial topic 7 learner-v2 targets before a clean re-run.

The reset is strictly scoped to Philosophy Paper II Socio-Political Philosophy
topic 07, ``Crime and Punishment : Corruption, Mass Violence, Genocide, Capital
Punishment.``  Topics 01-06, topics 08-10, every legacy-v1 package and all
unrelated trees are never touched.
"""
...
def drop_tracker_record() -> None:
    """Remove only this topic's learner-v2 record so a re-run stays at g2."""
    tracker = ROOT / "EXPORT-PDF-STATUS.json"
    data = json.loads(tracker.read_text(encoding="utf-8"))
    kept = [
        record
        for record in data["exports"]
        if not (
    ...
    if len(kept) == len(data["exports"]):
        return
    data["exports"] = kept
    pending = tracker.with_suffix(tracker.suffix + ".pending")
    pending.write_text(
        json.dumps(data, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    os.replace(pending, tracker)
    print(f"REMOVED TRACKER RECORD: {TOPIC_KEY}:learner-v2")
