"""Validate and inventory the completed Governance semantic review."""
...
import regenerate_governance_deep_review as deep
import run_governance_semantic_topic as subject
...
REPORT = (
    ...
    / "reviews"
    / "governance"
    / f"Governance-Subject-Semantic-Completion-{DATE}.md"
)
VALIDATION = (
    EXPORTS / f"governance-01-16-semantic-completeness-{DATE}-validation.json"
)
INVENTORY = (
    EXPORTS / f"governance-01-16-semantic-completeness-{DATE}-changed-files.txt"
)
...
def main() -> int:
    ...
    governance_rows = [
        row for row in semantic["topics"] if row["topic_key"].startswith("governance-")
    ]
    expected = [f"governance-{number:02d}" for number in range(1, 17)]
    errors: list[str] = []
    if [row["topic_key"] for row in governance_rows] != expected:
        errors.append("Governance semantic catalogue/order mismatch.")
    if any(row["status"] != "passed" for row in governance_rows):
        errors.append("Not every Governance semantic row is passed.")
    if semantic["next_topic"]["topic_key"] != "social-justice-01":
        errors.append("Global queue did not advance to social-justice-01.")

    tracker_text = (
        ROOT / "KNOWLEDGE-SEMANTIC-COMPLETENESS-TRACKER.md"
    ).read_text(encoding="utf-8")
    if "| 10 | Governance | 16 | 16 | passed | Complete |" not in tracker_text:
        errors.append("Human tracker does not record Governance 16/16 passed.")
    if "Social Justice — Subject-wide Syllabus — Social Justice, Inclusion and Welfare-State Framework" not in tracker_text:
    ...
    inventory: set[str] = {
        ...
        "tools\\finalize_governance_semantic_review.py",
        "tools\\governance_11_15_data.py",
        "tools\\regenerate_governance_deep_review.py",
        "tools\\run_governance_semantic_topic.py",
        "tools\\test_run_governance_semantic_topic.py",
        rel(REPORT),
        rel(VALIDATION),
        rel(INVENTORY),
    ...
    for number, key in enumerate(expected, 1):
        ...
        report_path = next(
            path
            for path in governance_rows[number - 1]["files_changed"]
            if path.startswith("upsc-ai-kit\\manifests\\reviews\\governance\\")
            and path.endswith(f"-semantic-completeness-review-{DATE}.md")
        )
        topics.append(
            {
                "topic_key": key,
                "title": governance_rows[number - 1]["title"],
    ...
    if not all_approval_false:
        errors.append("At least one Governance learner-v2 identity is approved.")
    ...
    payload = {
        ...
        "topic_count": 16,
        "passed": sum(row["status"] == "passed" for row in governance_rows),
        "next_topic_key": semantic["next_topic"]["topic_key"],
        "next_subject": semantic["next_topic"]["subject"],
        "next_topic": semantic["next_topic"]["title"],
        "checks": {
            "catalogue_order": [row["topic_key"] for row in governance_rows] == expected,
            "all_topics_passed": all(row["status"] == "passed" for row in governance_rows),
            ...
            "status_tracker_agreement": (
                "| 10 | Governance | 16 | 16 | passed | Complete |" in tracker_text
            ),
    ...
    REPORT.write_text(
        f"""# Governance Semantic-Completeness Completion — 5 September 2026

**Result:** {'FAILED' if errors else 'PASSED'}  
**Coverage:** {payload['passed']}/16  
**Next queue item:** `social-justice-01` — Social Justice, Inclusion and Welfare-State Framework

| Topic | Identity | Main pages | Workbook pages | Solved | MCQs | ASCII/graphical | Result |
|---|---|---:|---:|---:|---:|---:|---|
{rows}

Targeted Governance regression: {test_count} tests, zero failures.
All learner-v2 Governance identities remain `approved: false`; successor paths
