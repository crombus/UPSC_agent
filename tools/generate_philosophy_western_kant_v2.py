"""Generate Kant as a source-complete learner-v2 Philosophy topic."""
...
import philosophy_western_kant_v2_spec as kant_spec
...
TOPIC_TITLE = "Kant"
TOPIC_FOLDER = "topic-04"
CANONICAL_SEQUENCE_NUMBER = 4
...
ASCII_SPEC = (
    ...
    / "retrofits"
    / "ascii-panel-specs"
    / "philosophy--paper-i-western-philosophy-04-ascii-2026-08-26.json"
)
CONTENT_SPEC_DIR = (
    ROOT
    / "upsc-ai-kit"
...
CANONICAL_OWNER = "upsc-ai-kit\\knowledge\\Philosophy\\paper-1\\western\\Kant.md"
ADVANCED_DOSSIER = (
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Kant\\Kant_Layered-Complete-Learning-Session_2026-08-18.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Western-Philosophy\\"
    "learning-sessions\\Kant\\Kant_Layered-Solved-Practice-Workbook_2026-08-18.md"
)
BASELINE_REPORT = (
    EXPORT_MANIFEST_DIR
    / "philosophy-paper-i-western-philosophy-04-learner-v2-g2-"
...
SESSION_SPECS = kant_spec.SESSION_SPECS
ASCII_PANELS = kant_spec.ASCII_PANELS
REQUIRED_CORE_TERMS = kant_spec.REQUIRED_CORE_TERMS
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            knowledge_root / "assets" / "Kant-Critical-Architecture.png"
        ),
        "main_pdf": (
            notes_root
    ...
    topic.update(
        {
            ...
            "syllabus_mapping": (
                "Philosophy Optional, Paper I, Western Philosophy topic 4: "
                + OFFICIAL_CLAUSE
            ),
...
def make_concept_visual(output: Path) -> None:
    ...
    draw.text(
        (width / 2, 65),
        "KANT: THE CRITICAL ARCHITECTURE OF EXPERIENCE",
        font=title_font,
        fill="#F8FAFC",
        anchor="ma",
    )
    draw.text(
        (width / 2, 148),
        "Rationalist necessity + empirical content, valid only within possible experience",
        font=regular,
        fill="#8DE7F7",
        anchor="ma",
    )
    stages = [
        (
            ...
            ["SPACE: outer sense", "TIME: inner/all sense", "gives intuitions"],
            "FORM OF WHAT IS GIVEN",
        ),
        (
            670,
            "#263D6A",
            ...
            ["12 categories", "synthesis + apperception", "objective experience"],
            "RULES OF PHENOMENAL NATURE",
        ),
        (
            1248,
            "#4A315F",
            ...
            ["Soul / World / God", "regulative unity", "illusion if constitutive"],
            "LIMIT OF SPECULATIVE REASON",
        ),
    ]
    ...
    draw.text(
        (950, 988),
        "CORE RESULT: empirical realism + transcendental idealism",
        font=heading,
        fill="#FFF5D4",
        anchor="mm",
    )
    draw.text(
        (950, 1048),
        "Categories know appearances; noumena mark a boundary, not a second science.",
        font=small,
        fill="#F8FAFC",
        anchor="mm",
    )
...
def advanced_dossier_fragment() -> str:
    ...
    if not match:
        raise ValueError("The Kant advanced dossier section was not found.")
...
def insert_concept_visual(
    ...
    block = (
        f"\n\n![Kantian critical architecture]({image_path})\n\n"
        "*Concept map: sensibility supplies intuitions in space and time; "
        "understanding synthesises them through categories; reason seeks "
        "systematic unity but becomes illusory when it treats Soul, World and "
        "God as constitutively known objects.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "Kant critical-philosophy atlas"
        ),
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "Kant generation remain immutable."
                ),
                "panels": [
                    {
...
PYQ_MARKS_NOTES = (
    "It defines transcendental constitution, distinguishes it from empirical creation, and gives a reasoned Kant-Hegel verdict.",
    "It reconstructs both expositions, names the scientific function, and qualifies Kant through modern geometry without anachronistic dismissal.",
    "It states the trigger, all four pairs and the differentiated resolution before judging the phenomena-noumena link.",
    "It reconstructs the Cartesian inference, isolates the real-predicate premise, uses the thaler example and gives a bounded verdict.",
    "It defines apperception negatively and positively, then integrates it with space, time, synthesis and the categories.",
    "It explains construction and illusion, states every pair, distinguishes mathematical/dynamical resolution and assesses freedom.",
    "It preserves the printed wording, supplies A51/B75 accurately, and shows exactly what Kant retains and rejects from both schools.",
    "It cross-classifies judgments, gives mathematical and causal examples, derives the Copernican solution and states its limit.",
    "It reconstructs Hume's challenge and Kant's Second-Analogy response, then qualifies the scope of the victory.",
    "It explains the classical proof before evaluating Kant, the thaler example and the residual modal objection.",
)
ORIGINAL_MARKS_NOTES = (
    "The answer defines both terms, locates their work in the Critique and shows why the confusion destroys Kant's boundary.",
    "The answer states the heterogeneity problem, derives time as mediator, lists the four temporal forms and evaluates the concealed-art objection.",
    "The answer integrates the synthetic a priori, categories and phenomenal limit, then balances Humean success against Hegelian and Jacobi-style tensions.",
)
...
def owner_pyqs(ledger: str) -> list[str]:
    ...
    for line in ledger.splitlines():
        ...
        if match:
            questions.append(match.group(1).split("📝", 1)[0].strip())
    return questions
...
def render_ascii_pdf_safe(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Kant ASCII Master Flowchart"
        metadata["creator"] = "generate_philosophy_western_kant_v2.py"
    ...
    if not validation["passed"]:
        raise ValueError("Kant ASCII PDF metadata repair broke validation.")
...
def build_record(
    ...
    return {
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_western_kant_v2.py + "
...
def validate_content(
    ...
    if why_count != 13:
        errors.append(f"Expected 13 Why-this-earns-marks notes, found {why_count}.")
...
def run() -> int:
    ...
    if generation != 2:
        raise ValueError(
            f"Expected the next Kant learner-v2 generation to be g2, found g{generation}."
        )
    ...
    if len(source_pyqs) != 10:
        raise ValueError(f"Expected 10 verified owner PYQs, found {len(source_pyqs)}.")
    ...
    flow_metadata["ascii_master_source"] = "manual-authored-kant-spec"
    ...
    changed = {
        ...
        base.relative(ROOT / "tools" / "philosophy_western_kant_v2_spec.py"),
