"""Record a strict sequential Economy learner-v2 topic batch."""
...
import record_economy_completion as topic_record  # noqa: E402
...
def main() -> int:
    ...
    payload = {
        ...
        "scope": (
            f"Economy learner-v2 Topics "
            f"{args.topic_keys[0].removeprefix('economy-')}-"
            f"{args.topic_keys[-1].removeprefix('economy-')}, strictly sequential"
        ),
    ...
    paths: set[str] = {
        refresh.relative(completion.resolve()),
        refresh.relative(ledger.resolve()),
    }
    for key in args.topic_keys:
        for record in topic_record.tracker_records(key):
            paths.update(
                topic_record.changed_files(record, also_changed=args.also_changed)
            )
    ...
    print(json.dumps({k: v for k, v in payload.items() if k != "topics"}, ensure_ascii=False, indent=2))
