"""Generate Individual and State as a source-complete learner-v2 topic.
...
package for ``Individual and State`` only, and never rewrites topics 01-02 or
the planned topics 04-10 beyond the shared indexes and manifests that must
...
import philosophy_socio_political_individual_and_state_v2_spec as topic_spec
...
TOPIC_NUMBER = 3
OFFICIAL_CLAUSE = "Individual and State : Rights; Duties and Accountability."
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Individual-and-State\\"
    "Individual-and-State_Layered-Complete-Learning-Session_2026-08-19.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Socio-Political-Philosophy\\"
    "learning-sessions\\Individual-and-State\\"
    "Individual-and-State_Layered-Solved-Practice-Workbook_2026-08-19.md"
)
...
EXPECTED_PYQ_COUNT = 9
EXPECTED_MCQ_COUNT = 48

ENGLISH_FIRST_REPLACEMENTS = (
    (
        "the moral order through **dharma**, role-based obligation",
        "the moral order through right conduct and moral duty (**dharma**), "
        "role-based obligation",
    ),
    (
        "duty appears through **dharma**, obligation to family",
        "duty appears through right conduct and moral duty (**dharma**), "
        "obligation to family",
    ),
    (
        "Duty in the Indian tradition should not",
        "Duty in the Indian tradition, understood as right conduct (dharma), "
        "should not",
    ),
    (
        "the Indian idiom often begins from duty/dharma rather than",
        "the Indian idiom often begins from duty and right conduct (dharma) "
        "rather than",
    ),
    (
        "a synthesis marries dharma-language to equal dignity",
        "a synthesis marries the language of right conduct (dharma) to equal "
        "dignity",
    ),
    (
        "unconstitutional methods — including *satyāgraha*, non-cooperation",
        "unconstitutional methods — including truth-force (*satyāgraha*), "
        "non-cooperation",
    ),
    (
        "that *satyāgraha* is open to self-suffering",
        "that truth-force (*satyāgraha*) is open to self-suffering",
    ),
    (
        "Gandhi's satyagraha presupposes a regime",
        "Gandhi's truth-force campaign (satyagraha) presupposes a regime",
    ),
    (
        "Freedom requires self-rule (*swaraj*) in the ethical sense",
        "Freedom requires ethical self-rule (*swaraj*)",
    ),
)
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            ...
            / "assets"
            / TOPIC_KEY
            / "rights-duties-accountability-map.png"
        ),
        "main_pdf": (
            NOTES_OUTPUT
            / "notes"
...
def build_manifest(tracker: dict[str, Any], generation: int) -> dict[str, Any]:
    ...
    manifest["section"]["notes"] = (
        ...
        "syllabus/source order. Topics 01-03 are materialised as learner-v2; "
        "topics 04-10 remain planned until separately generated and validated."
    )
    return manifest
...
def make_concept_visual(output: Path) -> None:
    ...
    title_font = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 54)
    ...
    draw.text(
        (width / 2, 54),
        "ONE RELATION, THREE CLAIMS, ONE CHECK",
        font=title_font,
        fill="#F4F8FC",
        anchor="ma",
    )
    draw.text(
        (width / 2, 130),
        "Rights  ·  Duties  ·  Accountability — how public power may bind a person",
        font=regular,
        fill="#7FE0C6",
        anchor="ma",
    )
    ...
    cards = (
        (
            ...
            "#5FD7C2",
            "RIGHTS",
            "what the individual may CLAIM",
            [
                "claim, liberty, power, immunity",
                "natural (Locke) · legal (Bentham)",
                "moral rights judge unjust law",
        ...
        (
            ...
            [
                "legal duty: sanction and remedy",
                "moral duty: binds in conscience",
                "Gandhi: duty is the source of rights",
                "Article 51A, 1976: non-justiciable",
            ],
        ),
        (
            1250,
            ...
            "#DE9BEC",
            "ACCOUNTABILITY",
            "why public power must ANSWER",
            [
                "political: elections, opposition, press",
                "legal: rule of law, judicial review",
                "moral: shareable public reasons",
                "reciprocal, never one-way obedience",
            ],
        ),
    )
    for x, fill, outline, label, subtitle, items in cards:
        ...
        draw.text((x + 280, 250), label, font=heading, fill="#FFFFFF", anchor="ma")
        draw.text((x + 280, 305), subtitle, font=small, fill=outline, anchor="ma")
        y = 390
        for item in items:
            draw.ellipse((x + 40, y + 9, x + 54, y + 23), fill=outline)
            draw.text((x + 74, y), item, font=small, fill="#EDF4F9")
            y += 118
    for start, end in ((630, 660), (1220, 1250)):
        draw.line((start, 550, end, 550), fill="#7FE0C6", width=10)
        draw.polygon(
    ...
    draw.text(
        (width / 2, 1100),
        "subordination; power without answerability becomes domination.",
        font=regular,
        fill="#F4F8FC",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1142),
        "The individual is neither an atom outside society nor a cell inside the state.",
        font=small,
        fill="#A9DFF0",
        anchor="mm",
    )
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+3\.\s+Individual and State\s*(.*?)(?=^##\s+4\.\s+Forms)",
        text,
    )
    if not match:
        raise ValueError(
            "The Individual and State advanced dossier section was not found."
        )
    ...
    block = "\n\n".join(
        [
            ...
            (
                ...
                "philosophical-anarchism dispute, the fair-play against consent "
                "priority quarrel, the will-against-interest refinement and the "
                "answerability-against-sanction distinction are unnecessary for a "
                "competent Core answer on this clause. Use them only once rights, "
                "duties, accountability, the contract tradition and the "
                "obligation-and-disobedience ladder are secure."
            ),
            fragment,
        ]
    )
...
def insert_concept_visual(text: str, concept_visual: Path) -> str:
    ...
    block = (
        f"\n\n![Rights, duties and accountability in one relation]({image_path})\n\n"
        "*Concept map: one relation held together by three claims and one "
        "check — rights are what the individual may claim, duties are what she "
        "owes, and accountability is why public power must answer for its use.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "rights-duties-accountability atlas for the individual and the state"
        ),
        "generated_on": GENERATION_DATE,
        "scope": (
            "Philosophy Optional Paper II Socio-Political Philosophy topic 03 only"
        ),
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "Social and Political Ideals and Sovereignty packages and "
                    "every legacy-v1 Individual and State artifact remain "
                    "immutable."
                ),
                "panels": [
                    {
...
def owner_pyqs(ledger: str) -> list[str]:
    ...
    for line in ledger.splitlines():
        if "[Individual and State]" not in line or not line.lstrip().startswith("- ("):
            continue
...
def render_ascii_pdf(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Individual and State ASCII Master Flowchart"
        metadata["creator"] = (
            "generate_philosophy_socio_political_individual_and_state_v2.py"
        )
    ...
    if not validation["passed"]:
        raise ValueError("Individual and State ASCII PDF validation failed.")
...
def build_record(
    ...
    flow_metadata["ascii_master_source"] = (
        "manual-authored-individual-and-state-spec"
    )
    ...
    return {
        ...
        "command": (
            ...
            "— Socio-Political Philosophy — Individual and State"
        ),
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_socio_political_individual_and_state_v2.py"
...
def verify_final_state(generation: int, paths: dict[str, Path]) -> dict[str, Any]:
    ...
    if states[2].package_state != "generated":
        raise ValueError(
            f"Topic 03 state is {states[2].package_state!r}, expected generated."
        )
    earlier = [state.package_state for state in states[:2]]
    if any(state != "generated" for state in earlier):
        raise ValueError(
            "Topics 01-02 must remain generated: " + ", ".join(earlier)
        )
    later = [state.package_state for state in states[3:]]
    if any(state != "planned" for state in later):
        raise ValueError(
            "Topics 04-10 were unexpectedly marked beyond planned: " + ", ".join(later)
        )
    ...
    command = (
        "Generate learner-v2 topic: Philosophy Optional — Philosophy Paper II — "
        "Socio-Political Philosophy — Individual and State — Regenerate"
    )
    ...
    return {
        ...
        "topics_04_10_states": later,
        "approval": states[2].approval_state,
        "validation": states[2].validation_state,
...
def run() -> int:
    ...
    assembled = philosophy_v2.assemble_legacy(
        LegacyTopic(TOPIC_KEY, TOPIC_TITLE),        retained_main,
        retained_workbook,
    )
    assembled = re.sub(
    ...
    for index, stage in enumerate(graphical_data["stages"][:-1]):
        ...
        stage["matrix"] = (
            authored_matrix if stage["layout"] == "matrix" else []
        )
    ...
    changed: set[str] = {
        ...
        relative(
            ROOT
            / "tools"
            / "philosophy_socio_political_individual_and_state_v2_spec.py"
        ),
        relative(ROOT / "tools" / "validate_v2_export.py"),
        relative(ROOT / "tools" / "test_v2_export_foundation.py"),
        relative(MANIFEST),
        relative(ASCII_SPEC),
