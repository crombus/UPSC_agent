"""Generate Forms of Government as a source-complete learner-v2 topic.
...
package for ``Forms of Government`` only, and never rewrites topics 01-03 or
the planned topics 05-10 beyond the shared indexes and manifests that must
...
import philosophy_socio_political_forms_of_government_v2_spec as topic_spec
...
TOPIC_NUMBER = 4
OFFICIAL_CLAUSE = "Forms of Government : Monarchy; Theocracy and Democracy."
...
ASCII_SPEC = (
    ...
    / "retrofits"
    / "ascii-panel-specs"
    / f"philosophy--{SECTION_KEY}-04-ascii-{GENERATION_DATE}.json"
)
CONTENT_SPEC_DIR = (
    ROOT
    / "upsc-ai-kit"
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Forms-of-Government\\"
    "Forms-of-Government_Layered-Complete-Learning-Session_2026-08-19.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Socio-Political-Philosophy\\"
    "learning-sessions\\Forms-of-Government\\"
    "Forms-of-Government_Layered-Solved-Practice-Workbook_2026-08-19.md"
)
...
ENGLISH_FIRST_REPLACEMENTS = (
    (
        ...
        "politics should express the general will (*volonté générale*) of the "
        "people",
    ),
    (
        "Gandhi's decentralised ideal: village \"swaraj\" -- self-reliant, participatory\n"
    ...
    (
        "Gandhi's decentralised ideal of village SWARAJ",
        "Gandhi's decentralised ideal of village self-rule (SWARAJ)",
    ),
    ("village swaraj.", "village self-rule (swaraj)."),
)
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            ...
            / "assets"
            / TOPIC_KEY
            / "monarchy-theocracy-democracy-map.png"
        ),
        "main_pdf": (
            NOTES_OUTPUT
            / "notes"
...
def build_manifest(tracker: dict[str, Any], generation: int) -> dict[str, Any]:
    ...
    manifest["section"]["notes"] = (
        ...
        "syllabus/source order. Topics 01-04 are materialised as learner-v2; "
        "topics 05-10 remain planned until separately generated and validated."
    )
    return manifest
...
def make_concept_visual(output: Path) -> None:
    ...
    title_font = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 52)
    ...
    draw.text(
        (width / 2, 46),
        "THREE FORMS, THREE TITLES TO RULE, ONE SET OF TESTS",
        font=title_font,
        fill="#F4F8FC",
        anchor="ma",
    )
    draw.text(
        (width / 2, 120),
        "Monarchy  ·  Theocracy  ·  Democracy — who rules, by what title, "
        "for whose good, under what limits",
        font=regular,
        fill="#7FE0C6",
        anchor="ma",
    )
    ...
    cards = (
        (
            ...
            "#F0C177",
            "MONARCHY",
            "title: heredity and tradition",
            [
                "absolute rules; constitutional reigns",
                "divine right sacralises the crown",
                "objections: equality, merit,",
                "accountability, freedom",
                "above politics = symbolic neutrality",
            ],
        ),
        (
            660,
            ...
            "#C79BEC",
            "THEOCRACY",
            "title: revelation and sacred law",
            [
                "sovereignty of God, not the people",
                "clerical mediation of divine command",
                "defect: the interpretation monopoly",
                "who speaks for God?",
                "unequal citizenship, bound conscience",
            ],
        ),
        (
            1250,
            ...
            "#5FD7C2",
            "DEMOCRACY",
            "title: consent and equality",
            [
                "direct · representative · procedural",
                "substantive · participatory ·",
                "deliberative",
                "liberal democracy limits the majority",
                "form can survive while substance goes",
            ],
        ),
    )
    for x, fill, outline, label, subtitle, items in cards:
        ...
        draw.text((x + 280, 232), label, font=heading, fill="#FFFFFF", anchor="ma")
        draw.text((x + 280, 286), subtitle, font=small, fill=outline, anchor="ma")
        y = 370
        for item in items:
            draw.ellipse((x + 36, y + 8, x + 50, y + 22), fill=outline)
            draw.text((x + 70, y), item, font=small, fill="#EDF4F9")
            y += 100
    for start, end in ((630, 660), (1220, 1250)):
        draw.line((start, 545, end, 545), fill="#7FE0C6", width=10)
        draw.polygon(
            [(end - 16, 529), (end, 545), (end - 16, 561)],
            fill="#7FE0C6",
        )

    draw.rounded_rectangle(
        (120, 950, width - 120, 1235),
        20,
        fill="#101F31",
        outline="#F5CE79",
        width=3,
    ...
    draw.text(
        (width / 2, 995),
        "THE AXES ON WHICH THE COMPARISON IS DECIDED",
        font=heading,
        fill="#FFF2D2",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1055),
        "source of authority · type of legitimacy claimed · succession or "
    ...
    draw.text(
        (width / 2, 1100),
        "relation to law · treatment of dissent · equality of citizenship · "
        "CAPACITY TO CORRECT ERROR",
        font=regular,
        fill="#F4F8FC",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1155),
        "Democracy's claim is not guaranteed wisdom but equal standing, public "
        "justification",
        font=small,
        fill="#A9DFF0",
        anchor="mm",
    ...
    draw.text(
        (width / 2, 1195),
        "and the institutionalised correction of error — the axis candidates "
        "most often forget.",
        font=small,
        fill="#A9DFF0",
        anchor="mm",
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+4\.\s+Forms of Government\s*(.*?)(?=^##\s+5\.\s+Political)",
        text,
    )
    if not match:
        raise ValueError(
            "The Forms of Government advanced dossier section was not found."
        )
...
def insert_advanced_dossier(text: str, fragment: str) -> str:
    ...
    block = "\n\n".join(
        [
            ...
            (
                "> **Classification: OPTIONAL ADVANCED.** The residual dispute "
                "over why democracy is justified, the epistocracy quarrel and "
                "the republican anti-domination reading are unnecessary for a "
                "competent Core answer on this clause. Use them only once "
                "monarchy, theocracy, democracy, the classical classifications "
                "and the legitimacy typology are secure."
            ),
            fragment,
        ]
    )
...
def insert_concept_visual(text: str, concept_visual: Path) -> str:
    ...
    block = (
        f"\n\n![Monarchy, theocracy and democracy on shared axes]({image_path})\n\n"
        "*Concept map: three forms distinguished by the title each claims to "
        "rule — heredity, revelation and consent — and then judged on one "
        "shared set of axes ending in the capacity to correct error.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "monarchy-theocracy-democracy classification and legitimacy atlas"
        ),
        "generated_on": GENERATION_DATE,
        "scope": (
            "Philosophy Optional Paper II Socio-Political Philosophy topic 04 only"
        ),
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "Social and Political Ideals, Sovereignty and Individual "
                    "and State packages and every legacy-v1 Forms of "
                    "Government artifact remain immutable."
                ),
                "panels": [
                    {
...
def owner_pyqs(ledger: str) -> list[str]:
    ...
    for line in ledger.splitlines():
        if "[Forms of Government]" not in line or not line.lstrip().startswith("- ("):
            continue
...
def render_ascii_pdf(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Forms of Government ASCII Master Flowchart"
        metadata["creator"] = (
            "generate_philosophy_socio_political_forms_of_government_v2.py"
        )
    ...
    if not validation["passed"]:
        raise ValueError("Forms of Government ASCII PDF validation failed.")
...
def build_record(
    ...
    flow_metadata["ascii_master_source"] = "manual-authored-forms-of-government-spec"
    ...
    return {
        ...
        "command": (
            ...
            "— Socio-Political Philosophy — Forms of Government"
        ),
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_socio_political_forms_of_government_v2.py"
...
def verify_final_state(generation: int, paths: dict[str, Path]) -> dict[str, Any]:
    ...
    if states[3].package_state != "generated":
        raise ValueError(
            f"Topic 04 state is {states[3].package_state!r}, expected generated."
        )
    earlier = [state.package_state for state in states[:3]]
    if any(state != "generated" for state in earlier):
        raise ValueError(
            "Topics 01-03 must remain generated: " + ", ".join(earlier)
        )
    later = [state.package_state for state in states[4:]]
    if any(state != "planned" for state in later):
        raise ValueError(
            "Topics 05-10 were unexpectedly marked beyond planned: " + ", ".join(later)
        )
    ...
    command = (
        "Generate learner-v2 topic: Philosophy Optional — Philosophy Paper II — "
        "Socio-Political Philosophy — Forms of Government — Regenerate"
    )
    ...
    return {
        ...
        "topics_05_10_states": later,
        "approval": states[3].approval_state,
        "validation": states[3].validation_state,
...
def run() -> int:
    ...
    changed: set[str] = {
        ...
        relative(
            ROOT
            / "tools"
            / "philosophy_socio_political_forms_of_government_v2_spec.py"
        ),
