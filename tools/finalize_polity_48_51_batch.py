"""Finalize validation and publication records for the Polity 48-51 batch."""
...
import generate_polity_48_51_sequential as batch
...
POLITY_FLOW_VALIDATION = (
    EXPORTS / "polity-01-51-flow-learning-2026-08-25-validation.json"
)
ALL_FLOW_VALIDATION = (
...
REPORT = (
    ...
    / "POLITY-48-51-SEQUENTIAL-BATCH-REPORT.md"
)
...
def repair_mcq_rotation() -> None:
    ...
    for key in [f"polity-{number:02d}" for number in range(48, 52)]:
...
def write_case_evidence() -> dict[str, Any]:
    ...
    topic_keys = [f"polity-{number:02d}" for number in range(1, 52)]
...
def run_flow_publications(tests_passed: int) -> None:
    ...
    subprocess.run(
        [
            ...
            "polity-",
            "--report-path",
            "notes\\Flow-Learning\\POLITY-01-51-FLOW-LEARNING-REPORT.md",
            "--validation-path",
...
def build_validation(tests_passed: int) -> dict[str, Any]:
    ...
    checks = {
        ...
        "prior_80_clean_hashes_preserved": not state[
            "existing_clean_hash_mismatches"
        ],
        "prior_80_flow_hashes_preserved": not state[
            "existing_flow_hash_mismatches"
        ],
        "prior_generated_topics_preserved": not state[
            "prior_generated_topic_hash_mismatches"
        ],
        "four_item_inventory_84": four_tracker["topic_count"] == 84
        and four["status"] == "passed",
        "polity_flow_inventory_51": (
            polity_flow["summary"]["topic_folder_count"] == 51
            and polity_flow["status"] == "passed"
        ),
        "full_flow_inventory_84": (
            all_flow["summary"]["topic_folder_count"] == 84
...
def write_report(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Polity 48-51 Sequential Batch Report",
        "",
        "- Status: **PASSED**",
        "- Execution: strict order `polity-48 -> polity-49 -> polity-50 -> polity-51`.",
        "- Gates: A-J completed for each topic before the next topic started.",
        "- Approval: all four learner-v2 g2 records remain `approved: false`; legacy-v1 history is retained.",
        "- Final clean four-item library: **84 topics**. Final Flow Learning inventory: **84 topics**, including **Polity 51**.",
        "",
    ...
    lines.extend(
        [
            ...
            f"- Graphical stages: **{totals['graphical_core_stages']} core + 4 enrichment**",
            ...
            "- Existing 80 clean packages: prior canonical artifact files checked, 0 hash mismatches.",
            "- Existing 80 Flow Learning topics: prior topic artifacts checked, 0 hash mismatches.",
            f"- Full four-item library: 84 topics; {payload['inventories']['four_item_links_checked']} links checked; 0 broken.",
            f"- Polity Flow Learning: 51 topics; {payload['inventories']['polity_flow_links_checked']} links checked; 0 broken.",
            f"- Full Flow Learning: 84 topics; {payload['inventories']['full_flow_topic_artifact_files_checked']} topic artifacts hash-checked; {payload['inventories']['full_flow_links_checked']} navigation links checked; 0 broken; source/destination PDF and TXT hashes equal.",
...
def write_changed_files() -> list[str]:
    ...
    explicit = {
        ...
        ROOT / "tools" / "generate_polity_48_51_sequential.py",
        ROOT / "tools" / "finalize_polity_48_51_batch.py",
        ...
        / "Polity"
        / "advanced"
        / "48_Ministries-Departments-and-Central-Secretariat.md",
        ROOT
        / "upsc-ai-kit"
        / "knowledge"
        / "Polity"
        / "advanced"
        / "49_Regulatory-State-and-Quasi-Judicial-Institutions.md",
        ROOT
        / "upsc-ai-kit"
        / "knowledge"
        / "Polity"
        / "advanced"
        / "50_Concept-of-the-Constitution.md",
        ROOT
        / "upsc-ai-kit"
        / "knowledge"
        / "Polity"
        / "advanced"
        / "51_Rights-and-Liabilities-of-the-Government.md",
        ...
        / "notes"
        / "Flow-Learning"
        / "POLITY-01-51-FLOW-LEARNING-REPORT.md",
        FINAL_VALIDATION,
        REPORT,
        CASE_EVIDENCE,
        POLITY_FLOW_VALIDATION,
        ...
        / "Polity"
        / "Topic-PDFs"
        / "48_Ministries-Departments-and-Central-Secretariat_Deep-Learning.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Session-Level-Topic-PDFs"
        / "48_Ministries-Departments-and-Central-Secretariat_Session-Level.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Topic-PDFs"
        / "49_Regulatory-State-and-Quasi-Judicial-Institutions_Deep-Learning.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Session-Level-Topic-PDFs"
        / "49_Regulatory-State-and-Quasi-Judicial-Institutions_Session-Level.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Topic-PDFs"
        / "50_Concept-of-the-Constitution_Deep-Learning.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Session-Level-Topic-PDFs"
        / "50_Concept-of-the-Constitution_Session-Level.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Topic-PDFs"
        / "51_Rights-and-Liabilities-of-the-Government_Deep-Learning.pdf",
        ROOT
        / "notes"
        / "Polity"
        / "Session-Level-Topic-PDFs"
        / "51_Rights-and-Liabilities-of-the-Government_Session-Level.pdf",
    }
    roots = [
