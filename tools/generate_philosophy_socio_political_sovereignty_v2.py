"""Generate Sovereignty as a source-complete learner-v2 Socio-Political topic.
...
package for ``Sovereignty`` only, and never touches topic 01 or the planned
topics 03-10 beyond their existing manifest entries.
"""
...
import philosophy_socio_political_sovereignty_v2_spec as topic_spec
...
TOPIC_NUMBER = 2
OFFICIAL_CLAUSE = "Sovereignty : Austin, Bodin, Laski, Kautilya."
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Sovereignty\\"
    "Sovereignty_Layered-Complete-Learning-Session_2026-08-19.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Socio-Political-Philosophy\\"
    "learning-sessions\\Sovereignty\\"
    "Sovereignty_Layered-Solved-Practice-Workbook_2026-08-19.md"
)
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            ...
            / "assets"
            / TOPIC_KEY
            / "sovereignty-location-structure-limit-map.png"
        ),
        "main_pdf": (
            NOTES_OUTPUT
            / "notes"
...
def build_manifest(tracker: dict[str, Any], generation: int) -> dict[str, Any]:
    ...
    manifest["section"]["notes"] = (
        ...
        "syllabus/source order. Topics 01-02 are materialised as learner-v2; "
        "topics 03-10 remain planned until separately generated and validated."
    )
    return manifest
...
def make_concept_visual(output: Path) -> None:
    ...
    heading = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 36)
    ...
    for x, fill, outline, label, items in cards:
        ...
        draw.text((x + 280, 296), label, font=heading, fill="#FFFFFF", anchor="ma")
        y = 410
        for item in items:
            draw.ellipse((x + 46, y + 8, x + 60, y + 22), fill=outline)
            draw.text((x + 80, y), item, font=regular, fill="#EEF5F9")
            y += 92
    for start, end in ((630, 660), (1220, 1250)):
        draw.line((start, 545, end, 545), fill="#5FD7F2", width=10)
        draw.polygon(
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+2\.\s+Sovereignty\s*(.*?)(?=^##\s+3\.\s+Individual)",
        text,
    )
    if not match:
        raise ValueError("The Sovereignty advanced dossier section was not found.")
...
def insert_advanced_dossier(text: str, fragment: str) -> str:
    ...
    block = "\n\n".join(
        [
            ...
            (
                "> **Classification: OPTIONAL ADVANCED.** Constituent power, legal "
                "pluralism, emergency sovereignty and global-governance debates are "
                "unnecessary for a competent Core answer on this clause, and should "
                "be used only after Bodin, Austin, Laski and Kautilya are secure."
            ),
            fragment,
        ]
    )
...
def insert_concept_visual(text: str, concept_visual: Path) -> str:
    ...
    block = (
        f"\n\n![Sovereignty located, structured and limited]({image_path})\n\n"
        "*Concept map: the same question — where final authority is located, how "
        "it is structured and how far it is limited — is answered differently by "
        "Bodin, Austin, Laski and Kautilya.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "Sovereignty location-structure-limit atlas"
        ),
        "generated_on": GENERATION_DATE,
        "scope": (
            "Philosophy Optional Paper II Socio-Political Philosophy topic 02 only"
        ),
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "Social and Political Ideals package and every legacy-v1 "
                    "Sovereignty artifact remain immutable."
                ),
                "panels": [
                    {
...
def owner_pyqs(ledger: str) -> list[str]:
    ...
    for line in ledger.splitlines():
        if "[Sovereignty]" not in line or not line.lstrip().startswith("- ("):
            continue
...
def render_ascii_pdf(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Sovereignty ASCII Master Flowchart"
        metadata["creator"] = "generate_philosophy_socio_political_sovereignty_v2.py"
    ...
    if not validation["passed"]:
        raise ValueError("Sovereignty ASCII PDF validation failed.")
...
def build_record(
    ...
    flow_metadata["ascii_master_source"] = "manual-authored-sovereignty-spec"
    ...
    return {
        ...
        "command": (
            ...
            "— Socio-Political Philosophy — Sovereignty"
        ),
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_socio_political_sovereignty_v2.py + "
...
def verify_final_state(generation: int, paths: dict[str, Path]) -> dict[str, Any]:
    ...
    if states[1].package_state != "generated":
        raise ValueError(
            f"Topic 02 state is {states[1].package_state!r}, expected generated."
        )
    later = [state.package_state for state in states[2:]]
    if any(state != "planned" for state in later):
        raise ValueError(
            "Topics 03-10 were unexpectedly marked beyond planned: " + ", ".join(later)
        )
    ...
    command = (
        "Generate learner-v2 topic: Philosophy Optional — Philosophy Paper II — "
        "Socio-Political Philosophy — Sovereignty — Regenerate"
    )
    ...
    return {
        ...
        "topics_03_10_states": later,
        "approval": states[1].approval_state,
        "validation": states[1].validation_state,
...
def run() -> int:
    ...
    changed: set[str] = {
        ...
        relative(ROOT / "tools" / "philosophy_socio_political_sovereignty_v2_spec.py"),
