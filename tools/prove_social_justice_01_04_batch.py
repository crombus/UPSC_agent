"""Programmatic proof of the Social Justice 01-04 learner-v2 per-topic contracts."""
...
READ_ONLY_PREFIXES = (
    ...
    "upsc-ai-kit\\knowledge\\Social-Justice\\basic\\",
    "upsc-ai-kit\\knowledge\\Social-Justice\\advanced\\",
    ...
    for key in KEYS:
        ...
        completion = json.loads(
            (
                ...
                / "manifests"
                / "exports"
                / f"{key}-learner-v2-g{generation}-2026-09-02-completion.json"
        ...
        flow = record.get("continuous_core_first") or {}
        flow_files = [
            ROOT / str(flow.get(field, ""))
            for field in ("master_image", "poster_pdf", "tiled_pdf", "ascii_master")
            if flow.get(field)
        ]
        changed = (
            ROOT
            ...
            / "manifests"
            / "exports"
            / f"{key}-learner-v2-g{generation}-2026-09-02-changed-files.txt"
        ...
        checks = {
            ...
            "flowchart_package_present": bool(flow_files)
            and all(path.is_file() for path in flow_files),
