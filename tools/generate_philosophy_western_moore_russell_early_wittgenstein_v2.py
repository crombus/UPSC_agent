"""Generate Moore, Russell and Early Wittgenstein as a learner-v2 topic."""
...
import philosophy_western_moore_russell_early_wittgenstein_v2_spec as topic_spec
...
TOPIC_TITLE = "Moore, Russell and Early Wittgenstein"
TOPIC_FOLDER = "topic-06"
CANONICAL_SEQUENCE_NUMBER = 6
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Moore-Russell-Early-Wittgenstein\\"
    "Moore-Russell-Early-Wittgenstein_Layered-Complete-Learning-Session_"
    "2026-08-18.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Western-Philosophy\\"
    "learning-sessions\\Moore-Russell-Early-Wittgenstein\\"
    "Moore-Russell-Early-Wittgenstein_Layered-Solved-Practice-Workbook_"
    "2026-08-18.md"
)
BASELINE_REPORT = (
    EXPORT_MANIFEST_DIR
    / "philosophy-paper-i-western-philosophy-06-learner-v2-g2-"
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            knowledge_root / "assets" / "Analytic-Turn-Trio-Overview.png"
        ),
        "main_pdf": (
            notes_root
    ...
    topic.update(
        {
            ...
            "syllabus_mapping": (
                "Philosophy Optional, Paper I, Western Philosophy topic 6: "
                + OFFICIAL_CLAUSE
            ),
...
def make_concept_visual(output: Path) -> None:
    ...
    draw.text(
        (width / 2, 65),
        "THE ANALYTIC TURN: MOORE, RUSSELL AND EARLY WITTGENSTEIN",
        font=title_font,
        fill="#F8FAFC",
        anchor="ma",
    )
    draw.text(
        (width / 2, 148),
        "Common sense -> logical analysis -> limits of representation",
        font=regular,
        fill="#8DE7F7",
        anchor="ma",
    )
    stages = [
        (
            ...
            "#173B55",
            "MOORE",
            "Protect what is better known",
            ["common-sense propositions", "hands and external world", "awareness != object"],
            "CERTAINTY BEFORE SYSTEM",
        ),
        (
            670,
            "#263D6A",
            "RUSSELL",
            "Expose logical form",
            ["facts and atomic propositions", "constructions replace inference", "descriptions disappear"],
            "GRAMMAR != LOGIC",
        ),
        (
            1248,
            "#4A315F",
            "EARLY WITTGENSTEIN",
            "Delimit representation",
            ["facts, objects, states of affairs", "picture through logical form", "saying versus showing"],
            "PICTURE != RESEMBLANCE",
        ),
    ]
    ...
    draw.text(
        (950, 988),
        "COMMON MOVEMENT, DIFFERENT STANDARDS",
        font=heading,
        fill="#FFF5D4",
        anchor="mm",
    )
    draw.text(
        (950, 1048),
        "Moore secures knowledge; Russell reconstructs form; Wittgenstein marks its limits.",
        font=small,
        fill="#F8FAFC",
        anchor="mm",
    )
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+6\.\s+Moore[–-]Russell[–-]Early Wittgenstein\s*(.*?)"
        r"(?=^##\s+7\.\s+Logical Positivism)",
        text,
    )
    if not match:
        raise ValueError("The Moore-Russell-Wittgenstein dossier section was not found.")
...
def insert_concept_visual(
    ...
    block = (
        f"\n\n![Analytic turn trio overview]({image_path})\n\n"
        "*Concept map: Moore protects common-sense knowledge, Russell reveals "
        "logical form and constructs problematic entities, and the early "
        "Wittgenstein delimits meaningful representation through saying and showing.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "Moore-Russell-Early-Wittgenstein analytic-philosophy atlas"
        ),
        "generated_on": GENERATION_DATE,
        "scope": "Philosophy Optional Paper I Western Philosophy topic 06 only",
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "topic-06 generation remain immutable."
                ),
                "panels": [
                    {
...
PYQ_MARKS_NOTES = (
    "It derives saying/showing from picture theory, distinguishes factual saying from logical manifestation, and evaluates the ladder without reducing value-discourse to falsehood.",
    "It connects descriptions to atomism through elimination of apparent constituents, uses existence-uniqueness-predication and qualifies the claimed route to simples.",
    "It defines Moorean common-sense realism through truisms, knowing-versus-analysis and comparative certainty rather than merely repeating the hands gesture.",
    "It reconstructs the blue-consciousness distinction, explains transparency and limits the conclusion to the failure of an identity-based idealist argument.",
    "It distinguishes logical from physical atoms, relates atomic facts to atomic propositions and qualifies the independence and final-analysis claims.",
    "It presents Moore's qualified truism list, answers the sceptic's dialectical objection and distinguishes a valid proof from a complete theory of knowledge.",
    "It reads proposition 7 through saying/showing and 6.54, adds the self-referential objection and reaches a graded verdict on elucidatory nonsense.",
    "It moves from grammatical to logical form, explains facts and atomic propositions and shows why analysis is methodological rather than physical decomposition.",
    "It distinguishes pictorial from logical form, maps names to objects and explains why representation requires shared possibility rather than visual resemblance.",
    "It stages the King-of-France analysis, explains why existence failure makes the conjunction false and includes Strawson's presupposition challenge.",
    "It reconstructs Moore's truisms and certainty comparison, then tests whether common sense supplies an argument or only an epistemic starting point.",
    "It defines incomplete symbols contextually, connects descriptions and constructions to atomism and evaluates whether analysis really reaches independent simples.",
    "It eliminates the golden mountain through quantificational form, avoids Meinongian commitment and keeps falsehood distinct from meaninglessness.",
    "It uses the exact blue-green-consciousness contrast, explains diaphanous awareness and states why Moore's target is narrower than all idealism.",
)
ORIGINAL_MARKS_NOTES = (
    "The answer distinguishes proof from sceptical persuasion, gives Moore's conditions and states why certainty comparison remains philosophically significant.",
    "The answer relates grammatical form, acquaintance, description, construction and incomplete symbols without treating them as interchangeable labels.",
    "The answer compares all three programmes, reconstructs one central argument from each and reaches a qualified verdict on analysis as anti-metaphysical method.",
)
...
def owner_pyqs(ledger: str) -> list[str]:
    ...
    for line in ledger.splitlines():
        if "[Moore–Russell–Early Wittgenstein]" not in line:
            continue
...
def render_ascii_pdf_safe(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Moore, Russell and Early Wittgenstein ASCII Master Flowchart"
        metadata["creator"] = (
            "generate_philosophy_western_moore_russell_early_wittgenstein_v2.py"
        )
    ...
    if not validation["passed"]:
        raise ValueError("Topic-06 ASCII PDF metadata repair broke validation.")
...
def build_record(
    ...
    return {
        ...
        "command": (
            ...
            "— Western Philosophy — Moore, Russell and Early Wittgenstein"
        ),
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_western_moore_russell_early_wittgenstein_v2.py + "
...
def run() -> int:
    ...
    if generation != EXPECTED_GENERATION:
        raise ValueError(
            "Tracker resolution changed during generation: the audited target was "
            f"g{EXPECTED_GENERATION}, but the latest tracker now resolves g{generation}."
        )
    ...
    graphical_data["stages"][0]["groups"] = topic_spec.GRAPHICAL_STAGE_ZERO_GROUPS
    ...
    flow_metadata["ascii_master_source"] = (
        "manual-authored-moore-russell-early-wittgenstein-spec"
    )
