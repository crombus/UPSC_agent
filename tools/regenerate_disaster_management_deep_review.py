"""Deep-review and immutably regenerate all Disaster Management topic packages."""
...
FLOW_SUBJECT = "Disaster-Management"
...
CONTENT_SPECS = (
    ...
    / "manifests"
    / "v2"
    / "disaster-management--subject-wide-syllabus-content-specs"
)
REFRESHED_KNOWLEDGE = (
    ROOT
    / "upsc-ai-kit"
...
WORKFLOW = "disaster-management-deep-review-immutable-successor"
...
def topics() -> list[Topic]:
    ...
    if [topic.number for topic in result] != list(range(1, 16)):
        raise ValueError("Disaster Management manifest must contain topics 01-15 in order.")
    return result
...
def normalize_topic_pyq_metadata(topic: Topic, markdown: str) -> str:
    ...
    if topic.topic_key == "disaster-management-16":
        markdown = markdown.replace(
    ...
    if topic.topic_key == "disaster-management-17":
        for number, title in (
    ...
    if topic.topic_key == "disaster-management-21":
        for number, title in (
    ...
    if topic.topic_key == "disaster-management-25":
        markdown = markdown.replace(
    ...
    if topic.topic_key == "disaster-management-26":
        markdown = re.sub(
...
| H | What is the Disaster Management cutoff? | Gupta/post-Gupta scholasticism → Kumarila/Dharmakirti/Shankara boundary → later owners | importing mature medieval Vedanta |
...
def _base_augment_topic_semantic_content(
    ...
    *,
    ...
    if topic.topic_key == "disaster-management-27":
        supplement = (
    ...
    if topic.topic_key == "disaster-management-25":
        ...
        if insertion not in markdown:
            raise ValueError(f"{topic.topic_key}: required insertion point is absent.")
        return markdown.replace(insertion, supplement + "\n\n" + insertion, 1)
    if topic.topic_key == "disaster-management-24":
        replacements = {
            (
        ...
        if insertion not in markdown:
            raise ValueError(f"{topic.topic_key}: required insertion point is absent.")
        return markdown.replace(insertion, supplement + "\n\n" + insertion, 1)
    if topic.topic_key != "disaster-management-23":
        return markdown
    replacements = {
        (
...
|---|---|
| Syllabus boundary | Complete Disaster Management Core is taught before optional enrichment. |
...
def remove_invalid_topic_mcq_audits(topic: Topic, markdown: str) -> str:
    if topic.topic_key != "disaster-management-02":
        return markdown
    return re.sub(
...
def _base_render_artifacts(
    ...
    flow_metadata["ascii_master_source"] = (
        "manual-authored-disaster-management-deep-review-spec"
    )
    output_files = [
        paths["markdown"],
        paths["workbook_markdown"],
...
""",
    ...
    write_text(
        report,
        f"""# Deep Content Review — Disaster Management {topic.number:02d}: {topic.title}
...
def write_batch(path: Path, rows: list[dict[str, Any]], changed: set[str]) -> None:
    write_text(
        path,
        "# Disaster Management Deep Review Batch\n\n"
        + "\n".join(
    ...
    changed.update(
        {
            ...
            "notes\\Final-Learning-Packages\\Disaster Management\\INDEX.md",
            (
                "notes\\Final-Learning-Packages\\Disaster Management\\"
...
# Disaster Management direct overrides. These definitions intentionally use
...
DISASTER_MANAGEMENT_TEST_MODULES: tuple[str, ...] = ()
DISASTER_MANAGEMENT_GENERATOR_MODULES = tuple(
    f"generate_disaster_management_{number:02d}_sequential"
...
def topics() -> list[Topic]:
    ...
    if len(manifest_rows) < 18:
        raise ValueError("Disaster Management manifest must retain topics 01-18.")
    status = load(STATUS)
    ...
    for number, row in enumerate(manifest_rows, 1):
        expected_key = f"disaster-management-{number:02d}"
        if row.get("topic_key") != expected_key:
            raise ValueError("Disaster Management manifest keys must be contiguous and ordered.")
        if number <= 18 or expected_key in latest_by_key:
    ...
    review_keys = {
        ...
        if row["topic_key"].startswith("disaster-management-")
    }
    if review_keys and not review_keys.issubset(expected):
        raise ValueError("Disaster Management REVIEW-TRACKER contains out-of-manifest topics.")
    return result


DISASTER_MANAGEMENT_REVIEW_POINTS: dict[int, tuple[str, str, str]] = {
    1: (
        "Disaster risk joins hazard, exposure, vulnerability and capacity; resilience is the ability to resist, absorb, adapt, transform and recover without confusing recovery output with resilience outcome.",
        "Hazard occurrence is not disaster impact; vulnerability is not exposure; capacity is not resilience; the Sendai Framework's four priorities are not its seven global targets, and global targets are not national indicators.",
        "Define each risk term, show the prevention-to-Build-Back-Better cycle, and retain source/date/status for every Sendai target, indicator or current risk claim.",
    ),
    2: (
        "The Disaster Management Act, 2005 distributes policy, coordination, training, response and local planning roles across NDMA, NEC, NIDM, NDRF, SDMA, SEC and DDMA rather than creating one interchangeable chain.",
        "NDMA is not NEC, NIDM is not NDRF, SDMA is not DDMA, a National Plan is not a State or District Plan, and a notified fund window is not an allocation, release, expenditure or legal entitlement.",
        "State the enacted, amended, notified, draft or guideline status; cite the exact institution, level, plan, fund, date and legal character before claiming implementation or outcome.",
    ),
    3: (
        "Community-based DRR begins with local risk knowledge, inclusive mapping, trained volunteers, accessible warnings, shelters, drills, social protection and accountable local-government capacity.",
        "Participation is not consultation; volunteer enrolment is not trained deployable capacity; a drill is not proven readiness; equal messaging is not equitable protection for women, children, older persons, persons with disabilities or migrants.",
        "Name the local actor, vulnerable group, access barrier, capacity input, last-mile action and measured protection result; separate scheme design, coverage and outcome.",
    ),
    4: (
        "An end-to-end early-warning chain runs observation -> detection -> modelling/forecast -> risk interpretation -> authorised alert -> dissemination -> receipt -> comprehension -> trusted last-mile action -> feedback.",
        "Observation is not forecast; forecast is not watch; watch is not warning; warning is not evacuation order; message dissemination is not receipt, comprehension or protective action.",
        "Attribute the competent agency and hazard, then preserve issue time, lead time, spatial scale, threshold, uncertainty, channel, last-mile actor and operational status.",
    ),
    5: (
        "Earthquake risk combines seismic hazard, exposed people and assets, structural and non-structural vulnerability, code compliance, retrofitting, land-use control and response preparedness.",
        "Magnitude is not intensity; earthquake occurrence is not disaster impact; structural mitigation is not non-structural mitigation; code publication is not enforcement or compliant construction.",
        "Source every magnitude, intensity, zone, casualty or building claim with date, unit and event status; connect engineering measure to failure mode and residual risk.",
    ),
    6: (
        "Tsunami risk management links offshore detection, seismic and sea-level confirmation, modelling, INCOIS warning, coastal dissemination, signed evacuation routes, vertical or horizontal evacuation, shelters and safe return.",
        "Earthquake detection is not tsunami confirmation; advisory/watch/warning is not evacuation; coastal hazard is not coastal disaster; warning capability is not last-mile compliance or avoided loss.",
        "State the issuing agency, bulletin category, time, coastal reach and protective-action owner; distinguish ecosystem-based coastal buffering from engineered protection and emergency evacuation.",
    ),
    7: (
        "Cyclone disaster risk joins wind, storm surge, heavy rainfall, waves, river backflow, exposed coast, housing and livelihood vulnerability, forecast-based action, shelters, evacuation and resilient recovery.",
        "Cyclone category is not storm-surge height; forecast landfall is not observed landfall; warning is not evacuation; shelter capacity is not occupancy or safe access; preparedness input is not reduced mortality by itself.",
        "Use IMD terminology and timestamped bulletins precisely; source wind, rainfall, surge, casualty, evacuation and loss units and separate event fact from causal inference.",
    ),
    8: (
        "Flood risk analysis separates riverine, flash, pluvial, coastal, dam-related and urban drainage flooding while tracing catchment, floodplain, wetland, drainage, encroachment and governance mechanisms.",
        "Heavy rainfall is not automatically an urban flood; forecast rainfall is not observed rainfall; drainage capacity is not drainage performance; reservoir release is not by itself proof of dam failure or sole causation.",
        "Date and unit rainfall, discharge, reservoir, inundation and loss claims; distinguish structural works from zoning, wetland restoration, forecasting, maintenance and evacuation.",
    ),
    9: (
        "Drought and heat are slow-onset or compounding risks shaped by meteorological conditions, soil moisture, hydrology, crops, water demand, urban heat, health vulnerability and anticipatory action.",
        "Meteorological drought is not agricultural or hydrological drought; heat-wave criteria are not one universal temperature; forecast is not observed maximum; Heat Action Plan adoption is not implementation or mortality reduction.",
        "Preserve IMD threshold, station/region, anomaly, duration, date and unit; separate climate trend, event attribution, exposure, vulnerability and policy outcome.",
    ),
    10: (
        "Landslide, avalanche and GLOF risk in the Himalaya joins slope, geology, snow or lake condition, rainfall or seismic trigger, road cutting, drainage, settlement exposure, monitoring, zoning and evacuation.",
        "Susceptibility is not probability; trigger is not root vulnerability; glacial lake growth is not a GLOF event; satellite detection is not ground validation; road construction correlation is not single-cause proof.",
        "Source event date, location, rainfall, volume, magnitude, casualty and infrastructure data; qualify attribution and distinguish monitoring capacity from actionable warning coverage.",
    ),
    11: (
        "Forest-fire risk joins fuel, moisture, weather, ignition, terrain, forest type, people and assets, prevention, detection, initial attack, inter-agency response, ecosystem recovery and livelihood restoration.",
        "Hotspot is not confirmed fire; burnt area is not tree-cover loss; natural ignition is not human cause; satellite alert is not field verification; suppression output is not ecological resilience.",
        "Name sensor/product, resolution, date, area unit and verification status; separate forest-department ownership, district support and specialised response while retaining ecological trade-offs.",
    ),
    12: (
        "Industrial, chemical, nuclear and biological emergencies require life-cycle prevention, on-site and off-site planning, detection, incident command, protective action, specialist response, decontamination, liability and long-term recovery.",
        "Hazardous material is not a released dose; on-site plan is not off-site plan; incident is not disaster impact; CBRN disaster management is not Internal Security's hostile-threat operations or Science and Technology's reactor/pathogen engineering.",
        "Identify enacted law, notified rule, regulator, occupier, district authority and response force; date quantities, casualties, exposure or contamination and never infer readiness from licence, plan or drill.",
    ),
    13: (
        "Public-health emergency management joins surveillance, risk assessment, laboratory confirmation, clinical and public-health response, logistics, risk communication, continuity, social protection and recovery.",
        "Case is not outbreak; outbreak is not automatically epidemic or pandemic; alert is not laboratory confirmation; emergency declaration is not proof of transmission control; hospital capacity is not deployable staffed capacity.",
        "Preserve disease/event definition, reporting date, numerator, denominator, geography, legal status and data revision; separate health ownership from disaster-support coordination.",
    ),
    14: (
        "Urban and critical-infrastructure resilience maps interdependent power, water, transport, telecom, health, sanitation and digital systems, single points of failure, redundancy, continuity and equitable restoration.",
        "Critical is not merely expensive; redundancy is not unused duplication; sanctioned backup capacity is not deployed service; restored output is not resilience outcome; smart-city technology is not risk reduction without governance.",
        "Name service level, dependency, outage/restoration unit, responsible utility and date; distinguish robustness, redundancy, rapid recovery and transformation.",
    ),
    15: (
        "Climate adaptation and DRR overlap where climate-sensitive hazards, exposure and vulnerability are reduced, while loss and damage addresses residual impacts beyond prevention or adaptation at relevant limits.",
        "Climate hazard is not disaster impact; adaptation is not synonymous with DRR; mitigation of greenhouse gases is not disaster mitigation; loss and damage is not every adaptation expense or automatically compensation.",
        "Attribute framework, fund or policy status and date; separate announcement, pledge, operationalisation, allocation, access and disbursement, and cross-own Environment Topic 26 rather than duplicating its climate-law base.",
    ),
    16: (
        "Disaster finance combines ex-ante risk reduction, preparedness, contingency financing, response and relief funds, reconstruction, insurance or risk transfer, fiscal resilience and Build Back Better.",
        "Budget provision is not allocation, allocation is not release, release is not expenditure, insured value is not payout, relief norm is not automatic legal entitlement, and reconstruction output is not resilience outcome.",
        "Name fund, legal basis, finance tier, period, rupee unit and status; separate immediate relief, compensation, recovery, reconstruction and resilience investment.",
    ),
    17: (
        "Humanitarian operations connect needs assessment, incident command, search and rescue, medical triage, logistics, shelter, WASH, protection, relief, rehabilitation, livelihood restoration, recovery and Build Back Better.",
        "NDRF is not NDMA; response force is not civil administration; armed-forces aid to civil authority does not transfer civilian disaster ownership; relief is not rehabilitation, rehabilitation is not recovery, and recovery output is not resilience outcome.",
        "Identify lead and support roles, request/coordination status, capacity versus deployment, beneficiary unit, time window and protection standard; never infer entitlement or outcome from norms or dispatch.",
    ),
    18: (
        "Disaster governance integrates risk-informed development, plans, audits, training, exercises, local capacity, inter-state and international cooperation, knowledge systems and accountable learning across the full cycle.",
        "Institutional existence is not capacity; capacity is not deployment; plan approval is not implementation; Sendai's four priorities are not seven targets; global indicators are not automatically India's national indicators.",
        "Cross-own Environment Topic 26 for the environmental/climate base and Internal Security for hostile-threat operations; date every plan, guideline, initiative, alert, event and cooperation claim and distinguish announcement from operationalisation.",
    ),
}
...
def _answer_controls(question: str, title: str) -> dict[str, str]:
    ...
    if "prelims" in title.casefold() or re.search(r"\boption\b|\bwhich of the following\b", question, re.I):
        return {
            "demand": f"Treat ‘{focus}’ as a hazard/risk term, legal-status, institutional-mandate, cycle-stage, warning-stage, date and evidence problem.",
            "plan": "Define the hazard and exposed system; separate vulnerability, capacity and impact; fix Act/rule/plan/guideline or institutional mandate; test warning, response, finance and outcome status; reject the nearest date, unit or jurisdiction trap.",
            "why": "It prevents hazard from impact, forecast from warning, NDMA from NDRF, capacity from deployment, relief norm from entitlement, and allocation from expenditure being conflated.",
            "improve": f"For ‘{focus}’, explain why the nearest distractor fails on risk component, mandate, legal character, cycle stage, warning stage, unit, jurisdiction, status or date.",
        }
    return {
        "demand": f"The directive **{directive}** requires a direct position on ‘{focus}’, each clause, risk mechanism, named Indian law/institution/event, dated status, implementation and qualification.",
        "plan": f"For {marks} marks, spend one-sixth of the time decoding the directive and drawing hazard -> exposure/vulnerability/capacity -> risk/impact -> prevention/mitigation/preparedness -> response/recovery/BBB; write four to seven claim -> named evidence/example -> analysis -> qualification points; reserve the final minute for source, date, unit, status, mandate and event/inference checks.",
        "why": "The answer obeys the directive, uses India-centric evidence, and preserves risk terms, legal character, institutional mandate, warning/cycle stage, finance status, implementation and resilience distinctions.",
        "improve": f"For ‘{focus}’, replace the weakest generic point with one exact Indian mechanism, institution/guideline/event, dated source and unit, implementation bottleneck, local-capacity or inclusion safeguard and answer-specific qualification.",
    }
...
def _detailed_model_answer(block: str, question: str) -> str:
    ...
    thesis = thesis_match.group(1).strip() if thesis_match else (solution_match.group(1).strip() if solution_match else f"The answer must resolve the Disaster Management demand in ‘{question}’.")
    ...
    if not evidence:
        evidence = [
            "Define hazard, exposure, vulnerability, capacity, risk, impact and resilience at the scale required by the question.",
            "Explain the causal or operating chain before listing interventions across prevention, mitigation, preparedness, response and recovery.",
            "Use a named Indian Act, institution, guideline, warning agency, plan, fund or verified disaster event with date and status.",
            "Separate structural and non-structural measures, lead and support institutions, capacity and deployment, and warning from protective action.",
            "Evaluate inclusion, local capacity, ecosystem-based DRR, finance, implementation, recovery and Build Back Better.",
            "Test source, date, unit, jurisdiction, enacted/notified/draft status, uncertainty, attribution, exception and residual risk.",
        ]
    body = "\n".join(
...
def _review_block(topic: Topic) -> str:
    points = DISASTER_MANAGEMENT_REVIEW_POINTS[topic.number]
    return (
...
def _wrapped_review_groups(topic: Topic) -> list[list[str]]:
    ...
    return [
        ...
        for label, point in zip(
            labels, DISASTER_MANAGEMENT_REVIEW_POINTS[topic.number]
        )
    ]
...
def build_ascii_spec(
    ...
    """Finalize the inherited inventory with explicit Disaster Management review controls."""
    ...
    for panel, control_lines in zip(panels[-3:], groups):
        ...
        for path in (
            ...
            if value not in references:
                references.append(value)
    spec["constraints"]["disaster_management_review_controls"] = True
    return spec


def mcq_blocks(area: str) -> list[tuple[int, int, str]]:
    """Parse both legacy MCQ headings and the real Disaster Management `Qn. stem` layout."""
    matches = list(
        re.finditer(
            r"(?im)^#{3,6}\s+("
            r"(?:(?:Hard|Learning|Practice|Remedial|Broad)\s+)?MCQ\s*\d+.*"
...
def augment_topic_semantic_content(
    ...
    *,
    ...
    result = _base_augment_topic_semantic_content(topic, markdown, workbook=workbook)
    if workbook:
        return result
    if "### DISASTER MANAGEMENT DEEP-REVIEW CORE CONTROL" not in result:
        result = result.replace("## BASIC MCQS / REMEDIATION", _review_block(topic) + "\n\n## BASIC MCQS / REMEDIATION", 1)
    ...
    points = DISASTER_MANAGEMENT_REVIEW_POINTS[topic.number]
    supplement = f"""### SESSION 15 — CORE CONSOLIDATION — INTEGRATED DISASTER MANAGEMENT ANSWER CHECK
...
PREVENTION -> MITIGATION -> PREPAREDNESS -> RESPONSE
                     ↓
RELIEF -> REHABILITATION -> RECOVERY -> BUILD BACK BETTER
```
...
- Reconstruct the risk and institutional mechanism before listing policy responses.
- End with one dated India-centric law, institution, guideline, warning chain, fund or event and one explicit implementation or evidence limit.

#### MINI RECAP

- Risk terms, cycle stage, law/status, institutional mandate, warning/action and dated evidence must agree across the session, workbook, graphical master and ASCII master.
"""
    return result.replace("## BASIC MCQS / REMEDIATION", supplement + "\n\n## BASIC MCQS / REMEDIATION", 1)
...
def validate_generated(
    ...
    inherited_errors = [
        ...
        if "Disaster Management control" not in error
        and "Disaster Management review control" not in error
    ]
    errors: list[str] = []
    required_contract = (
        "Risk boundary",
    ...
    for phrase in required_contract:
        if phrase.casefold() not in main.casefold():
            errors.append(
                f"Learning session lacks Disaster Management control: {phrase}"
            )
    if "### DISASTER MANAGEMENT DEEP-REVIEW CORE CONTROL" not in main:
        errors.append(
            "Topic-specific Disaster Management review control is absent."
        )
    for label in ("MUST REMEMBER:", "CLOSE DISTINCTION:", "EVIDENCE LIMIT:"):
        if label not in standalone_ascii:
            errors.append(
                f"ASCII master lacks Disaster Management control: {label}"
            )
    ...
    result["hard_gates"].update(
        {
            "disaster_management_legal_institutional_status_precision": not errors,
            "institutional_mandate_and_federal_role_precision": not errors,
            "event_casualty_magnitude_rainfall_temperature_fund_source_discipline": not errors,
            "announcement_operationalisation_allocation_release_expenditure_separation": not errors,
            "hazard_impact_warning_action_capacity_deployment_separation": not errors,
            "environment_and_internal_security_cross_ownership": not errors,
            "law_rule_notification_guideline_status_precision": not errors,
            "disaster_management_topic_specific_precision_control": not errors,
        }
    )
    result["metrics"]["disaster_management_review_control_count"] = 3
...
def update_ledgers(rows: list[dict[str, Any]], changed: set[str]) -> None:
    ...
    for row in rows:
        ...
        issues.extend(
            (
                f"| DM{number:02d}-001 | high | `{key}` | all four artifacts | "
                "Risk mechanism, institution, law/status, unit/date/status and "
                f"current-claim controls | Fresh review required | "
                f"E-DM{number:02d}-001 | MD-DM{number:02d}-001 | "
                f"closed in g{generation} |",
                f"| DM{number:02d}-002 | high | `{key}` | solved practice | "
                ...
                f"{metrics['question_count']} | E-DM{number:02d}-002 | "
                f"MD-DM{number:02d}-002 | closed in g{generation} |",
                f"| DM{number:02d}-003 | high | `{key}` | MCQs and flows | "
                "Strict A→B→C→D plus independently complete graphical/ASCII "
                f"reconstruction | Baseline MCQs={metrics['mcq_count']}, panels="
                f"{metrics['flow_panel_count']} | E-DM{number:02d}-003 | "
                f"MD-DM{number:02d}-003 | closed in g{generation} |",
            )
        )
        evidence.extend(
            (
                f"| E-DM{number:02d}-001 | `{key}` | Basic, canonical provenance, "
                ...
                f"| E-DM{number:02d}-002 | `{key}` | Generated content preserves "
                "risk, legal, institutional, warning, finance, unit/date/status and "
                f"announcement-to-outcome boundaries | generated validation | "
                f"`{row['validation']}` | {DATE} | verified; approval false |",
                f"| E-DM{number:02d}-003 | `{key}` | Session, workbook, "
        ...
        suggestions.extend(
            (
                f"| MD-DM{number:02d}-001 | high | `{key}` | generated "
                "session/flows | Disaster Management precision and status controls absent | "
                f"E-DM{number:02d}-001 | Add risk-cycle, mandate, warning, "
                "finance, unit/date/status and cross-ownership controls | Generated only | "
                f"applied g{generation}; canonical owners unchanged |",
                f"| MD-DM{number:02d}-002 | high | `{key}` | generated practice | "
                f"Per-answer execution controls incomplete | E-DM{number:02d}-002 | "
                "Repair each model and timed plan without changing verified PYQ "
                f"wording | Generated only | applied g{generation}; canonical "
                "owners unchanged |",
                f"| MD-DM{number:02d}-003 | high | `{key}` | generated MCQs/flows | "
                f"Rotation and independent flow completeness required | "
                f"E-DM{number:02d}-003 | Regenerate all four agreeing artifacts | "
                f"Generated only | applied and verified g{generation} |",
            )
        )
    append_once(REVIEW_ROOT / "ISSUE-LEDGER.md", "| DM01-001 |", issues, changed)
    append_once(
        REVIEW_ROOT / "EVIDENCE-LEDGER.md", "| E-DM01-001 |", evidence, changed
    )
    append_once(
        REVIEW_ROOT / "MD-CHANGE-SUGGESTIONS.md",
        "| MD-DM01-001 |",
...
def update_review_tracker(
    ...
    for item in tracker["topics"]:
        ...
        item["md_change_ids"] = [
            f"MD-DM{topic.number:02d}-{index:03d}" for index in range(1, 4)
        ]
        item["evidence_ids"] = [
            f"E-DM{topic.number:02d}-{index:03d}" for index in range(1, 4)
        ]
        item["reviewer_notes"] = (
            ...
            "hash-locked; generation-local hazard, institutional, unit/date/status, "
...
def export_library(**kwargs: Any) -> dict[str, Any]:
    ...
    for attempt in range(1, 6):
        ...
        snapshot = (
            EXPORTS / f"disaster-management-live-status-snapshot-{DATE}.json"
        )
...
def _run_tracker_sync() -> dict[str, Any]:
    ...
    if completed.returncode:
        raise RuntimeError(
            "Disaster Management tracker synchronization failed: "
...
def _republish_master_library() -> dict[str, Any]:
    """Republish all live topics and preserve completed Disaster Management reviews."""
    review_fields = (
        "status",
        "artifacts",
        "scores",
    ...
    if after != before:
        raise RuntimeError(
            "Complete live synchronization altered Disaster Management review results."
        )
...
def _publish_before_tracker_sync_when_needed() -> dict[str, Any] | None:
    ...
    if not expected_set.issubset(live_ids):
        raise RuntimeError("Live EXPORT-PDF-STATUS lacks completed Disaster Management manifest topics.")
    ...
    if not missing:
        ...
        if review_set != expected_set:
            raise RuntimeError("Disaster Management REVIEW identities are incomplete despite complete MASTER scope.")
        return None
...
def run_subject_review() -> int:
    """Review topics 01-18 in manifest order and publish only a selected manifest."""
    changed: set[str] = {
        rel(Path(__file__)),
        "tools\\test_regenerate_disaster_management_deep_review.py",
    }
    all_topics = topics()
    rows: list[dict[str, Any]] = []
    ...
    for topic in all_topics:
        ...
        if topic.number in batch_ends:
            ...
            write_batch(
                ...
                / "batch-reports"
                / (
                    f"Disaster-Management-Topics-{start:02d}-{end:02d}-"
                    f"{DATE}.md"
                ),
                rows[start - 1 : end],
                changed,
    ...
    tests = [
        run_unittest("test_regenerate_disaster_management_deep_review"),
        *[
            run_unittest(module)
            for module in DISASTER_MANAGEMENT_TEST_MODULES
        ],
    ...
    validation_report = (
        EXPORTS / f"disaster-management-deep-review-validation-{DATE}.json"
    )
    dump(
        validation_report,
        {
    ...
    reconciliation = (
        EXPORTS / f"disaster-management-deep-review-reconciliation-{DATE}.json"
    )
    dump(
        reconciliation,
        {
    ...
    subject_report = (
        REVIEW_ROOT
        / "subject-reports"
        / f"Disaster-Management-Subject-Completion-{DATE}.md"
    )
    write_text(
        subject_report,
        "# Disaster Management Subject Completion — 4 September 2026\n\n"
        "All 18 Disaster Management topics passed immutable deep review. A complete "
    ...
    inventory = (
        EXPORTS / f"disaster-management-deep-review-{DATE}-changed-files.txt"
    )
...
def main() -> int:
    global _DISASTER_MANAGEMENT_RUN_STARTED_NS
    _DISASTER_MANAGEMENT_RUN_STARTED_NS = time.time_ns()
    ...
    write_text(
        report,
        "# Disaster Management Subject Completion — 4 September 2026\n\n"
        "All 18 Disaster Management topics were reviewed in manifest order and repaired "
        "through immutable successors. Basic/Core remains complete and easy-first "
        "before optional Advanced depth. Risk terminology, disaster-cycle, Sendai, legal-status, "
        "institutional-mandate, early-warning, community-capacity, hazard-specific, climate/DRR, "
        "finance, relief/recovery, evidence, answer-execution, MCQ-rotation and dual-flow controls "
        "passed across all authoritative topics. Environment Topic 26 and Internal Security "
        "overlaps remain explicitly cross-owned. Canonical Basic, substantive canonical "
        ...
        f"validation, MASTER and REVIEW identities agree. Represented: {count}; "
        f"passed: {count}; target score: 98/100; failures: 0; mismatches: 0; "
        "approval: false.",
    )
    ...
    nul_inventory = (
        EXPORTS / f"disaster-management-deep-review-{DATE}-changed-files.nul"
    )
    candidates = {
        line
    ...
    candidates.update(
        {
            ...
            rel(
                EXPORTS
                / f"disaster-management-live-status-snapshot-{DATE}.json"
            ),
    ...
    if len(ordered) != len(set(ordered)):
        raise RuntimeError("Disaster Management changed-file inventory contains duplicates.")
    missing = [
        path
        for path in ordered
    ...
    if missing:
        raise RuntimeError(
            "Disaster Management changed-file inventory contains missing paths: "
            + ", ".join(missing[:20])
        )
    ...
    if (
        ...
        raise RuntimeError("Disaster Management UTF-8 NUL inventory failed round-trip.")
