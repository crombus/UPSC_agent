"""Focused tests for the Governance immutable deep-review driver."""
...
class GovernanceDeepReviewTests(unittest.TestCase):
    ...
    def test_review_controls_cover_all_sixteen_topics(self) -> None:
        samples = {
            1: "Government is the formal authority structure",
            2: "implementation is a multi-level chain",
            3: "Development industry is an analytical label",
            4: "section 8 company",
            5: "digital-by-default",
            6: "DPI is not every government portal",
            7: "single-window is not single accountability",
            8: "CAG audit is not conviction",
            9: "Mission Karmayogi is an executive",
            10: "Commission recommendation is not law",
            11: "Independence is protection from improper direction",
            12: "Eleventh and Twelfth Schedule",
            13: "utilisation certificate is not outcome",
            14: "Attendance is not participation",
            15: "correlation is not causation",
            16: "BCCI a cricket governing body",
        }
    ...
    def test_contract_enforces_governance_boundaries(self) -> None:
    ...
    def test_every_governance_generator_test_is_wired(self) -> None:
        self.assertEqual(
            tuple(
                f"test_generate_governance_{number:02d}_sequential"
                for number in range(1, 17)
            ),
            deep.GOVERNANCE_TEST_MODULES,
        )
...
**Question:** Analyse the implementation and accountability chain. Answer in 250 words.
**Model thesis:** Authority must reach citizens through accountable delivery.
**Claim → named evidence → analysis → qualification:**
- A named Indian institution demonstrates the bounded implementation mechanism.
**Qualified conclusion:** Delivery requires authority, capacity, remedy and feedback.
...
"""
        ...
        self.assertIn("Centre-state-local", repaired)
        self.assertIn("duty-holder", repaired)
    ...
    def test_live_tracker_rows_are_fresh_pending_before_review(self) -> None:
        ...
        rows = [
            ...
            if row["topic_key"].startswith("governance-")
        ]
