"""Generate Empiricism as a source-complete learner-v2 Philosophy topic."""
...
import generate_philosophy_western_rationalism_v2 as base
import philosophy_western_empiricism_v2_spec as empiricism_spec
...
TOPIC_TITLE = "Empiricism"
TOPIC_FOLDER = "topic-03"
...
ASCII_SPEC = (
    ...
    / "retrofits"
    / "ascii-panel-specs"
    / "philosophy--paper-i-western-philosophy-03-ascii-2026-08-26.json"
)
CONTENT_SPEC_DIR = (
    ROOT
    / "upsc-ai-kit"
...
CANONICAL_OWNER = (
    "upsc-ai-kit\\knowledge\\Philosophy\\paper-1\\western\\Empiricism.md"
)
ADVANCED_DOSSIER = (
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Empiricism\\"
    "Empiricism_Layered-Complete-Learning-Session_2026-08-18.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Western-Philosophy\\"
    "learning-sessions\\Empiricism\\"
    "Empiricism_Layered-Solved-Practice-Workbook_2026-08-18.md"
)
BASELINE_REPORT = (
    EXPORT_MANIFEST_DIR
    / "philosophy-paper-i-western-philosophy-03-learner-v2-g2-"
...
SESSION_SPECS = empiricism_spec.SESSION_SPECS
ASCII_PANELS = empiricism_spec.ASCII_PANELS
REQUIRED_CORE_TERMS = empiricism_spec.REQUIRED_CORE_TERMS
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            knowledge_root / "assets" / "Empiricism-Pressure-Sequence.png"
        ),
        "main_pdf": (
            notes_root
    ...
    topic.update(
        {
            ...
            "syllabus_mapping": (
                "Philosophy Optional, Paper I, Western Philosophy topic 3: "
                + OFFICIAL_CLAUSE
            ),
...
def make_concept_visual(output: Path) -> None:
    ...
    bold = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 44)
    ...
    small = ImageFont.truetype(r"C:\Windows\Fonts\segoeui.ttf", 27)
    tiny = ImageFont.truetype(r"C:\Windows\Fonts\segoeui.ttf", 21)

    draw.text(
        (width / 2, 62),
        "EMPIRICISM: ONE EXPERIENTIAL RULE, THREE PRESSURE TESTS",
        font=title_font,
        fill="#F8FAFC",
        anchor="ma",
    )
    draw.text(
        (width / 2, 145),
        "Locke's representative realism → Berkeley's immaterialism → Hume's sceptical naturalism",
        font=regular,
        fill="#8DE7F7",
        anchor="ma",
    )
    cards = [
        (
            ...
            [
                ...
                "Unknown material substratum",
                "Consciousness and memory",
            ],
            "PRESSURE: How can ideas resemble matter?",
        ),
        (
            670,
            "#263D6A",
            ...
            [
                "No abstract material substance",
                "Esse est percipi",
                "All sensible qualities are ideas",
                "God secures ordered continuity",
            ],
            "PRESSURE: What supports self and causal power?",
        ),
        (
            1250,
            "#4A315F",
            ...
            "Impressions + ideas",
            [
                "Copy Principle and Fork",
                "Bundle self",
                "Constant conjunction + custom",
                "Mitigated scepticism",
            ],
            "RESULT: Natural belief without rational necessity",
        ),
    ]
    for x, colour, heading, subtitle, lines, footer in cards:
        draw.rounded_rectangle((x, 255, x + 560, 925), 28, fill=colour, outline="#61DDF2", width=4)
        draw.text((x + 280, 310), heading, font=title_font, fill="#FFFFFF", anchor="ma")
        draw.text((x + 280, 390), subtitle, font=bold, fill="#F8D27A", anchor="ma")
        y = 500
        for line in lines:
            draw.ellipse((x + 48, y + 8, x + 62, y + 22), fill="#61DDF2")
            draw.text((x + 82, y), line, font=regular, fill="#F3F7FA")
            y += 82
        draw.rounded_rectangle((x + 34, 830, x + 526, 895), 16, fill="#0B202E")
        draw.text((x + 280, 862), footer, font=tiny, fill="#FFF1B8", anchor="mm")
    for start, end in ((650, 670), (1230, 1250)):
        draw.line((start, 590, end, 590), fill="#61DDF2", width=10)
        draw.polygon(
            [(end - 16, 574), (end, 590), (end - 16, 606)],
            fill="#61DDF2",
        )
    draw.rounded_rectangle((190, 1000, 1710, 1110), 20, fill="#0E2B3D", outline="#F8D27A", width=3)
    draw.text(
        (950, 1038),
        "EXAM ROUTE: thesis → exact doctrine → argument → objection/reply",
        font=small,
        fill="#FFF5D4",
        anchor="mm",
    )
    draw.text(
        (950, 1075),
        "→ matched comparison → qualified verdict",
        font=small,
        fill="#FFF5D4",
        anchor="mm",
    )
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+3\.\s+Empiricism\s*(.*?)"
        r"(?=^##\s+4\.\s+Kant)",
        text,
    )
    if not match:
        raise ValueError("The Empiricism advanced dossier section was not found.")
...
def insert_concept_visual(
    ...
    block = (
        f"\n\n![Empiricist pressure-sequence map]({image_path})\n\n"
        "*Concept map: Locke derives ideas from sensation and reflection while "
        "retaining represented matter; Berkeley removes material substance; "
        "Hume subjects substance, self and causal necessity to the same "
        "experiential test.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "Empiricism conceptual atlas"
        ),
        "generated_on": GENERATION_DATE,
        "scope": "Philosophy Optional Paper I Western Philosophy topic 03 only",
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "Empiricism generation remain immutable."
                ),
                "panels": [
                    {
...
"""
    ...
    text = text.replace(
        "MCQs: 24 core plus 8 remedial, exactly 32",
        "MCQs: 24 core plus 16 remedial, exactly 40",
    )
    text = text.replace(
    ...
    text = text.replace(
        "Questions 1-24 are core coverage across all ten subtopics; Questions "
        "25-32 are remedial items targeting the most common misreadings. Correct "
        "answers rotate strictly A -> B -> C -> D eight times.",
        "Questions 1-24 are core coverage across all ten subtopics; Questions "
        "25-40 are remedial items targeting the most common misreadings. Correct "
        "answers rotate strictly A -> B -> C -> D ten times.",
    )
    return text
...
def render_ascii_pdf_safe(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Empiricism ASCII Master Flowchart"
        metadata["creator"] = "generate_philosophy_western_empiricism_v2.py"
    ...
    if not validation["passed"]:
        raise ValueError("Empiricism ASCII PDF metadata repair broke validation.")
...
def build_record(
    ...
    return {
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_western_empiricism_v2.py + "
...
def validate_content(
    ...
    if (
        ...
        errors.append("Core Answer-Grabbing Lines are missing, duplicated or altered.")
...
def run() -> int:
    ...
    if generation != 2:
        raise ValueError(
            f"Expected the next Empiricism learner-v2 generation to be g2, found g{generation}."
        )
    ...
    flow_metadata["ascii_master_source"] = "manual-authored-empiricism-spec"
    ...
    changed = {
        ...
        base.relative(ROOT / "tools" / "philosophy_western_empiricism_v2_spec.py"),
...
def main() -> int:
    ...
    try:
        ...
        base.carvaka_flowchart.CarvakaError,
    ) as exc:
