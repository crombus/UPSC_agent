"""Finalize validation and publication records for the Polity 38-42 batch."""
...
import finalize_polity_33_37_batch as precedent
import generate_polity_38_42_sequential as batch
...
POLITY_FLOW_VALIDATION = (
    EXPORTS / "polity-01-42-flow-learning-2026-08-25-validation.json"
)
ALL_FLOW_VALIDATION = (
...
REPORT = (
    ...
    / "POLITY-38-42-SEQUENTIAL-BATCH-REPORT.md"
)
...
def repair_mcq_rotation() -> None:
    """Rebuild the five outputs with the command's strict A-B-C-D contract."""
    ...
    for key in [f"polity-{number:02d}" for number in range(38, 43)]:
...
def write_case_evidence() -> dict[str, Any]:
    ...
    topic_keys = [f"polity-{number:02d}" for number in range(1, 43)]
...
def run_flow_publications(tests_passed: int) -> None:
    ...
    subprocess.run(
        [
            ...
            "polity-",
            "--report-path",
            "notes\\Flow-Learning\\POLITY-01-42-FLOW-LEARNING-REPORT.md",
            "--validation-path",
...
def build_validation(tests_passed: int) -> dict[str, Any]:
    ...
    checks = {
        ...
        "prior_70_clean_hashes_preserved": not state[
            "existing_clean_hash_mismatches"
        ],
        "prior_70_flow_hashes_preserved": not state[
            "existing_flow_hash_mismatches"
        ],
        "prior_generated_topics_preserved": not state[
            "prior_generated_topic_hash_mismatches"
        ],
        "four_item_inventory_75": four_tracker["topic_count"] == 75
        and four["status"] == "passed",
        "polity_flow_inventory_42": (
            polity_flow["summary"]["topic_folder_count"] == 42
            and polity_flow["status"] == "passed"
        ),
        "full_flow_inventory_75": (
            all_flow["summary"]["topic_folder_count"] == 75
...
def write_report(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Polity 38-42 Sequential Batch Report",
        "",
        "- Status: **PASSED**",
        "- Execution: strict order `polity-38 -> polity-39 -> polity-40 -> polity-41 -> polity-42`.",
        ...
        "- Final clean four-item library: **75 topics**. Final Flow Learning inventory: **75 topics**, including **Polity 42**.",
        "",
    ...
    lines.extend(
        [
            ...
            "- Existing 70 clean packages: prior canonical artifact files checked, 0 hash mismatches.",
            "- Existing 70 Flow Learning topics: prior topic artifacts checked, 0 hash mismatches.",
            f"- Full four-item library: 75 topics; {payload['inventories']['four_item_links_checked']} links checked; 0 broken.",
            f"- Polity Flow Learning: 42 topics; {payload['inventories']['polity_flow_links_checked']} links checked; 0 broken.",
            f"- Full Flow Learning: 75 topics; {payload['inventories']['full_flow_topic_artifact_files_checked']} topic artifacts hash-checked; {payload['inventories']['full_flow_links_checked']} navigation links checked; 0 broken; source/destination PDF and TXT hashes equal.",
...
def write_changed_files() -> list[str]:
    ...
    explicit = {
        ...
        ROOT / "tools" / "generate_polity_38_42_sequential.py",
        ROOT / "tools" / "finalize_polity_38_42_batch.py",
        ROOT / "tools" / "export_four_item_library.py",
        ROOT / "tools" / "export_flow_learning_library.py",
        ...
        / "notes"
        / "Flow-Learning"
        / "POLITY-01-42-FLOW-LEARNING-REPORT.md",
        FINAL_VALIDATION,
        REPORT,
        CASE_EVIDENCE,
        POLITY_FLOW_VALIDATION,
