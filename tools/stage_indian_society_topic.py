"""Stage one Indian Society learner-v2 topic without touching the tracker.
...
def stage(topic_key: str, *, tests_passed: int, tests_scope: str) -> dict[str, object]:
    ...
    tracker_hash = refresh.sha256(refresh.TRACKER)
    tracker = refresh.load_tracker()
    row, record = refresh.process_new_topic_spec(spec_path, tracker)
    if refresh.sha256(refresh.TRACKER) != tracker_hash:
        raise refresh.RefreshError("Tracker changed during stage-only generation.")
...
def main() -> int:
    ...
    parser.add_argument(
        "--tests-scope",
        default="Indian Society sequential generator suite and shared refresh suites",
    )
