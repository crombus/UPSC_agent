"""Validate and inventory the completed Science and Technology semantic review."""
...
REPORT = (
    ...
    / f"Science-and-Technology-Subject-Semantic-Completion-{DATE}.md"
)
VALIDATION = (
    EXPORTS
    / f"science-and-technology-01-26-semantic-completeness-{DATE}-validation.json"
)
INVENTORY = (
    EXPORTS
    / f"science-and-technology-01-26-semantic-completeness-{DATE}-changed-files.txt"
)
LIVE_AUDIT = (
    EXPORTS / f"science-and-technology-authoritative-live-source-audit-{DATE}.json"
)
...
def run_tests() -> tuple[int, list[str]]:
    modules = [
        ...
        "test_export_flow_learning_library",
    ...
    for module in modules:
        ...
        match = __import__("re").search(r"Ran (\d+) tests?", output)
...
def main() -> int:
    ...
    if [row["topic_key"] for row in rows] != expected:
        errors.append("Science and Technology catalogue/order mismatch.")
    if any(row["status"] != "passed" for row in rows):
        errors.append("Not every Science and Technology topic passed.")
    if semantic["next_topic"]["topic_key"] != "internal-security-01":
        errors.append("Global queue did not advance to internal-security-01.")
    ...
    tracker_marker = (
        "| 15 | Science and Technology | 26 | 26 | passed | Complete |"
    )
    if tracker_marker not in tracker_text:
        errors.append("Human tracker does not record Science and Technology 26/26.")
    ...
    inventory: set[str] = {
        ...
        "tools\\finalize_science_and_technology_semantic_review.py",
        rel(SEMANTIC),
        rel(REPORT),
        rel(VALIDATION),
        rel(INVENTORY),
    ...
    failed_run_output = (
        EXPORTS
        / f"science-and-technology-authoritative-live-source-audit-{DATE}-run-output.json"
    )
    if failed_run_output.exists():
        inventory.add(rel(failed_run_output))
    ...
    if not approval_false:
        errors.append("At least one Science and Technology learner identity is approved.")
    if not h2_order_ok:
    ...
    REPORT.write_text(
        f"""# Science and Technology Subject Semantic Completion

**Date:** 6 September 2026  
**Result:** {'PASSED' if not errors else 'FAILED'}  
**Coverage:** {payload['passed']}/26  
