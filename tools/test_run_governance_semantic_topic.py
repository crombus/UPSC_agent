"""Tests for the strictly sequential Governance semantic driver."""
...
class GovernanceSemanticTopicRunnerTests(unittest.TestCase):
    ...
    def test_driver_uses_complete_governance_catalogue(self) -> None:
        self.assertEqual(set(range(1, 17)), set(subject.runner.TOPIC_CHOICES))
        self.assertEqual(
            [f"governance-{number:02d}" for number in range(1, 17)],
            [topic.topic_key for topic in deep.topics()],
        )
        self.assertEqual("2026-09-05", subject.REPORT_DATE)
        self.assertEqual("2026-09-05", deep.DATE)

    def test_live_sources_and_canonical_controls_cover_every_topic(self) -> None:
        self.assertEqual(set(range(1, 17)), set(deep.LIVE_OFFICIAL_SOURCES))
        self.assertEqual(set(range(1, 17)), set(deep.CANONICAL_OWNER_CONTROLS))
        for number, control in deep.CANONICAL_OWNER_CONTROLS.items():
            ...
            self.assertIn("5 September 2026", control)
    ...
    def test_reports_and_inventories_use_requested_date(self) -> None:
        ...
        self.assertIn("Governance Semantic-Completeness Review", wrapper)
        self.assertIn("2026-09-05", wrapper)
