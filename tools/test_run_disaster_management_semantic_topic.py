"""Tests for the strictly sequential Disaster Management semantic driver."""
...
import run_disaster_management_semantic_topic as subject


class DisasterManagementSemanticTopicRunnerTests(unittest.TestCase):
    ...
    def test_driver_uses_complete_authoritative_catalogue(self) -> None:
        self.assertEqual(set(range(1, 19)), set(subject.runner.TOPIC_CHOICES))
        self.assertEqual(
            [f"disaster-management-{number:02d}" for number in range(1, 19)],
    ...
    def test_live_audit_covers_every_topic(self) -> None:
        ...
        self.assertEqual(18, len(payload["topics"]))
    ...
    def test_topic_configs_and_pyq_notes_are_complete(self) -> None:
        self.assertEqual(set(range(1, 19)), set(subject.CONFIGS))
        self.assertEqual(set(range(1, 19)), set(subject.runner.PYQ_STATUS))
