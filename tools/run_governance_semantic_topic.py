"""Run one strictly sequential Governance semantic-completeness review."""
...
runner.TOPIC_CHOICES = range(1, 17)
runner.DEEP_REVIEW_TEST_MODULE = "test_regenerate_governance_deep_review"
runner.REPORT_DIR = (
    deep.ROOT / "upsc-ai-kit" / "manifests" / "reviews" / "governance"
)
runner.DRIVER_FILES = {
    "tools\\governance_11_15_data.py",
    "tools\\regenerate_governance_deep_review.py",
    "tools\\test_regenerate_governance_deep_review.py",
...
runner.PYQ_STATUS = {
    number: re.split(
        r"(?<=[.!?])\s+",
        deep.GOVERNANCE_PYQ_STATUS[number],
        maxsplit=1,
    )[0]
    for number in range(1, 17)
}
deep.POLITY_LIVE_OFFICIAL_SOURCES = deep.GOVERNANCE_LIVE_OFFICIAL_SOURCES
    ...
    row["findings"] = [
        {
            "severity": "closed",
            "finding": (
                "Four-ledger hostile audit closed; governance concepts, public-"
                "administration taxonomy, legal/institutional status, schemes and "
...
def report_text(
    ...
    return f"""# Governance Semantic-Completeness Review {topic.number:02d} — {topic.title}
...
prerequisites, standard governance/public-administration taxonomy, canonical
...
def run_tests() -> list[dict[str, object]]:
    modules = [
        "test_regenerate_governance_deep_review",
        "test_run_governance_semantic_topic",
        *[
...
def run(topic_number: int) -> dict[str, object]:
    if topic_number not in runner.TOPIC_CHOICES:
        raise ValueError("Governance runner accepts only topic numbers 1 through 16.")
