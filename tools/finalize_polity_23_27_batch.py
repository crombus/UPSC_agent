"""Finalize validation and publication records for the Polity 23-27 batch."""
...
import generate_polity_23_27_sequential as batch
...
CASE_EVIDENCE = EXPORTS / "polity-flowchart-case-year-repair-2026-08-24-validation.json"
POLITY_FLOW_VALIDATION = EXPORTS / "polity-01-27-flow-learning-2026-08-24-validation.json"
ALL_FLOW_VALIDATION = EXPORTS / "all-completed-topics-flow-learning-2026-08-24-validation.json"
FINAL_VALIDATION = EXPORTS / f"{batch.BATCH_ID}-validation.json"
REPORT = ROOT / "notes" / "Final-Learning-Packages" / "POLITY-23-27-SEQUENTIAL-BATCH-REPORT.md"
...
def _clean_topic_folder(number: int) -> Path:
    candidates = list(
        (
            ROOT
            / "notes"
    ...
    if len(candidates) != 1:
        raise RuntimeError(f"Expected one clean Polity folder for {number}: {candidates}")
    return candidates[0]


def write_case_evidence() -> dict[str, Any]:
    records = latest_records()
    topic_keys = [f"polity-{number:02d}" for number in range(1, 28)]
    ...
    for key in topic_keys:
        ...
        clean = _clean_topic_folder(int(key[-2:]))
        pairs = [
            (
    ...
    payload = {
        ...
        "cases": [
            batch.case_years.source_record(case_id) for case_id in distinct_cases
        ],
        "topics": rows,
    }
...
def run_flow_publications(tests_passed: int) -> None:
    ...
    subprocess.run(
        [
            ...
            "27",
            "--report-path",
            "notes\\Flow-Learning\\POLITY-01-27-FLOW-LEARNING-REPORT.md",
            "--validation-path",
...
def _legacy_retained(tracker: dict[str, Any], topic_key: str) -> bool:
    return any(
        isinstance(record, dict)
...
def build_validation(tests_passed: int) -> dict[str, Any]:
    ...
    for item in state["topics"]:
        ...
        continuous = record["continuous_core_first"]
        times = [item["gate_times"][f"{letter}_completed"] for letter in "ABCDEFGHIJ"]
        topics.append(
            {
                **item,
                "gate_timestamps_monotonic": nondecreasing(times),
                "legacy_record_retained": _legacy_retained(tracker, key),
    ...
    checks = {
        ...
        "tracker_records_unapproved": all(item["approved"] is False for item in topics),
        "legacy_history_retained": all(item["legacy_record_retained"] for item in topics),
        ...
        "graphical_same_master": all(item["graphical_same_master"] for item in topics),
        ...
        "prior_55_clean_hashes_preserved": not state[
            "existing_clean_hash_mismatches"
        ],
        "prior_55_flow_hashes_preserved": not state[
            "existing_flow_hash_mismatches"
        ],
        "prior_generated_topics_preserved": not state[
            "prior_generated_topic_hash_mismatches"
        ],
        "four_item_inventory_60": four["topic_count"] == 60
        and four["status"] == "passed",
        "polity_flow_inventory_27": polity_flow["summary"]["topic_folder_count"] == 27
        and polity_flow["status"] == "passed",
        "full_flow_inventory_60": all_flow["summary"]["topic_folder_count"] == 60
        ...
        "full_flow_links_passed": all_flow["navigation_links"]["passed"]
    ...
    payload = {
        ...
        "inventories": {
            ...
            "full_flow_topic_artifact_files_checked": all_flow["summary"][
                "topic_folder_count"
            ]
            * 3,
...
def write_report(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Polity 23-27 Sequential Batch Report",
        "",
        "- Status: **PASSED**",
        "- Execution: strict order `polity-23 -> polity-24 -> polity-25 -> polity-26 -> polity-27`.",
        ...
        "- Final clean four-item library: **60 topics**. Final Flow Learning inventory: **60 topics**, including **Polity 27**.",
        "",
    ...
    for item in topics:
        lines.extend(
            [
                ...
                "- J regression: case years, links, layout, hashes, rotation and prior-topic preservation passed.",
    ...
    lines.extend(
        [
            ...
            "- Existing 55 clean packages: 440 canonical artifact files checked, 0 hash mismatches.",
            "- Existing 55 Flow Learning topics: 165 topic artifacts checked, 0 hash mismatches.",
            f"- Full four-item library: 60 topics; {payload['inventories']['four_item_links_checked']} links checked; 0 broken.",
            f"- Polity Flow Learning: 27 topics; {payload['inventories']['polity_flow_links_checked']} links checked; 0 broken.",
            f"- Full Flow Learning: 60 topics; {payload['inventories']['full_flow_topic_artifact_files_checked']} topic artifacts hash-checked; {payload['inventories']['full_flow_links_checked']} navigation links checked; 0 broken; source/destination PDF and TXT hashes equal.",
...
def write_changed_files() -> list[str]:
    start = datetime.fromisoformat("2026-08-24T19:19:44+05:30").timestamp()
    candidates: set[Path] = set()
    explicit = [
        ROOT / "EXPORT-PDF-COMMAND-INDEX.md",
        ROOT / "EXPORT-PDF-STATUS.json",
        ROOT / "V2-SUBJECT-SECTION-COMMAND-INDEX.md",
        ROOT / "tools" / "generate_polity_23_27_sequential.py",
        ROOT / "tools" / "finalize_polity_23_27_batch.py",
    ...
    roots = [
        ...
        ROOT / "notes" / "Polity" / "learning-session-v2",
        ROOT / "upsc-ai-kit" / "knowledge" / "Learner-v2-Refreshed" / "Polity",
