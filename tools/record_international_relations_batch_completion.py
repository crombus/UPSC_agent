"""Prove and record the batch completion manifest for one International Relations run.

Reads the per-topic completion manifests, tracker records, ASCII panel specs and
graphical specs for a sequential International Relations learner-v2 batch,
re-proves every hard gate from the live artifacts, and writes a batch completion
manifest plus a batch changed-files manifest. It is read-only with respect to
...
import record_international_relations_completion as topic_record  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--baseline-topics", type=int, default=281)
    parser.add_argument("--baseline-panels", type=int, default=3244)
    parser.add_argument(
        "--also-changed",
        action="append",
        default=[],
    ...
    if authored_topics != args.baseline_topics + len(args.topic_keys):
        coverage_errors.append(
            "shared ASCII topic coverage did not increase by exactly "
            f"{len(args.topic_keys)} from the live baseline of {args.baseline_topics}."
        )
    if authored_panels != args.baseline_panels + 12 * len(args.topic_keys):
        coverage_errors.append(
            "shared ASCII panel coverage did not increase by exactly "
            f"{12 * len(args.topic_keys)} from the live baseline of "
            f"{args.baseline_panels}."
        )
    payload = {
        "schema_version": 1,
        ...
        "result": "failed" if failed or coverage_errors else "passed",
        "sequence": {
    ...
    return 1 if payload["result"] == "failed" else 0
