"""Generate Political Ideologies as a source-complete learner-v2 topic.
...
package for ``Political Ideologies`` only, and never rewrites topics 01-04 or
the planned topics 06-10 beyond the shared indexes and manifests that must
reflect this topic's completion.
"""
...
import philosophy_socio_political_political_ideologies_v2_spec as topic_spec
...
TOPIC_NUMBER = 5
OFFICIAL_CLAUSE = "Political Ideologies: Anarchism; Marxism and Socialism."
...
ASCII_SPEC = (
    ...
    / "retrofits"
    / "ascii-panel-specs"
    / f"philosophy--{SECTION_KEY}-05-ascii-{GENERATION_DATE}.json"
)
CONTENT_SPEC_DIR = (
    ROOT
    / "upsc-ai-kit"
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Political-Ideologies\\"
    "Political-Ideologies_Layered-Complete-Learning-Session_2026-08-19.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Socio-Political-Philosophy\\"
    "learning-sessions\\Political-Ideologies\\"
    "Political-Ideologies_Layered-Solved-Practice-Workbook_2026-08-19.md"
)
...
CORE_COMPLETENESS_MARKERS = (
    ...
    "four dimensions",
    "formally free contract",
    "structural exploitation",
    "socially rooted framework",
...
ENGLISH_FIRST_REPLACEMENTS = (
    (
        "**Syllabus (verbatim):** Political Ideologies : Anarchism; Marxism and "
        "Socialism.",
        f"**Syllabus (verbatim):** {OFFICIAL_CLAUSE}",
    ),
    (
        "Gandhi's ideal of *swarāj* begins with self-rule:",
        "Gandhi's ideal of self-rule (*swarāj*) begins with rule over oneself:",
    ),
    (
        "village *swarāj* and *sarvodaya*",
        "village self-rule (*swarāj*) and the welfare of all (*sarvodaya*)",
    ),
    (
        "| *dominio* — coercion |",
        "| coercion (*dominio*) |",
    ...
    (
        "| *satyāgraha*, trusteeship, constructive work |",
        "| truth-force (*satyāgraha*), trusteeship, constructive work |",
    ),
    (
        "trusteeship · *sarvodaya*",
        "trusteeship · welfare of all (*sarvodaya*)",
    ),
    (
        "writings on trusteeship, *swarāj* and *sarvodaya*.",
        "writings on trusteeship, self-rule (*swarāj*) and the welfare of all "
        "(*sarvodaya*).",
    ),
    (
        "Gandhi's swarāj begins with self-rule;",
        "Gandhi's self-rule (*swarāj*) begins with rule over oneself;",
    ),
    (
        "Gandhi's swarāj begins with",
        "Gandhi's self-rule (swarāj) begins with",
    ),
    (
        "AFFINITY (swarāj, village republics",
        "AFFINITY (self-rule/swarāj, village republics",
    ),
    (
        "decentralised swarāj,",
        "decentralised self-rule/swarāj,",
    ),
    (
        "village swarāj",
        "village self-rule (swarāj)",
    ),
    (
        "  sarvodaya; objection",
        "  welfare of all (sarvodaya); objection",
    ),
    (
        "satyagraha/trusteeship;",
        "truth-force (satyagraha)/trusteeship;",
    ),
    (
        "affinity via swarāj/minimal",
        "affinity via self-rule/swarāj/minimal",
    ),
    (
        "trusteeship/swarāj/sarvodaya cluster",
        "trusteeship/self-rule (swarāj)/sarvodaya cluster",
    ),
    (
        "trusteeship/decentralisation/swarāj cluster",
        "trusteeship/decentralisation/self-rule (swarāj) cluster",
    ),
    (
        "Gandhi's swaraj and minimal-state decentralism",
        "Gandhi's self-rule (swaraj) and minimal-state decentralism",
    ),
    (
        "decentralisation and village swaraj, rejecting",
        "decentralisation and village self-rule (swaraj), rejecting",
    ),
    (
        "C. Gandhi rejected swaraj and decentralisation",
        "C. Gandhi rejected self-rule (swaraj) and decentralisation",
    ),
    (
        "affinity (decentralised swaraj, minimal state)",
        "affinity (decentralised self-rule/swaraj, minimal state)",
    ),
    (
        "(swaraj + non-violence +",
        "(self-rule/swaraj + non-violence +",
    ),
    (
        "decentralised production and sarvodaya, seeking equality",
        "decentralised production and the welfare of all (sarvodaya), seeking "
        "equality",
    ),
    (
        "decentralisation, sarvodaya -- equality",
        "decentralisation, welfare of all (sarvodaya) -- equality",
    ),
)
...
def generation_paths(generation: int) -> dict[str, Path]:
    ...
    return {
        ...
        "concept_visual": (
            ...
            / "assets"
            / TOPIC_KEY
            / "anarchism-marxism-socialism-map.png"
        ),
        "main_pdf": (
            NOTES_OUTPUT
            / "notes"
...
def build_manifest(tracker: dict[str, Any], generation: int) -> dict[str, Any]:
    ...
    manifest["section"]["notes"] = (
        ...
        "topics 06-10 remain planned until separately generated and validated."
    )
    return manifest
...
def make_concept_visual(output: Path) -> None:
    ...
    title_font = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 50)
    ...
    draw.text(
        (width / 2, 44),
        "ONE DISCRIMINATING QUESTION, THREE DIAGNOSES OF DOMINATION",
        font=title_font,
        fill="#F4F8FC",
        anchor="ma",
    )
    draw.text(
        (width / 2, 116),
        "What chiefly produces domination — coercive authority, the capitalist "
        "mode of production, or uncontrolled productive property?",
        font=regular,
        fill="#7FE0C6",
        anchor="ma",
    )
    ...
    cards = (
        (
            ...
            "#EC9BB4",
            "ANARCHISM",
            "domination = coercive authority",
            [
                "monopoly of law and coercion",
                "burden of justification unmet",
                "mutual aid, federation, recall",
                "Proudhon · Bakunin · Kropotkin",
                "abolish, do not capture",
        ...
        (
            ...
            "#F0B277",
            "MARXISM",
            "domination = the mode of production",
            [
                "forces, relations, mode",
                "base and superstructure",
                "alienation and surplus value",
                "socialised vs private appropriation",
                "transitional power, then classless",
                "Lenin · Gramsci · Althusser",
            ],
        ),
        (
            1250,
            ...
            "#5FD7C2",
            "SOCIALISM",
            "domination = uncontrolled property",
            [
                "social ownership + democratic control",
                "utopian · Fabian · guild · market",
                "Gandhian trusteeship and self-rule",
    ...
    for x, fill, outline, label, subtitle, items in cards:
        ...
        draw.text((x + 280, 224), label, font=heading, fill="#FFFFFF", anchor="ma")
        draw.text((x + 280, 278), subtitle, font=small, fill=outline, anchor="ma")
        y = 350
        for item in items:
            draw.ellipse((x + 34, y + 7, x + 48, y + 21), fill=outline)
            draw.text((x + 68, y), item, font=small, fill="#EDF4F9")
            y += 88
    for start, end in ((630, 660), (1220, 1250)):
        draw.line((start, 540, end, 540), fill="#7FE0C6", width=10)
        draw.polygon(
            [(end - 16, 524), (end, 540), (end - 16, 556)],
            fill="#7FE0C6",
        )

    draw.rounded_rectangle(
        (120, 946, width - 120, 1256),
        20,
        fill="#101F31",
        outline="#F5CE79",
        width=3,
    ...
    draw.text(
        (width / 2, 992),
        "THE SHARED AXES ON WHICH EVERY COMPARISON IS DECIDED",
        font=heading,
        fill="#FFF2D2",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1052),
        "authority · productive property · class · revolution or reform · "
        "the state · freedom",
        font=regular,
        fill="#F4F8FC",
        anchor="mm",
    ...
    draw.text(
        (width / 2, 1100),
        "transition design · violence and means · scale of production · "
        "WHO IS MADE ANSWERABLE, AND HOW",
        font=regular,
        fill="#F4F8FC",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1156),
        "The diagnosis generates the prescription: fix what causes domination, "
        "and the demand to abolish,",
        font=small,
        fill="#A9DFF0",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1196),
        "to capture transitionally, or to socialise under democratic control "
        "follows from it — never the reverse.",
        font=small,
        fill="#A9DFF0",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1232),
        "Marxism is one member of the socialist family; anarchism refuses the "
        "transitional state that Marxism requires.",
        font=small,
        fill="#F0C177",
        anchor="mm",
    )
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+5\.\s+Political Ideologies\s*(.*?)(?=^##\s+6\.\s+Humanism)",
        text,
    )
    if not match:
        raise ValueError(
            "The Political Ideologies advanced dossier section was not found."
        )
...
def insert_advanced_dossier(text: str, fragment: str) -> str:
    ...
    block = "\n\n".join(
        [
            ...
            (
                "> **Classification: OPTIONAL ADVANCED.** The analytic-against-"
                "orthodox Marxism dispute, the market-socialism design debate, "
                "the class-reduction quarrel and the institutional reading of "
                "anarchism are unnecessary for a competent Core answer on this "
                "clause. Use them only once anarchism, historical materialism, "
                "alienation, exploitation, the state, socialism and the "
                "socialism-communism boundary are secure."
            ),
            fragment,
        ]
    )
...
def insert_concept_visual(text: str, concept_visual: Path) -> str:
    ...
    block = (
        f"\n\n![Anarchism, Marxism and socialism on shared axes]({image_path})\n\n"
        "*Concept map: one discriminating question about the source of "
        "domination separates the three named ideologies, and the demand to "
        "abolish, to capture transitionally or to socialise under democratic "
        "control follows from the diagnosis rather than preceding it.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "anarchism-Marxism-socialism diagnosis, revision and criticism atlas"
        ),
        "generated_on": GENERATION_DATE,
        "scope": (
            "Philosophy Optional Paper II Socio-Political Philosophy topic 05 only"
        ),
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "State and Forms of Government packages and every "
                    "legacy-v1 Political Ideologies artifact remain immutable."
                ),
                "panels": [
                    {
...
def owner_pyqs(ledger: str) -> list[str]:
    ...
    for line in ledger.splitlines():
        if "[Political Ideologies]" not in line or not line.lstrip().startswith("- ("):
            continue
...
def render_ascii_pdf(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = "Political Ideologies ASCII Master Flowchart"
        metadata["creator"] = (
            "generate_philosophy_socio_political_political_ideologies_v2.py"
        )
    ...
    if not validation["passed"]:
        raise ValueError("Political Ideologies ASCII PDF validation failed.")
...
def build_record(
    ...
    flow_metadata["ascii_master_source"] = "manual-authored-political-ideologies-spec"
    ...
    return {
        ...
        "command": (
            ...
            "— Socio-Political Philosophy — Political Ideologies"
        ),
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_socio_political_political_ideologies_v2.py"
...
def verify_final_state(generation: int, paths: dict[str, Path]) -> dict[str, Any]:
    ...
    if states[4].package_state != "generated":
        raise ValueError(
            f"Topic 05 state is {states[4].package_state!r}, expected generated."
        )
    earlier = [state.package_state for state in states[:4]]
    if any(state != "generated" for state in earlier):
        raise ValueError(
            "Topics 01-04 must remain generated: " + ", ".join(earlier)
        )
    later = [state.package_state for state in states[5:]]
    if any(state != "planned" for state in later):
        raise ValueError(
            "Topics 06-10 were unexpectedly marked beyond planned: " + ", ".join(later)
        )
    ...
    command = (
        "Generate learner-v2 topic: Philosophy Optional — Philosophy Paper II — "
        "Socio-Political Philosophy — Political Ideologies — Regenerate"
    )
    ...
    return {
        ...
        "topics_06_10_states": later,
        "approval": states[4].approval_state,
        "validation": states[4].validation_state,
...
def run() -> int:
    ...
    changed: set[str] = {
        ...
        relative(
            ROOT
            / "tools"
            / "philosophy_socio_political_political_ideologies_v2_spec.py"
        ),
        relative(MANIFEST),
        relative(ASCII_SPEC),
