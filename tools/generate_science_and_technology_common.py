"""Reusable authoring-only engine for Science and Technology learner-v2 topics.

The engine follows the completed Economy and Environment single-writer
workflow while enforcing science-specific category and status discipline.
It never invents institutional mandates, performance values, programme
outcomes, procurement or deployment status, mission targets, PYQs or answers.
"""
...
DATE = os.environ.get("SCIENCE_TOPIC_DATE", "2026-09-03")
...
COMMON_CROSS = [
    ...
    KNOWLEDGE / "REVISION-CHART_Principles-Applications-and-Distinctive-Features.md",
...
_KEYWORD_EXCLUSIONS = {
    ...
    "owner", "programme", "question", "science", "source", "status",
    "technology", "topic", "which", "while", "with", "without",
}
REGISTER_HEADINGS = (
    "SYSTEM, MISSION AND INSTITUTION MAP",
    "CATEGORY, STATUS AND NUMBER FIREWALLS",
    "ANSWER-WRITING SPINE",
    "LIVE-SOURCE AND VOLATILE-CLAIM BOUNDARY",
)
...
def make_plans(
    ...
    if len(titles) != 15:
        raise ValueError("Exactly fifteen Science and Technology sessions are required.")
    return [
    ...
    for marks, prompt, indexes in prompts:
        ...
        model = " ".join(
            (
                ...
                "**Analysis:** This fixes the scientific process, system boundary, "
                "institution, application and verified capability rung before "
                "drawing the policy implication. **Qualification:** The conclusion "
                "retains the owner's date, unit, institution, system category, "
                "legal character and development-to-deployment status boundary."
            )
...
def topic(
    ...
    return {
        ...
        "ocr_note": (
            ...
            "answer key, performance value, procurement outcome, operational "
            "deployment, programme target or technology milestone was inferred."
        ),
        "pyq_note": pyq_note,
...
def _session_keywords(title: str, selected: list[tuple[str, str]]) -> list[str]:
    ...
    for word in re.findall(r"[A-Za-z][A-Za-z0-9'-]{3,}", source):
        ...
        if len(result) == 6:
            break
    if len(result) < 4:
        raise ValueError(f"{title}: fewer than four science-specific keywords.")
    return result
...
def _session_fragment(
    ...
    return (
        ...
        "examinable scientific or technological system.\n\n"
        f"**Technical definition:** The answer fixes the physical process, "
        "mission or service layer, responsible institution, unit of measurement "
        "and verified capability rung before comparing performance or impact.\n\n"
        "#### ANSWER-GRABBING OPENING — WRITE/ADAPT IN THE EXAM\n\n"
        f"> {title} must be analysed through {joined}, while preserving the "
        "difference between design, test, launch, operation and outcome.\n\n"
        "#### MUST-WRITE KEYWORDS\n\n"
        ...
        f"institution, unit and status. Qualification: {caution}\n\n"
        "#### VISUAL FIRST\n\n```text\n"
        + "\n".join(visual)
        + "\n```\n\n*The rail fixes the technical sequence and the claim boundary "
        "before explanation.*\n\n#### CORE EXPLANATION\n\n"
        + " ".join(statement for _, statement in selected)
        + "\n\n#### SIMPLE SYSTEM EXAMPLE\n\n"
        + f"Read {title} as a sequence: identify {labels[0]}, follow the "
        "named mechanism to its user or output, and stop at the last verified "
        "status rather than assuming the next stage.\n\n"
        + "#### TECHNICAL DISTINCTION\n\n"
        + f"The decisive boundary is {caution} This keeps {joined} in the "
        "correct scientific category, institution and unit of measurement."
        + "\n\n#### NAMED EVIDENCE AND MECHANISM\n\n"
        + "\n".join(f"- {statement}" for _, statement in selected)
        + f"\n\n#### EXAMINER CAUTION\n\n- {caution}\n\n#### EXAM LINK\n\n"
        "- **Prelims:** Keep institution, system, platform, service, process, "
        "legal instrument, unit and status in their exact category.\n"
        f"- **Mains:** {exam_use}\n\n#### MINI RECAP\n\n"
        ...
        f"MECHANISM / ARGUMENT: connect {joined} without changing the unit or status\n"
        f"CONSEQUENCE / CONTRAST: {exam_use}\n"
        f"UPSC TRAP / ANSWER-USE: {caution}\n"
        "ANSWER-GRABBING FORMULATION: capability is only the last officially "
        "verified rung, not the next announced rung\n```"
    )
...
def _build_mcqs(config: dict[str, object]) -> str:
    ...
    variants = [
        ...
        "Which option preserves the technical boundary of {label}?",
        "Which statement uses {label} without changing its institution, unit or status?",
    ...
    for fact_index, (label, statement) in enumerate(facts):
        for variant_index, template in enumerate(variants):
            ...
            blocks.append(
                ...
                "scientific category, institutional owner, measurement, mission "
                "rung or operating status."
            )
    return "\n\n".join(blocks)
...
def _assemble(
    ...
    workbook = (
        ...
        "science distinctions and strict A-B-C-D rotation.\n\n"
        "## BASIC MCQS / REMEDIATION\n\n" + mcqs
        + "\n\n## PYQS AND ANSWER PRACTICE\n\n" + practice + "\n"
    )
...
def ensure_section_manifest() -> Path:
    ...
    target = next(
        ...
        if item.get("topic_key") == "science-and-technology-01"
    )
    path = section_indexes.materialize_catalog_section_manifest(ROOT, catalog, target)
    if path != SECTION_MANIFEST:
        raise ValueError(f"Unexpected Science and Technology manifest: {path}")
    return path
...
def validate_catalog(topics: list[dict[str, object]]) -> None:
    ...
    if missing:
        raise ValueError(f"Science and Technology topics missing from catalogue: {missing}")
...
def _write_generation_spec(
    ...
    if missing:
        raise FileNotFoundError("Missing Science and Technology sources: " + ", ".join(missing))
    catalog = json.loads(CATALOG.read_text(encoding="utf-8"))
    catalog_topic = next(
    ...
    payload = {
        ...
        "supersedes": (
            "science-and-technology-01:legacy-v1:g1"
            if config["key"] == "science-and-technology-01" else None
        ),
