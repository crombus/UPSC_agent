"""Validate and inventory the completed International Relations semantic review."""
...
import run_international_relations_semantic_topic as subject
...
REPORT = (
    ...
    / f"International-Relations-Subject-Semantic-Completion-{DATE}.md"
)
VALIDATION = (
    EXPORTS
    / f"international-relations-01-12-semantic-completeness-{DATE}-validation.json"
)
INVENTORY = (
    EXPORTS
    / f"international-relations-01-12-semantic-completeness-{DATE}-changed-files.txt"
)
...
def main() -> int:
    ...
    expected = [f"international-relations-{number:02d}" for number in range(1, 13)]
    errors: list[str] = []
    if [row["topic_key"] for row in rows] != expected:
        errors.append("International Relations semantic catalogue/order mismatch.")
    if any(row["status"] != "passed" for row in rows):
        errors.append("Not every International Relations semantic row is passed.")
    if semantic["next_topic"]["topic_key"] != "economy-01":
        errors.append("Global queue did not advance to economy-01.")

    tracker_text = (
        ROOT / "KNOWLEDGE-SEMANTIC-COMPLETENESS-TRACKER.md"
    ).read_text(encoding="utf-8")
    if "| 12 | International Relations | 12 | 12 | passed | Complete |" not in tracker_text:
        errors.append("Human tracker does not record International Relations 12/12 passed.")
    if "Economy — Subject-wide Syllabus — National Income: GDP, GVA, GNP, NDP and Measurement" not in tracker_text:
    ...
    inventory: set[str] = {
        ...
        "tools\\finalize_international_relations_semantic_review.py",
    ...
    for number, key in enumerate(expected, 1):
        ...
        report_path = next(
            ...
            if path.startswith(
                "upsc-ai-kit\\manifests\\reviews\\international-relations\\"
            )
    ...
    if not all_approval_false:
        errors.append("At least one International Relations identity is approved.")
    ...
    payload = {
        ...
        "checks": {
            ...
            "status_tracker_agreement": (
                "| 12 | International Relations | 12 | 12 | passed | Complete |"
                in tracker_text
            ),
    ...
    REPORT.write_text(
        f"""# International Relations Semantic-Completeness Completion — 6 September 2026

**Result:** {'FAILED' if errors else 'PASSED'}  
**Coverage:** {payload['passed']}/12  
**Next queue item:** `economy-01` — National Income: GDP, GVA, GNP, NDP and Measurement

| Topic | Identity | Main pages | Workbook pages | Solved | MCQs | ASCII/graphical | Result |
|---|---|---:|---:|---:|---:|---:|---|
{table_rows}

Targeted International Relations regression: {test_count} tests, zero failures.
All learner-v2 International Relations identities remain `approved: false`;
...
""",
    ...
    if errors:
        raise RuntimeError("International Relations final validation failed: " + "; ".join(errors))
