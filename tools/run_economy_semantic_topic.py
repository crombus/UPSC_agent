"""Run one strictly sequential Economy semantic review."""
...
runner.TOPIC_CHOICES = range(1, 32)
runner.DEEP_REVIEW_TEST_MODULE = "test_regenerate_economy_deep_review"
runner.REPORT_DIR = (
    deep.ROOT
    / "upsc-ai-kit"
    / "manifests"
...
runner.PYQ_STATUS = {
    number: re.split(
        r"(?<=[.!?])\s+",
        deep.ECONOMY_PYQ_STATUS[number],
        maxsplit=1,
    )[0]
    for number in range(1, 32)
}
deep.POLITY_LIVE_OFFICIAL_SOURCES = (
    deep.ECONOMY_LIVE_OFFICIAL_SOURCES
)
    ...
    row["findings"] = [
        {
            "severity": "closed",
            "finding": (
                "Four-ledger hostile audit closed; literal syllabus, prerequisites, "
                "textbook taxonomy, formulas, stock-flow and nominal-real boundaries, "
                "institutions, programme stages, data vintages, transmission chains, "
                "WTO qualifications, current official evidence, PYQ ownership, answer "
...
def report_text(
    ...
    return f"""# Economy Semantic-Completeness Review {topic.number:02d} - {topic.title}
...
def run_tests() -> list[dict[str, object]]:
    modules = [
        "test_regenerate_economy_deep_review",
        "test_run_economy_semantic_topic",
        *[
...
def run(topic_number: int) -> dict[str, object]:
    if topic_number not in runner.TOPIC_CHOICES:
        raise ValueError(
            "Economy runner accepts only topic numbers 1 through 31."
        )
