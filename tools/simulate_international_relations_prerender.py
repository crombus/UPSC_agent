"""Pre-render simulation for International Relations learner-v2 topics.
...
def simulate(topic_key: str, title: str) -> dict[str, object]:
    ...
    # A session title that the shared publisher classifies as workflow metadata
    # is silently demoted, so the published package loses a numbered session
    # while the authored source still shows fifteen. Detect that here, before a
    # PDF is rendered, instead of discovering it in the completion proof.
    published_sessions = re.findall(r"(?m)^### SESSION (\d+) — ", transformed)
    meta_titles = [
        item for item in authored_titles if refresh.session_title_is_meta(item)
    ]
    session_errors: list[str] = []
    if len(authored_titles) != 15:
        session_errors.append(
            f"authored session count is {len(authored_titles)}, expected 15."
        )
    if len(published_sessions) != 15:
        session_errors.append(
            "published session count after sessionize is "
            f"{len(published_sessions)}, expected 15."
        )
    if transformed.count("#### VISUAL FIRST") != 15:
        session_errors.append(
            "published visual-first count is "
            f"{transformed.count('#### VISUAL FIRST')}, expected 15."
        )
    for item in meta_titles:
        session_errors.append(
            f"session title is classified as workflow metadata: {item!r}"
        )
    return {
        "topic_key": topic_key,
        "sessionize_ok": bool(transformed),
        "authored_session_count": len(authored_titles),
