"""Generate Crime and Punishment as a learner-v2 topic.
...
package for ``Crime and Punishment : Corruption, Mass Violence, Genocide,
Capital Punishment.`` only, and never rewrites topics 01-06 or the planned
topics 08-10 beyond the shared indexes and manifests that must reflect this
topic's completion.
"""
...
import philosophy_socio_political_crime_and_punishment_v2_spec as topic_spec
...
ASCII_SPEC = (
    ...
    / "retrofits"
    / "ascii-panel-specs"
    / f"philosophy--{SECTION_KEY}-07-ascii-{GENERATION_DATE}.json"
)
CONTENT_SPEC_DIR = (
    ROOT
    / "upsc-ai-kit"
...
RETAINED_SESSION = (
    ...
    "learning-sessions\\Crime-and-Punishment\\"
    "Crime-and-Punishment_Layered-Complete-Learning-Session_"
    "2026-08-20.md"
)
RETAINED_WORKBOOK = (
    "upsc-ai-kit\\knowledge\\Philosophy\\Socio-Political-Philosophy\\"
    "learning-sessions\\Crime-and-Punishment\\"
    "Crime-and-Punishment_Layered-Solved-Practice-Workbook_"
    "2026-08-20.md"
)
...
EXPECTED_PYQ_COUNT = 12
EXPECTED_MCQ_COUNT = 48
PYQ_OWNER_LABEL = "[Crime and Punishment]"

CORE_COMPLETENESS_MARKERS = (
    "three doors with three different keys",
    "criminalisation filter",
    "validity does not prove justice",
    "revenge with paperwork",
    "graded by intention",
    "causal contribution",
    "ground and the ceiling",
    "annulment",
    "limiting or negative retributivism",
    "works on his opportunities",
    ...
    "side-constraint",
    "promise-keeping",
    "supplies the repair of harm",
    "secular penance",
    "standing to punish",
    "reintegrative shaming",
    "genealogy",
    "continued existence was itself the target",
    "fiduciary",
    "particularistic",
    "systemic ecology",
    ...
    "rarest of rare",
    "advisory report",
    "adjudicate one live rival",
    "graded verdict",
)

ENGLISH_FIRST_REPLACEMENTS = (
    (
        "**Syllabus (verbatim):** Crime and Punishment : Corruption, Mass "
        "Violence, Genocide, Capital Punishment.",
        f"**Syllabus (verbatim):** {OFFICIAL_CLAUSE}",
    ),
    (
        "culpability by mens rea;",
        "culpability by a guilty mind (mens rea);",
    ),
    (
        "public wrong · culpability · mens rea · proportionality",
        "public wrong · culpability · guilty mind (mens rea) · proportionality",
    ),
    (
        "in technical vocabulary: *mens rea*,",
        "in technical vocabulary: guilty mind (*mens rea*),",
    ),
    (
        "ANALYSIS: Mala in se versus mala prohibita.",
        "ANALYSIS: Wrong in itself (mala in se) versus wrong because "
        "prohibited (mala prohibita).",
    ),
    (
        "- mala in se (wrong in itself) vs mala prohibita (wrong because "
        "prohibited); the",
        "- wrong in itself (mala in se) against wrong because prohibited "
        "(mala prohibita); the",
    ),
)
...
def build_manifest(tracker: dict[str, Any], generation: int) -> dict[str, Any]:
    ...
    manifest["section"]["notes"] = (
        ...
        "topics 08-10 remain planned until separately generated and validated."
    )
    return manifest
...
def make_concept_visual(output: Path) -> None:
    ...
    title_font = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 46)
    heading = ImageFont.truetype(r"C:\Windows\Fonts\segoeuib.ttf", 32)
    regular = ImageFont.truetype(r"C:\Windows\Fonts\segoeui.ttf", 26)
    small = ImageFont.truetype(r"C:\Windows\Fonts\segoeui.ttf", 21)

    draw.text(
        (width / 2, 36),
        "THREE DOORS FROM A WRONG TO A PUNISHMENT, AND FOUR NAMED CASES",
        font=title_font,
        fill="#F2F6FA",
        anchor="ma",
    )
    draw.text(
        (width / 2, 100),
        "When, and on what terms, may a political community turn a wrong into "
        "a crime and attach punishment to it?",
        font=regular,
        fill="#7ADFB6",
        anchor="ma",
    )
    ...
    doors = (
        (
            ...
            "#8FB6F2",
            "DOOR 1  WRONGDOING",
            "has a justified norm been breached?",
            [
                "legal crime and moral wrong overlap",
                "an unjust law can hit the innocent",
                "a grave wrong can escape the code",
                "validity does not prove justice",
            ],
        ),
        (
            ...
            "#13302C",
            "#5FD7A8",
            "DOOR 2  CRIMINALISATION",
            "may the state bring it inside the code?",
            [
                "harm to others",
                "rights violated",
                "public wrong, not private only",
                "culpability, then proportionality",
            ],
        ),
        (
            1286,
            ...
            "#F09090",
            "DOOR 3  PUNISHMENT",
            "by which theory, within which limits?",
            [
                "retributive desert, backward",
                "deterrent and incapacitative, forward",
                "reformative reintegration",
                "restorative repair and censure",
            ],
        ),
    )
    for x, fill, outline, label, subtitle, items in doors:
        ...
        draw.text((x + 274, 190), label, font=heading, fill="#FFFFFF", anchor="ma")
        draw.text((x + 274, 240), subtitle, font=small, fill=outline, anchor="ma")
        y = 300
        for item in items:
            draw.ellipse((x + 32, y + 6, x + 46, y + 20), fill=outline)
            draw.text((x + 64, y), item, font=small, fill="#EAF1F7")
            y += 74
    for start, end in ((634, 686), (1234, 1286)):
        draw.line((start, 400, end, 400), fill="#7ADFB6", width=9)
        draw.polygon(
            [(end - 16, 384), (end, 400), (end - 16, 416)],
            fill="#7ADFB6",
        )
    draw.text(
        (width / 2, 660),
        "EACH DOOR NEEDS ITS OWN KEY  ·  abhorrence at door one never opens "
        "door three",
        font=regular,
        fill="#F5CE79",
        anchor="ma",
    ...
    cases = (
        (
            ...
            "#9BB7E8",
            "CORRUPTION",
            "power held in TRUST turned to partial use",
        ),
        (
            548,
            "#22252F",
        ...
        (
            ...
            "#E0A2D6",
            "GENOCIDE",
            "the group's EXISTENCE is itself the target",
        ),
        (
            1472,
            "#33232A",
            "#F0A5A5",
            "CAPITAL PUNISHMENT",
            "IRREVERSIBLE, and after control is established",
        ),
    )
    for x, fill, outline, label, line in cases:
        draw.rounded_rectangle(
            (x, 718, x + 400, 918),
            20,
            fill=fill,
        ...
        draw.text((x + 200, 748), label, font=heading, fill="#FFFFFF", anchor="ma")
        words = line.split()
        first = " ".join(words[: len(words) // 2])
        second = " ".join(words[len(words) // 2 :])
        draw.text((x + 200, 812), first, font=small, fill=outline, anchor="ma")
        draw.text((x + 200, 846), second, font=small, fill=outline, anchor="ma")

    draw.rounded_rectangle(
        (100, 960, width - 100, 1352),
        20,
        fill="#101C28",
        outline="#F5CE79",
        width=3,
    ...
    draw.text(
        (width / 2, 1006),
        "SCALE DOES NOT MAKE THE WRONG - THE DISTINCTIVE FEATURE DOES",
        font=heading,
        fill="#FFF1D0",
        anchor="mm",
    )
    for offset, line in enumerate(
        (
            "separate the three justifying questions: why the institution "
            "exists, whom it may touch, and how far it may go",
            "desert supplies the ceiling, prevention supplies the aim, "
            "condemnation supplies the meaning, restoration supplies the repair",
            "guilt is individual; responsibility admits organisational, "
            "political and omission forms, and never guilt by membership",
            "the custody distinction, not the absoluteness of rights, carries "
            "the argument against an irreversible penalty",
        )
    ):
        draw.text(
            (width / 2, 1064 + offset * 46),
            line,
            font=regular,
            fill="#F2F6FA",
            anchor="mm",
    ...
    draw.text(
        (width / 2, 1266),
        "Deterrent effect is an empirical controversy; a judgment, an advisory "
        "report and a statute are three different things.",
        font=small,
        fill="#A6DCEF",
        anchor="mm",
    )
    draw.text(
        (width / 2, 1310),
        "Answer at the level of criteria: this package names no episode, "
        "community, movement, technology or government.",
        font=small,
        fill="#F0C177",
        anchor="mm",
    )
...
def advanced_dossier_fragment() -> str:
    ...
    match = re.search(
        r"(?ims)^##\s+7\.\s+Crime and Punishment\s*"
        r"(.*?)(?=^##\s+8\.\s+Development and Social Progress)",
        text,
    )
    if not match:
        raise ValueError(
            "The Crime and Punishment advanced dossier section was not found."
        )
...
def insert_advanced_dossier(text: str, fragment: str) -> str:
    ...
    block = "\n\n".join(
        [
            ...
            (
                "> **Classification: OPTIONAL ADVANCED.** The residual "
                "question whether censure-communication can be coercively "
                "delivered, the question whether restorative practice scales "
                "to grave and mass crimes, the collective and command "
                "responsibility refinement and the abolition-against-"
                "constrained-retention debate are unnecessary for a competent "
                "Core answer on this clause. Use them only once the "
                "criminalisation filter, the conditions of responsible "
                "punishment, the five theories, limiting retributivism, the "
                "three-question grid, the expressive, communicative and "
                "restorative family, the three named public wrongs and the "
                "custody distinction are already secure."
            ),
            fragment,
        ]
    )
...
def insert_concept_visual(text: str, concept_visual: Path) -> str:
    ...
    block = (
        f"\n\n![Three doors from a wrong to a punishment, and the four named "
        f"cases of the clause]({image_path})\n\n"
        "*Concept map: wrongdoing, criminalisation and punishment are three "
        "gates with three different keys, and corruption, mass violence, "
        "genocide and capital punishment are each distinguished by the "
        "feature that makes the wrong what it is rather than by how many "
        "people it touches.*\n"
    )
...
def make_ascii_spec(markdown: Path, generation: int) -> dict[str, Any]:
    return {
        ...
        "benchmark": (
            ...
            "crime, punishment, corruption, mass violence, genocide and "
            "capital punishment atlas"
        ),
        "generated_on": GENERATION_DATE,
        "scope": (
            "Philosophy Optional Paper II Socio-Political Philosophy topic 07 only"
        ),
        "constraints": {
        ...
        "topics": [
            {
                ...
                "benchmark_preservation": (
                    ...
                    "State, Forms of Government, Political Ideologies and "
                    "Humanism, Secularism and Multiculturalism packages and "
                    "every legacy-v1 Crime and Punishment artifact remain "
                    "immutable."
                ),
                "panels": [
                    {
...
def render_ascii_pdf(text: str, output_path: Path) -> dict[str, Any]:
    ...
    with fitz.open(output_path) as document:
        ...
        metadata["title"] = (
            "Crime and Punishment ASCII Master Flowchart"
        )
        metadata["creator"] = (
            "generate_philosophy_socio_political_crime_and_punishment_v2.py"
        )
    ...
    if not validation["passed"]:
        raise ValueError(
            "Crime and Punishment ASCII PDF validation "
            "failed."
        )
...
def build_record(
    ...
    flow_metadata["ascii_master_source"] = (
        "manual-authored-crime-and-punishment-spec"
    )
    ...
    return {
        ...
        "command": (
            ...
            "— Socio-Political Philosophy — Crime and Punishment"
        ),
        ...
        "validation": {
            ...
            "validator": (
                "tools/generate_philosophy_socio_political_crime_and_"
                "punishment_v2.py + tools/validate_v2_export.py"
            ),
        },
        "generated_on": GENERATION_DATE,
...
def verify_final_state(generation: int, paths: dict[str, Path]) -> dict[str, Any]:
    ...
    if states[6].package_state != "generated":
        raise ValueError(
            f"Topic 07 state is {states[6].package_state!r}, expected generated."
        )
    earlier = [state.package_state for state in states[:6]]
    if any(state != "generated" for state in earlier):
        raise ValueError(
            "Topics 01-06 must remain generated: " + ", ".join(earlier)
        )
    later = [state.package_state for state in states[7:]]
    if any(state != "planned" for state in later):
        raise ValueError(
            "Topics 08-10 were unexpectedly marked beyond planned: " + ", ".join(later)
        )
    ...
    command = (
        "Generate learner-v2 topic: Philosophy Optional — Philosophy Paper II — "
        "Socio-Political Philosophy — Crime and Punishment — Regenerate"
    )
    ...
    return {
        ...
        "topic_07_state": states[6].package_state,
        "topics_08_10_states": later,
        "approval": states[6].approval_state,
        "validation": states[6].validation_state,
...
def run() -> int:
    ...
    for index, stage in enumerate(graphical_data["stages"][:-1]):
        ...
        stage["answer_line"] = topic_spec.GRAPHICAL_STAGE_ANSWERS[index]
        stage["mechanism_strip"] = topic_spec.GRAPHICAL_STAGE_MECHANISMS[index]
    ...
    changed: set[str] = {
        ...
        relative(
            ROOT
            / "tools"
            / "philosophy_socio_political_crime_and_punishment_v2_spec.py"
        ),
        relative(MANIFEST),
        relative(ASCII_SPEC),
