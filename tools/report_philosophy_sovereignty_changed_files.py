"""Write the exhaustive changed-files manifest for the Sovereignty learner-v2 run.
...
``philosophy-paper-ii-socio-political-philosophy-02:learner-v2:g2``, grouped by
role, with a SHA-256 for each entry.  Compiler caches are excluded and no
unrelated tree is inspected or rewritten.
"""
...
EXCLUDED = re.compile(r"(?:^|\\)__pycache__(?:\\|$)|\.pending$|\.pyc$")

GROUPS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "Tooling (generator, content spec, reset helper, shared validator and test)",
        re.compile(r"^tools\\"),
    ),
    (
...
def main() -> int:
    ...
    lines = [
        ...
        "",
        f"- **Topic:** Philosophy Optional — Philosophy Paper II — "
        f"Socio-Political Philosophy — Sovereignty",
        ...
        "",
        "Every path below is repository-relative. Compiler caches, temporary "
        "`.pending` swap files and this manifest itself are excluded. No "
        "unrelated tree was rewritten.",
        "",
    ]
