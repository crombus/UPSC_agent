"""Finalize validation and publication records for the Polity 28-32 batch."""
...
import generate_polity_28_32_sequential as batch
...
POLITY_FLOW_VALIDATION = (
    EXPORTS / "polity-01-32-flow-learning-2026-08-24-validation.json"
)
ALL_FLOW_VALIDATION = (
...
REPORT = (
    ...
    / "POLITY-28-32-SEQUENTIAL-BATCH-REPORT.md"
)
...
def write_case_evidence() -> dict[str, Any]:
    ...
    topic_keys = [f"polity-{number:02d}" for number in range(1, 33)]
...
def run_flow_publications(tests_passed: int) -> None:
    ...
    subprocess.run(
        [
            ...
            "polity-",
            "--report-path",
            "notes\\Flow-Learning\\POLITY-01-32-FLOW-LEARNING-REPORT.md",
            "--validation-path",
...
def build_validation(tests_passed: int) -> dict[str, Any]:
    ...
    checks = {
        ...
        "prior_60_clean_hashes_preserved": not state[
            "existing_clean_hash_mismatches"
        ],
        "prior_60_flow_hashes_preserved": not state[
            "existing_flow_hash_mismatches"
        ],
        "prior_generated_topics_preserved": not state[
            "prior_generated_topic_hash_mismatches"
        ],
        "four_item_inventory_65": four["topic_count"] == 65
        and four["status"] == "passed",
        "polity_flow_inventory_32": (
            polity_flow["summary"]["topic_folder_count"] == 32
            and polity_flow["status"] == "passed"
        ),
        "full_flow_inventory_65": (
            all_flow["summary"]["topic_folder_count"] == 65
...
def write_report(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Polity 28-32 Sequential Batch Report",
        "",
        "- Status: **PASSED**",
        "- Execution: strict order `polity-28 -> polity-29 -> polity-30 -> polity-31 -> polity-32`.",
        ...
        "- Final clean four-item library: **65 topics**. Final Flow Learning inventory: **65 topics**, including **Polity 32**.",
        "",
    ...
    lines.extend(
        [
            ...
            "- Existing 60 clean packages: 480 canonical artifact files checked, 0 hash mismatches.",
            "- Existing 60 Flow Learning topics: 180 topic artifacts checked, 0 hash mismatches.",
            f"- Full four-item library: 65 topics; {payload['inventories']['four_item_links_checked']} links checked; 0 broken.",
            f"- Polity Flow Learning: 32 topics; {payload['inventories']['polity_flow_links_checked']} links checked; 0 broken.",
            f"- Full Flow Learning: 65 topics; {payload['inventories']['full_flow_topic_artifact_files_checked']} topic artifacts hash-checked; {payload['inventories']['full_flow_links_checked']} navigation links checked; 0 broken; source/destination PDF and TXT hashes equal.",
...
def write_changed_files() -> list[str]:
    ...
    explicit = {
        ...
        ROOT / "tools" / "generate_polity_28_32_sequential.py",
        ROOT / "tools" / "finalize_polity_28_32_batch.py",
        ...
        / "notes"
        / "Flow-Learning"
        / "POLITY-01-32-FLOW-LEARNING-REPORT.md",
        FINAL_VALIDATION,
        REPORT,
        CASE_EVIDENCE,
        POLITY_FLOW_VALIDATION,
