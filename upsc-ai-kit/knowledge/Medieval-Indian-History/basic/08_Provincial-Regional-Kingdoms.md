# Provincial & Regional Kingdoms (Bengal, Gujarat, Malwa, Jaunpur, Kashmir) — MUST-DO

> **Subject:** History (Medieval India) · **Tier:** Must-Do (foundation) · **GS Paper:** GS-I (also Prelims).
> **Grounded in:** Satish Chandra, *History of Medieval India* — chapter "Struggle for Empire in North India—II" (regional kingdoms: Bengal, Gujarat, Malwa, Jaunpur, Kashmir) + official UNESCO Moidams property page for the bounded Ahom/Assam extension.
> ✅ = from source book · ⚠️ = inference / standard knowledge · 📰 = current affairs.
> *Companion: `advanced/08_Provincial-Regional-Kingdoms.md`. Chronology spine: `00_Master-Chronology.md`.*

---

## Mini-timeline

| Date | Event |
|---|---|
| ✅ 1338 | Bengal again breaks away from Delhi during Muhammad bin Tughlaq's troubled reign |
| ✅ 1342 | Ilyas Khan captures Lakhnauti and Sonargaon; rules as Sultan Shamsuddin Ilyas Shah |
| ✅ 1389–1409 | Ghiyasuddin Azam Shah of Bengal — justice tradition, contacts with Hafiz and China |
| ✅ 1407 | Zafar Khan formally proclaims Gujarat independent as Muzaffar Shah |
| ✅ 1411–42 | Ahmad Shah I consolidates Gujarat and founds Ahmedabad (the assigned source places the foundation in 1413) |
| ✅ 1420–70 | Zainul Abidin rules Kashmir and reverses earlier persecution |
| ✅ 1459–1511 | Mahmud Begarha rules Gujarat; captures Girnar and Champaner |
| ✅ 1484 | Bahlul Lodi annexes Jaunpur and ends the Sharqi kingdom |

## 1. Snapshot & core idea

**Foundation — why regional kingdoms mattered.**

✅ Timur's invasion and the weakening of Delhi encouraged provincial governors and local powers to assert independence. Bengal, Gujarat, Malwa, Jaunpur and Kashmir were not minor footnotes: they carried forward administration, trade, literature and regional architecture when Delhi was weak.
- ✅ A balance of power emerged: Gujarat, Malwa and Mewar checked one another in the west; Bengal was checked by Orissa and Jaunpur; the Lodis later fought Jaunpur for the Ganga-Yamuna valley.
- ✅ Malwa's plateau position made it a strategic "barrier" between Gujarat, Mewar, the Bahmanis and Delhi.
- ⚠️ UPSC uses this phase to test whether you see medieval India as **regional state formation**, not just Delhi Sultanate chronology.

**Foundation — regional culture, trade and tolerance.**

✅ Bengal flourished because Delhi found it hard to control: distance, climate and water-based communications favoured autonomy. Its sultans patronised Bengali language, and Alauddin Hussain Shah offered high offices to Hindus, including Rupa and Sanatan.
- ✅ Gujarat was rich due to fertile soil, handicrafts and seaports; Ahmad Shah I built Ahmedabad and developed a regional style using Jain architectural traditions.
- ✅ Jaunpur became the **"Shiraz of the East"** because Sharqi rulers patronised learning, saints and architecture.
- ✅ Kashmir under Zainul Abidin cancelled anti-Hindu orders, abolished jizyah and cow slaughter, restored temples and welcomed back non-Muslims.

## 2. Key classification / data

| Region / kingdom | Core ruler(s) | What to remember |
|---|---|---|
| ✅ Bengal | Ilyas Shah; Ghiyasuddin Azam Shah; Alauddin Hussain Shah | Long autonomy; Pandua/Gaur; Bengali literature; Chittagong trade |
| ✅ Gujarat | Ahmad Shah I; Mahmud Begarha | Ahmedabad; ports; Jain-influenced architecture; Girnar + Champaner |
| ✅ Malwa | Hushang Shah; Mahmud Khalji | Mandu; massive architecture; strategic plateau; rivalry with Gujarat/Mewar |
| ✅ Jaunpur | Sharqi sultans | "Shiraz of the East"; lofty gates and huge arches; annexed by Bahlul Lodi |
| ✅ Kashmir | Sikandar Shah; Zainul Abidin | Earlier persecution reversed by Zainul Abidin's broad toleration |

### Ahom/Assam — bounded Prelims extension

| Theme | What to remember |
|---|---|
| ⚠️ State formation | Sukapha entered the Brahmaputra valley in the thirteenth century; the Ahom polity developed outside the Delhi-centred regional-state system covered by the source chapter |
| ⚠️ Paik organisation | Adult male subjects owed rotational labour/military service as *paiks*; do not reduce the system to chattel slavery |
| ⚠️ Records | *Buranjis* are Ahom chronicles and a key source for political and social history |
| ✅ Royal funerary landscape | UNESCO describes Charaideo's moidams as Tai-Ahom royal mound burials created from the thirteenth to nineteenth centuries |

> **Boundary/source note:** The Ahom extension is supplementary and lies outside the three assigned Satish Chandra PDFs. Medieval History owns state formation, administration and chronicles; Art & Culture owns moidam form, cosmology, World Heritage status and conservation.

## 3. Study links

> **Study link:** ✅ Delhi Sultanate decline → `basic/05_Tughlaqs.md` and `00_Master-Chronology.md`.
> **Study link:** ⚠️ Regional states connect forward to Babur/Rana Sanga and the Mughal entry (`basic/12` onward).

## 4. Must-Know Facts (Prelims)

- ✅ Bengal's autonomy rested partly on **distance, climate and waterways**.
- ✅ **Ghiyasuddin Azam Shah** is remembered for justice and diplomatic-literary contacts.
- ✅ **Ahmad Shah I** founded Ahmedabad; **Mahmud Begarha** captured Girnar and Champaner.
- ✅ **Mandu** architecture = massive, high plinths, coloured/glazed tiles.
- ✅ **Jaunpur** = Sharqi capital, "Shiraz of the East", lofty gates and huge arches.
- ✅ **Zainul Abidin** of Kashmir = abolition of jizyah/cow slaughter and restoration of temples.

## 5. UPSC Traps

> 🔑 Trap: Do not reduce regional kingdoms to "decline of Delhi"; many were centres of prosperity and cultural synthesis.

- ❌ Bengal remained subordinate to Delhi after Firuz Tughlaq's campaigns. → It exchanged gifts but was **not subordinate**.
- ❌ Gujarat architecture simply copied Delhi. → It drew on **Jain/Gujarati traditions** and developed a distinct style.
- ❌ Malwa was important only culturally. → Its **geography** made it central to north Indian power politics.
- ❌ Zainul Abidin continued Sikandar's persecution. → He **cancelled** those orders and adopted broad toleration.

## 6. 📰 Current link

📰 **Current linkage (verify live before use):** Conservation of Gaur-Pandua, Champaner, Mandu, Jaunpur and Kashmir's built heritage can be used as a regional-state hook. For the Ahom extension, verify Moidams status and dates from UNESCO; keep funerary architecture in Art & Culture.

## 7. Mains angles

- ⚠️ Examine how geography shaped regional autonomy in Bengal, Malwa and Kashmir.
- ⚠️ "Regional kingdoms were laboratories of cultural synthesis." Discuss with Gujarat, Bengal and Jaunpur.
- ⚠️ Analyse the balance-of-power politics among Gujarat, Malwa, Mewar, Jaunpur and Bengal before Mughal expansion.

<!-- BEGIN GENERATED PYQ INTEGRATION: 2018-2023 -->
## Historical PYQ Integration (2018-2023)

> **Status:** Question-level PYQ demand is integrated into this owner.
> **Provenance:** Audited local official-paper routing ledgers: `_PYQ-ROUTING-PRELIMS-2018-2023.md`.
> **Answer-key rule:** The official 2018-2023 Prelims/CSAT keys are not held locally; no option or answer has been inferred.

- **Years represented:** 2022, 2023
- **Paper(s):** Prelims GS-I
- **Routed question demands:** 3

| Year | Paper | Q | PYQ demand (neutral rendering) | Directive / format | Source status | Owner requirement |
|---:|---|---:|---|---|---|---|
| 2022 | Prelims GS-I | 92 | Early medieval king and dynasty pairs identification | Objective question; official key unavailable locally | Routed; key unavailable locally | Cover the named fact/concept and its likely statement-level distinctions. |
| 2023 | Prelims GS-I | 45 | Medieval dynasties Hoysala Gahadavala Kakatiya Yadava founding period | Objective question; official key unavailable locally | Routed; key unavailable locally | Cover the named fact/concept and its likely statement-level distinctions. |
| 2023 | Prelims GS-I | 49 | Medieval Gujarat rulers Portuguese and Diu surrender | Objective question; official key unavailable locally | Routed; key unavailable locally | Cover the named fact/concept and its likely statement-level distinctions. |

### What this owner must now support

- Early medieval king and dynasty pairs identification
- Medieval dynasties Hoysala Gahadavala Kakatiya Yadava founding period
- Medieval Gujarat rulers Portuguese and Diu surrender

> The table integrates the examinable demand and paper metadata. It does not turn an unkeyed objective question into a solved answer, and it does not claim that lexical presence alone proves full conceptual sufficiency.
<!-- END GENERATED PYQ INTEGRATION: 2018-2023 -->
