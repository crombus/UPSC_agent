# Social Justice - Knowledge Base Index

> **Subject:** Social Justice | **GS Paper:** GS-II, with GS-I society, GS-III
> labour and GS-IV justice links.
> **Spine:** Official UPSC GS-II syllabus; audited local UPSC GS-II and Essay papers
> (2024-2025); the local Laxmikant, Ramesh Singh and Economic Survey 2025-26 texts;
> India Code; official ministries, commissions, surveys and scheme portals.
> **Start here:** `00_Master-Framework.md`.

---

## How this folder works

- **Two tiers per topic:** `basic/NN` builds the must-do entitlement, law, scheme
  and access framework; `advanced/NN` adds implementation gaps, legal nuance,
  institutional trade-offs, data interpretation and Mains depth.
- **Tagging:** ✅ = directly grounded in an official source or verified PYQ;
  ⚠️ = analytical inference or standard linkage; 📰 = a dated current legal,
  scheme or outcome-data anchor.
- **Date discipline:** survey indicators, scheme coverage, crime data, budget
  allocations and portal totals must retain their reporting year/date. For volatile
  claims, this review is current to **21 July 2026**; retain both the source date and
  the status (announced, enacted, notified, commenced, funded or operational).
- **Legal discipline:** an enacted Act, notified rule, pending Bill, judicial
  direction and policy recommendation are not interchangeable.

## Boundaries with other subjects

- **Polity owns constitutional doctrine and constitutional bodies:** equality,
  reservation doctrine, Fifth/Sixth Schedules and NCSC/NCST/NCBC composition.
  Social Justice studies how protective law and welfare delivery affect vulnerable
  groups. NCW, NCM and NCPCR are treated here because their statutory protective
  roles are not covered by a dedicated Polity file.
- **Governance owns generic delivery machinery:** policy design, DBT/PFMS,
  NGOs/SHGs, grievance redress, social audit and monitoring/evaluation. Social
  Justice applies these mechanisms to entitlements and group-specific outcomes.
- **Economy owns macro measurement and production systems:** poverty/inequality
  measurement, procurement/buffer stocks, labour-market trends and fiscal policy.
  Social Justice owns access, protection, entitlements and welfare outcomes.

## Topic map

| # | Topic | Core exam area |
|---|---|---|
| 01 | Social Justice, Inclusion and Welfare-State Framework | Conceptual foundation |
| 02 | Poverty, Hunger, Food and Nutrition Security | Deprivation and nutrition |
| 03 | Health Systems, Public Health and Universal Health Coverage | Social-sector health |
| 04 | Education and Human-Resource Development | Education, skills and capabilities |
| 05 | Women and Gender Justice | Gender rights and empowerment |
| 06 | Children and Child Protection | Child rights and digital-era risks |
| 07 | Scheduled Castes: Rights, Atrocities and Welfare | Protection and mobility |
| 08 | Scheduled Tribes, PVTGs and Tribal Welfare | FRA, PESA and tribal development |
| 09 | OBC, EWS and Social Mobility | Backwardness, targeting and mobility |
| 10 | Minorities: Rights and Welfare | Minority institutions and schemes |
| 11 | Persons with Disabilities | Accessibility and reasonable accommodation |
| 12 | Elderly and Senior Citizens | Maintenance, care and active ageing |
| 13 | Transgender Persons and Denotified/Nomadic Communities | Recognition and rehabilitation |
| 14 | Sanitation, Manual Scavenging and Safai Karamcharis | Dignity, mechanisation and rehabilitation |
| 15 | Labour Social Security, Unorganised and Gig Workers | Informal-work protection |
| 16 | Urban Poor, Homeless and Migrant Workers | Spatial and mobility vulnerability |
| 17 | Scheme Performance, Convergence, Targeting and Data | Synthesis and evaluation |

## Study sequence

1. **Foundation:** `01-02` — justice framework and basic deprivation.
2. **Social-sector capabilities:** `03-04` — health, education and human resources.
3. **Group-specific protection:** `05-13` — women, children, caste, tribe,
   backward classes, minorities, disability, ageing and identity-based exclusion.
4. **Precarious work and habitation:** `14-16` — sanitation labour, informal work,
   migration, homelessness and urban poverty.
5. **Synthesis:** `17` — targeting, convergence, outcome data and correction.

## Primary source routes

- India Code and Gazette notifications for laws and rules.
- MoSJE, MoTA, MWCD, DEPwD and Ministry of Minority Affairs.
- MoHFW, Ministry of Education, Ministry of Labour and MoHUA.
- Ministry of Rural Development and official scheme portals.
- NFHS, PLFS/MoSPI, NCRB and Census sources.
- NITI Aayog SDG and multidimensional-poverty reports.
