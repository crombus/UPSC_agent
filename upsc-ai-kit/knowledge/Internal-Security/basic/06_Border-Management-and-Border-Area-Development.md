...
- ✅ **2024 Q19 (corrected against the paper):** "India
  ...
  - Structure: examine China border issues (disputed LAC, Galwan/Tawang
    ...
    (segment-wise — Radcliffe Line smuggling/tunnelling, LoC infiltration
    and ceasefire dynamics, AGPL altitude/logistics, and the unsettled
    Sir Creek) separately, not as one undifferentiated "China-Pakistan"
    challenge → contrast both with the India-Bangladesh record, where
    negotiated instruments (2014 arbitral award, 2015 Land Boundary
    Agreement) actually settled territory, to show what dispute resolution
    looks like when it works → then describe BADP (village-level
    development within 0-10 km, non-lapsable Special Central Assistance)
    and BIM (Central Sector umbrella scheme for roads, electricity and
    communication infrastructure), citing the current MHA document for
    scheme period/allocation.
