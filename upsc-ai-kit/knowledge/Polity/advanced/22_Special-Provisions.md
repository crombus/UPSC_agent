# Special Provisions for Some States (Articles 370–371-J, Part XXI) — ADVANCED / COMPLETE

> **Subject:** Polity · **Tier:** Advanced (exam depth) · **GS Paper:** GS-II
> **Grounded in:** Indian Polity by M. Laxmikant, Ch. 37 (Qdrant export + source book).
> ✅ = from source book · ⚠️ = inference / case law · 📰 = current affairs.
> *Companion: `basic/Special-Provisions.md`.*

---

# PART A — ARTICLE 370 (former special status of J&K) ⭐
✅ **Art 370 (Part XXI, "Temporary, Transitional & Special Provisions")** gave J&K a special autonomous status —
own constitution, separate flag, and Union laws applied only with the state's concurrence (via **Art 35A**,
added by a 1954 Presidential Order, on permanent residents/property).
📰 ✅ **Abrogation (5 Aug 2019):** Presidential Orders (C.O. 272/273) + Parliament resolutions rendered Art 370
inoperative; the **J&K Reorganisation Act 2019** split the state into **two UTs — J&K (with legislature) &
Ladakh (without)** w.e.f. **31 Oct 2019**.
📰 ⚠️ **SC verdict — *In Re Article 370* (11 Dec 2023):** unanimously **upheld** the abrogation; held Art 370 was
**temporary** and J&K retained **no internal sovereignty**; directed **restoration of statehood "at the
earliest"** and **Assembly elections by Sept 2024**.
📰 ✅ **Latest (verified 2026):** Assembly elections **held Sept–Oct 2024** (90-member Assembly constituted);
**statehood NOT yet restored** — petitions still pending in the SC.

---

# PART B — ARTICLE 371 to 371-J: SPECIAL PROVISIONS FOR 12 STATES ⭐
✅ **Part XXI, Art 371–371-J** — special provisions for **Maharashtra, Gujarat, Nagaland, Assam, Manipur, Andhra
Pradesh, Telangana, Sikkim, Mizoram, Arunachal Pradesh, Goa, Karnataka.** Aim: protect tribal/cultural interests,
backward regions, or disturbed law & order. **Not in the original Constitution** — added by later amendments.

| Article | State(s) | Core special provision |
|---|---|---|
| ✅ **371** | **Maharashtra & Gujarat** | Governor's special responsibility for **development boards** (Vidarbha, Marathwada / Saurashtra, Kutch) |
| ✅ **371-A** | **Nagaland** | Parliament laws on **Naga customary law, land, civil/criminal justice** don't apply unless the Assembly decides; Governor's special law-&-order role |
| ✅ **371-B** | **Assam** | Committee of Assembly for **Tribal Areas** |
| ✅ **371-C** | **Manipur** | Committee of Assembly for **Hill Areas**; Governor's special responsibility |
| ✅ **371-D/E** | **Andhra Pradesh & Telangana** | Equitable public employment/education ("local cadres"); **Administrative Tribunal**; Central University (371-E). *371-D extended to Telangana in 2014* |
| ✅ **371-F** | **Sikkim** | Assembly ≥30; 1 Lok Sabha seat; protection of population sections (36th Amdt 1975) |
| ✅ **371-G** | **Mizoram** | Parliament laws on **Mizo customary law, land, justice** don't apply unless the Assembly decides; Assembly ≥40 |
| ✅ **371-H** | **Arunachal Pradesh** | Governor's special law-&-order responsibility; Assembly ≥30 |
| ✅ **371-I** | **Goa** | Assembly ≥30 |
| ✅ **371-J** | **Karnataka** | Development board + **reservation** (education + jobs) for the **Hyderabad-Karnataka region** (98th Amdt 2012) |

⚠️ **Note:** Art 371-A (Nagaland) & 371-G (Mizoram) give the **strongest protection** — Union laws on customary
law/land need Assembly consent. These are distinct from the **Sixth Schedule** (tribal autonomous councils in
Assam, Meghalaya, Tripura, Mizoram) and **Fifth Schedule** (scheduled areas).

---

## UPSC Traps
- ❌ Art 370 has been fully deleted from the Constitution → it still **exists in text** but was rendered
  **inoperative** (2019); SC upheld this (2023).
- ❌ Special provisions (Art 371) are in Part III → they are in **Part XXI** ("Temporary, Transitional & Special").
- ❌ Art 371 covers all NE states via one clause → **separate articles** (371-A Nagaland, 371-C Manipur, 371-G
  Mizoram, 371-H Arunachal); Assam = 371-B.
- ❌ 371-A/371-G let Parliament override state customary law → those Union laws **don't apply unless the state
  Assembly consents**.
- ❌ 371-D applies only to Andhra Pradesh → **extended to Telangana** (2014).
- ❌ Sixth Schedule = Art 371 → different mechanisms (Sixth Schedule = autonomous district councils).

## 📰 CA hooks
- 📰 ✅ **J&K statehood restoration** pending post the 2024 Assembly elections — SC monitoring; live federalism +
  Art 370 aftermath issue *(verified 2026)*.
- 📰 ⚠️ **Ladakh's demand for Sixth Schedule status / statehood** — recurring protests (Leh/Kargil).
- 📰 ⚠️ Debates on extending special protections (e.g. Art 371-type safeguards) to other regions.

## Mains angles
- Was the abrogation of Article 370 a step towards genuine integration or an erosion of federal/consent
  principles? Critically examine in light of the 2023 SC verdict.
- Asymmetric federalism (Art 371 provisions): how does it accommodate diversity without threatening unity?
- Ladakh's Sixth Schedule demand — balancing development, ecology and tribal autonomy.

## Verified PYQ application

- ✅ **2025 GS-II Q4 (10 marks, 150 words):** “Discuss the nature of Jammu and Kashmir
  Legislative Assembly after the Jammu and Kashmir Reorganization Act, 2019. Briefly
  describe the powers and functions of the Assembly of the Union Territory of Jammu and
  Kashmir.” Route: UT legislature under the 2019 Act -> elected law-making and financial/
  accountability functions -> exclusions and Union/LG override architecture -> compare
  with a State legislature without treating J&K as constitutionally identical to Delhi.
