...
) / "Philosophy-Optional/Philosophy-Paper-I-—-Western-Philosophy/06-Moore,-Russell-and-Early-Wittgenstein"
FORMAL_SOURCES = {
    "session": SOURCE_DIR / "Learning-Session.md",
    "workbook": SOURCE_DIR / "Solved-Practice-Workbook.md",
}
CANONICAL = REPO / "knowledge/Philosophy/paper-1/western/Moore-Russell-EarlyWittgenstein.md"
PYQ_LEDGERS = (
...
WITNESS_REUSE_WHITELIST = {
    (
        "session-answer-grabbing-opening-write-adapt-in-the-exam-11-00740f59b2",
        (
            "package_destination|REVISION-GUIDE.md|"
            "ANSWER-GRABBING OPENING — WRITE/ADAPT IN THE EXAM|"
            "SESSION 10 — Saying and Showing, Criticism and the Comparative Answer Spine"
        ),
        "session-definition-what-this-is-called-01-9fc3f6e317",
        (
            "package_destination|REVISION-GUIDE.md|"
        ...
        "Reuse is necessary because Session 11's answer-opening claim and its saying/showing "
        "destination both require the exact technical term 'logical form'; replacing that term "
        "with a synonym would alter the Tractarian doctrine."
    ),
}
    ...
    if global_state is not None and normalized:
        ...
        for prior in entries:
            ...
            if exact_duplicate and not reuse_allowed:
                failures.append(
                    {
                        "reason": "cross-block duplicate witness misdirection",
                        "context": context,
                        "witness": witness,
                ...
                failures.append(
                    {
                        "reason": "cross-block near-duplicate witness",
                        "context": context,
                        "witness": witness,
...
MOORE_ROUTE_FORBIDDEN_SEMANTIC_ASSERTIONS = [
    {
        "id": "moore-or-transparency-entails-direct-realism",
        "pattern": (
            r"(?:moore|transparency)(?:(?!russell).){0,160}"
            r"(?:establish(?:es|ed)?|prov(?:e|es|ed)|entail(?:s|ed)?|"
            r"yield(?:s|ed)?|constitut(?:e|es|ed))"
            r"(?:(?!russell).){0,100}direct realism|"
            r"direct realism(?:(?!russell).){0,100}"
            r"(?:establish(?:es|ed)?|prov(?:e|es|ed)|entail(?:s|ed)?|"
            r"yield(?:s|ed)?|constitut(?:e|es|ed))"
            r"(?:(?!russell).){0,80}(?:moore|transparency)"
        ),
    },
    {
        "id": "moore-directly-knows-ordinary-object",
        "pattern": (
    ...
    {
        "id": "moore-defends-ordinary-object",
        "pattern": (
    ...
    {
        "id": "moore-restores-common-sense",
        "pattern": (
            r"moore(?:(?!russell).){0,180}"
            r"(?:restore(?:s|d)?|recover(?:s|ed)?|re-establish(?:es|ed)?)"
            r"(?:(?!russell).){0,80}common sense"
        ),
    },
]
...
CROSS_TOPIC_ROUTE_OBLIGATIONS = [
    {
        "id": "WP-2018-Q3A-MOORE-RUSSELL-BERKELEY",
        ...
        "bounded_scope": (
            "Supply Moore's act/object response and Russell's logical-construction response only "
            "after Berkeley's immaterialism is established; retain Topic 03 as the sole primary "
            "owner of 2018 Q3(a)."
        ),
        ...
        "source_owner_evidence": {
            ...
            "witnesses": [
                "Berkeley",
                "Moore and Russell",
                "difference between Moore",
            ],
        },
        "destination_evidence": [
            {
                "file": "COVERAGE-LEDGER.md",
                "anchor": "Bounded route obligations",
                "witnesses": [
                    "WP-2018-Q3A-MOORE-RUSSELL-BERKELEY",
                    "act/object",
                    "logical-construction",
                ],
                "required_semantic_assertions": [
                    {
                        "id": "ledger-moore-qualified-route",
                        "term_groups": [
                            ["moore"],
                            ["transparency diagnosis"],
                            ["act of awareness"],
                            ["blocks berkeley's identity-based inference"],
                            ["leaves a complete replacement theory of perception open"],
                        ],
                    },
                    {
                        "id": "ledger-russell-reconstructs",
                        "term_groups": [
                            ["russell"],
                            ["reconstructs physical-object discourse"],
                            ["logical constructions"],
                        ],
                    },
                ],
                "forbidden_semantic_assertions": MOORE_ROUTE_FORBIDDEN_SEMANTIC_ASSERTIONS,
            },
            {
                "file": "../03-Empiricism/ANSWER-WRITING-TOOLKIT.md",
                ...
                "reviewed_assertions": [
                    {
                        "id": "moore-act-object-blocks-identity",
                        "claim": (
                            "Moore distinguishes the act of awareness from its object and thereby "
                            "blocks Berkeley's identity-based inference."
                        ),
                        "required_exact_text": [
                            "distinguishing the act of awareness from its object",
                            "It blocks the identity inference",
                        ],
                    },
                    {
                        "id": "moore-no-replacement-theory",
                        "claim": (
                            "Transparency presses toward realism but does not itself provide a "
                            "complete replacement theory of perception."
                        ),
                        "required_exact_text": [
                            "transparency argument presses toward realism",
                            "does not itself supply a full replacement theory of perception",
                        ],
                    },
                ],
                "forbidden_exact_text": [
                    "transparency argument yields direct realism",
                    "Moore defends the ordinary object as directly known",
                    "Moore restores common sense",
                ],
                "required_semantic_assertions": [
                    {
                        "id": "moore-blocks-identity-semantically",
                        "term_groups": [
                            ["moore"],
                            ["act of awareness"],
                            ["object"],
                    ...
                    {
                        "id": "moore-leaves-perception-open-semantically",
                        "term_groups": [
                            ["transparency"],
                            ["perception"],
                            [
                                "does not itself supply",
                                "leaves the positive analysis of perception open",
                            ],
                        ],
                    },
                ],
            ...
            {
                "file": "../03-Empiricism/REVISION-GUIDE.md",
                "anchor": (
                ...
                "witnesses": [
                    "act of awareness",
                    "perceptual theory open",
                    "Russell reconstructs",
                ],
                "required_semantic_assertions": [
                    {
                        "id": "revision-moore-diagnostic-only",
                        "term_groups": [
                            ["moore"],
                    ...
                    {
                        "id": "revision-russell-reconstructs",
                        "term_groups": [
                            ["russell"],
                            ["reconstructs", "logical construction"],
                            ["structural objectivity", "physical-object discourse"],
                        ],
                    },
                ],
                "forbidden_semantic_assertions": MOORE_ROUTE_FORBIDDEN_SEMANTIC_ASSERTIONS,
            ...
            {
                "file": "../03-Empiricism/MCQ-QUESTIONS.md",
                ...
                "required_semantic_assertions": [
                    {
                        "id": "question-neutral-correct-proposition",
                        "term_groups": [
                            ["moore"],
                            ["blocks the act-object identity"],
                            ["leaving perception open"],
                            ["russell reconstructs physical objects"],
                        ],
                    },
            ...
            {
                "file": "../03-Empiricism/MCQ-SOLUTIONS.md",
                "anchor": "MCQ 20. Moore and Russell against Berkeley",
                "witnesses": [
                    "transparency diagnosis exposes the act-object confusion",
                    "without supplying a complete perceptual theory",
                    "Russell reconstructs physical-object discourse",
                ],
                "required_semantic_assertions": [
                    {
                        "id": "solution-moore-qualified",
                        "term_groups": [
                            ["moore"],
                            ["transparency diagnosis"],
                    ...
                    {
                        "id": "solution-russell-reconstructs",
                        "term_groups": [
                            ["russell"],
                            ["reconstructs physical-object discourse"],
            ...
            },
            {
                ...
                "anchor": "Bounded cross-links — not Topic 06-owned",
                "witnesses": [
                    "2018 Q3(a)",
                    "Empiricism",
                    "act/object",
                ...
                "required_semantic_assertions": [
                    {
                        "id": "topic06-toolkit-moore-qualified-route",
                        "term_groups": [
                            ["moore"],
                            ["transparency diagnosis"],
                            ["act of awareness"],
                            ["blocks berkeley's identity-based inference"],
                            ["leaving a complete replacement theory of perception open"],
                        ],
                    },
                    {
                        "id": "topic06-toolkit-russell-reconstructs",
                        "term_groups": [
                            ["russell"],
                            ["logical-construction strategy"],
                            ["reconstructs physical-object discourse"],
                        ],
                    },
                ],
    ...
    {
        "id": "WP-2023-Q4C-EARLY-LATER-TRANSITION",
        "origin_primary_owner": "../08-Later-Wittgenstein",
        "paired_owner": ".",
        "verified_route_source": "../../../../../knowledge/Philosophy/paper-1/_PYQ-Western-Philosophy-2018-2025.md",
        "bounded_scope": (
            "Supply only the early picture-theory baseline before Topic 08 explains the move to "
            "meaning-as-use; retain Topic 08 as the sole primary owner of 2023 Q4(c)."
        ),
        "status": "fulfilled_substantively",
        "source_owner_evidence": {
            "file": "../../../../../knowledge/Philosophy/paper-1/_PYQ-Western-Philosophy-2018-2025.md",
        ...
        "destination_evidence": [
            {
                ...
                "anchor": "Bounded route obligations",
                "witnesses": [
                    "WP-2023-Q4C-EARLY-LATER-TRANSITION",
                    "picture-theory",
                    "meaning-as-use",
                ],
            },
            {
                "file": "../08-Later-Wittgenstein/ANSWER-WRITING-TOOLKIT.md",
                "anchor": "5. 2023 Q4(c) · 15 marks",
                "witnesses": [
                    "picture theory",
                    "meaning",
                    "use",
    ...
    {
        "id": "WP-2025-Q3A-EARLY-LATER-TRANSITION",
        "origin_primary_owner": "../08-Later-Wittgenstein",
        "paired_owner": ".",
        "verified_route_source": "../../../../../knowledge/Philosophy/paper-1/_PYQ-Western-Philosophy-2018-2025.md",
        "bounded_scope": (
            "Supply only the early single-logical-structure baseline; retain Topic 08 as the sole "
            "primary owner of the transition to later method in 2025 Q3(a)."
        ),
        "status": "fulfilled_substantively",
        "source_owner_evidence": {
            "file": "../../../../../knowledge/Philosophy/paper-1/_PYQ-Western-Philosophy-2018-2025.md",
        ...
        "destination_evidence": [
            {
                ...
                "anchor": "Bounded route obligations",
                "witnesses": [
                    "WP-2025-Q3A-EARLY-LATER-TRANSITION",
                    "logical-structure",
                    "later method",
                ],
            },
    ...
    },
    {
        ...
        "paired_owner": "../07-Logical-Positivism",
        "verified_route_source": "../../../../../knowledge/Philosophy/paper-1/_PYQ-Western-Philosophy-2026.md",
        "bounded_scope": (
            "Retain 2026 Q4(a) as Topic 06-owned; Topic 07 receives only the bounded "
        ...
        "destination_evidence": [
            {
                ...
                "anchor": "Bounded route obligations",
                "witnesses": [
                    "WP-2026-Q4A-METALANGUAGE-UNIFIED-SCIENCE",
                    "metalanguage",
                    "unified-science",
                ],
            },
            {
                "file": "../07-Logical-Positivism/ANSWER-WRITING-TOOLKIT.md",
                "anchor": "Source and ownership discipline",
                "witnesses": [
                    "2026 Q4(a)",
                    "metalanguage",
                    "unified science",
...
def coverage_negative_test_results(
        ...
        unit["propositions"] = [
            {
                ...
                "claim": "analytic topic panel generic fragment",
                "witnesses": ["analytic"],
            }
        ]
    ...
    truncated_inventory = copy.deepcopy(
        chronology_decision["distributed_proposition_inventory"]
    )
    truncated_inventory[0]["propositions"][0]["claim"] = (
        "Settl examin proposi stat."
    )
    truncated_inventory_failures = distributed_inventory_validation_failures(
        chronology,
        truncated_inventory,
        chronology_decision["package_destinations"],
        package_text,
    )
    boilerplate_inventory = copy.deepcopy(
        chronology_decision["distributed_proposition_inventory"]
    )
    boilerplate_inventory[0]["propositions"][0]["claim"] = (
        "The source explains how chronology analysis supports or limits the topic argument."
    )
    boilerplate_inventory_failures = distributed_inventory_validation_failures(
        chronology,
        boilerplate_inventory,
        chronology_decision["package_destinations"],
        package_text,
    )
    similar_inventory = copy.deepcopy(
        chronology_decision["distributed_proposition_inventory"]
    )
    similar_inventory[0]["propositions"][0]["claim"] = (
        "Moore separates the 1903 paper from the 1925 paper and preserves chronology."
    )
    similar_inventory[0]["propositions"][1]["claim"] = (
        "Russell separates the 1905 paper from the 1914 paper and preserves chronology."
    )
    similar_inventory_failures = distributed_inventory_validation_failures(
        chronology,
        similar_inventory,
        chronology_decision["package_destinations"],
        package_text,
    )
    unrelated_inventory = copy.deepcopy(
        chronology_decision["distributed_proposition_inventory"]
    )
    unrelated_inventory[0]["propositions"][0]["claim"] = (
        "Kant derives synthetic unity from transcendental apperception and the categories."
    )
    unrelated_inventory_failures = distributed_inventory_validation_failures(
        chronology,
        unrelated_inventory,
        chronology_decision["package_destinations"],
        package_text,
    )
    weak_floor_failures = aggregate_correspondence_failures(
        chronology["source_signature"],
        destination_body(chronology_decision),
        .08,
    )
    generic_witness_failures = witness_quality_failures(
        "in the",
        context="negative-test:generic-witness",
    )
    duplicate_witness_state: dict = {"entries": []}
    witness_quality_failures(
        "act object distinction",
        context="negative-test:first-block",
        global_state=duplicate_witness_state,
        block_id="first-block",
        witness_id="first-witness",
        source_body="Moore distinguishes the act from the object.",
    )
    cross_block_duplicate_failures = witness_quality_failures(
        "act object distinction",
        context="negative-test:second-block",
        global_state=duplicate_witness_state,
        block_id="second-block",
        witness_id="second-witness",
        source_body="Russell analyses definite descriptions.",
        reuse_justification=(
            "This duplicate is allowed because the reviewer supplied more than eight words."
        ),
    )
    near_duplicate_witness_state: dict = {"entries": []}
    witness_quality_failures(
        "logical form possible fact",
        context="negative-test:near-first-block",
        global_state=near_duplicate_witness_state,
        block_id="near-first-block",
        witness_id="near-first-witness",
        source_body="A proposition shares logical form with a possible fact.",
    )
    cross_block_near_duplicate_failures = witness_quality_failures(
        "logical form possible fact picturing relation",
        context="negative-test:near-second-block",
        global_state=near_duplicate_witness_state,
        block_id="near-second-block",
        witness_id="near-second-witness",
        source_body=(
            "Logical form connects a possible fact through the picturing relation."
        ),
    )
    whitelist_state: dict = {"entries": []}
    whitelist_prior_block = (
        "session-definition-what-this-is-called-01-9fc3f6e317"
    )
    whitelist_prior_witness_id = (
        "package_destination|REVISION-GUIDE.md|"
        "DEFINITION / WHAT THIS IS CALLED|"
        "SESSION 1 — The Analytic Turn: Three Programmes, One Revolt"
    )
    whitelist_current_block = (
        "session-answer-grabbing-opening-write-adapt-in-the-exam-11-00740f59b2"
    )
    whitelist_current_witness_id = (
        "package_destination|REVISION-GUIDE.md|"
        "ANSWER-GRABBING OPENING — WRITE/ADAPT IN THE EXAM|"
        "SESSION 10 — Saying and Showing, Criticism and the Comparative Answer Spine"
    )
    ...
    whitelist_reason = WITNESS_REUSE_WHITELIST[whitelist_key]
    witness_quality_failures(
        "logical form",
        context="negative-test:whitelist-prior",
        global_state=whitelist_state,
        block_id=whitelist_prior_block,
        witness_id=whitelist_prior_witness_id,
        source_body="Analytic philosophy investigates logical form.",
    )
    whitelist_failures = witness_quality_failures(
        "logical form",
        context="negative-test:whitelist-current",
        global_state=whitelist_state,
        block_id=whitelist_current_block,
        witness_id=whitelist_current_witness_id,
        source_body="Saying and showing turn on logical form.",
        reuse_justification=whitelist_reason,
    )
    dependent_subject_inventory = copy.deepcopy(
        chronology_decision["distributed_proposition_inventory"]
    )
    dependent_subject_inventory[0]["propositions"][0]["claim"] = (
        "It does not prove the chronology because the dates remain disputed."
    )
    dependent_subject_failures = distributed_inventory_validation_failures(
        chronology,
        dependent_subject_inventory,
        chronology_decision["package_destinations"],
        package_text,
    )
    ...
    reduced_panel_failures = distributed_inventory_validation_failures(
        master_flow,
        master_decision["distributed_proposition_inventory"][:-1],
    ...
    moore_route = next(
        ...
        if obligation["id"] == "WP-2018-Q3A-MOORE-RUSSELL-BERKELEY"
    )
    direct_realism_matrix = []
    direct_realism_subjects = ("Moore", "Transparency")
    direct_realism_predicates = (
        "establishes",
        "proves",
        "entails",
        "yields",
    ...
    for evidence in moore_route["destination_evidence"]:
        path = (TOPIC / evidence["file"]).resolve()
        text = path.read_text(encoding="utf-8")
        section = find_heading_section(text, evidence["anchor"])
        destination = f"{evidence['file']} :: {evidence['anchor']}"
        for subject in direct_realism_subjects:
            for predicate in direct_realism_predicates:
                injected = f"{subject} {predicate} direct realism."
                mutated = section + "\n" + injected + "\n"
                injection_count = normalise(mutated).count(normalise(injected))
                failures_for_cell = route_assertion_validation_failures(
                    evidence,
                    mutated,
                )
                expected_rule_fired = any(
                    failure.get("reason")
                    == "forbidden semantic route assertion present"
                    and failure.get("assertion")
                    == "moore-or-transparency-entails-direct-realism"
                    for failure in failures_for_cell
                )
                direct_realism_matrix.append(
                    {
                        ...
                        "pass": injection_count == 1 and expected_rule_fired,
                    }
                )
    direct_realism_matrix_test = {
        "name": "direct_realism_paraphrase_matrix",
        "surface_count": len(moore_route["destination_evidence"]),
        "paraphrases_per_surface": (
            len(direct_realism_subjects) * len(direct_realism_predicates)
        ),
        "cell_count": len(direct_realism_matrix),
        "passed_cells": sum(cell["pass"] for cell in direct_realism_matrix),
        "failed_cells": sum(not cell["pass"] for cell in direct_realism_matrix),
        "cells": direct_realism_matrix,
        "pass": bool(direct_realism_matrix)
        and all(cell["pass"] for cell in direct_realism_matrix),
    }
    ...
    tests = [
        ...
        record(
            "observed_truncated_stem_claim",
            truncated_inventory_failures,
            ["truncated or stemmed claim fragment"],
        ),
        record(
            "observed_boilerplate_relationship_padding",
            boilerplate_inventory_failures,
            [
                "boilerplate proposition template",
                "generic relationship-token padding",
            ],
        ),
        record(
            "high_structural_similarity_claim_pair",
            similar_inventory_failures,
            ["distributed claims have high structural similarity"],
        ),
        record(
            "claim_without_source_or_destination_correspondence",
            unrelated_inventory_failures,
            [
                "claim lacks discriminative source vocabulary",
                "claim lacks destination semantic correspondence",
            ],
        ),
        record(
            "declared_overlap_floor_regression",
            weak_floor_failures,
            ["declared aggregate correspondence floor is too weak"],
        ),
        record(
            "generic_witness_in_the",
            generic_witness_failures,
            ["generic or insufficiently specific witness"],
        ),
        record(
            "cross_block_duplicate_witness_misdirection",
            cross_block_duplicate_failures,
            ["cross-block duplicate witness misdirection"],
        ),
        record(
            "cross_block_near_duplicate_witness_variant",
            cross_block_near_duplicate_failures,
            ["cross-block near-duplicate witness"],
        ),
        {
            "name": "strict_whitelist_logical_form_reuse",
            "whitelist_key": list(whitelist_key),
            "recorded_reason": whitelist_reason,
            "actual_failures": whitelist_failures,
            "pass": not whitelist_failures,
        },
        record(
            "dependent_subject_it_does_not",
            dependent_subject_failures,
            ["distributed claim lacks an explicit independent subject"],
        ),
        record(
            "master_flow_drops_final_panel",
            reduced_panel_failures,
            ["distributed inventory does not span every extracted unit"],
        ),
        record(
...
def build_formal_audit() -> dict:
    ...
    return {
        "schema_version": 6,
        "topic": "06 Moore, Russell and Early Wittgenstein",
...
def formal_audit_checks() -> dict:
    ...
    ledger_topic_parts = re.findall(
        r"^- \*\*(Q\d+\([a-z]\)) · \d+ marks · \[Moore[^]]*Russell[^]]*Early Wittgenstein\]\([^)]+\):\*\*",
        ledger_2026,
        re.M,
    )
    latest_upgrade_rows = [
    ...
    latest_owner_assertion = {
        ...
        "required_primary_parts": ["Q1(e)", "Q4(a)"],
        "forbidden_misattribution": "Q4(c)",
        "review_rows_checked": [row.get("id") for row in latest_upgrade_rows],
        "pass": (
            set(ledger_topic_parts) == {"Q1(e)", "Q4(a)"}
            and "Q4(c)" not in ledger_topic_parts
            ...
            and "## 15. 2026 Q1(e) · 10 marks" in toolkit_text
            and "## 16. 2026 Q4(a) · 20 marks" in toolkit_text
            and "Logical Positivism" in find_heading_section(
                toolkit_text, "Bounded cross-links — not Topic 06-owned"
            )
        ),
    }
...
def mcq_checks(questions: list[dict], solutions: list[dict], question_text: str) -> dict:
    ...
    for item in solutions:
        ...
        for letter, explanation in item["explanations"].items():
            ...
            status_pass = (
                explanation.startswith("Correct:")
                if letter == answer
                else not explanation.startswith("Correct:")
            )
...
def toolkit_checks(toolkit: str) -> dict:
    ...
    for index, match in enumerate(matches):
            ...
            "## Original transfer drills", match.end()
        )
...
def regenerate_pdfs() -> dict:
    ...
    descriptors = {
        "Revision-Guide.pdf": "Philosophy Optional · Paper I · Western Philosophy · Topic 06",
        ...
        "Answer-Writing-Toolkit.pdf": "Sixteen verified PYQs and eight original solved models",
    }
    files = {}
    for pdf_name, source_name in PDF_SOURCES.items():
        ...
        unicode_markdown_pdf.build_pdf(
            ...
            footer_label=f"Moore, Russell and Early Wittgenstein | {source_name.removesuffix('.md')}",
        )
        files[pdf_name] = {
            "source": source_name,
