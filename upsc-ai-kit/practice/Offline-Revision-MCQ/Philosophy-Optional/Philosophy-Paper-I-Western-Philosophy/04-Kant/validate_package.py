...
) / "Philosophy-Optional/Philosophy-Paper-I-—-Western-Philosophy/04-Kant"
FORMAL_SOURCES = {
    "session": SOURCE_DIR / "Learning-Session.md",
    "workbook": SOURCE_DIR / "Solved-Practice-Workbook.md",
}
CANONICAL = REPO / "knowledge/Philosophy/paper-1/western/Kant.md"
PYQ_LEDGERS = (
...
CROSS_TOPIC_INBOUND_OBLIGATIONS = [
    {
        ...
        "source_owner_evidence": {
            "file": "../03-Empiricism/COVERAGE-LEDGER.md",
        ...
        "destination_evidence": [
            ...
            },
            {
                ...
                "anchor": "SESSION 8 — THE ANALOGIES OF EXPERIENCE: SUBSTANCE, CAUSALITY AND COMMUNITY, THE REAL ANSWER TO HUME",
                "witnesses": [
                    "Hume",
            ...
            },
            {
                ...
                "anchor": "MCQ 39. Irreversible succession in the Second Analogy",
                "witnesses": [
                    "objective succession",
                    "rule-governed causality",
                    "2026 Q2(b)",
                ],
            },
            {
...
def formal_audit_checks() -> dict:
    ...
    ledger_kant_parts = re.findall(
        r"^- \*\*(Q\d+\([a-z]\)) · \d+ marks · \[Kant\]\([^)]+\):\*\*",
        ledger_2026,
        re.M,
    )
    latest_upgrade_rows = [
    ...
    latest_owner_assertion = {
        "ledger_kant_parts": ledger_kant_parts,
        "required_primary_part": "Q4(b)",
        "forbidden_misattribution": "Q2(b)",
        "review_rows_checked": [row.get("id") for row in latest_upgrade_rows],
        "pass": (
            "Q4(b)" in ledger_kant_parts
            and "Q2(b)" not in ledger_kant_parts
            and len(latest_upgrade_rows) == 2
            and all(
                "2026 Q4(b)" in row.get("review_basis", "")
                and "2026 Q2(b)" in row.get("review_basis", "")
...
def extract_answer(section: str) -> str:
    match = re.search(
        r"^##### Independent model answer\s*\n(.*?)(?=^##### (?:Qualification and criticism|Why this earns marks|Exam-length execution|Closed-book answer spine)|\Z)",
        section,
        re.M | re.S,
    )
...
def toolkit_checks(toolkit: str) -> dict:
    ...
    owned_set_pass = set(sections) == set(expected_keys) and len(expected) == 11
    originals_pass = (
...
def pdf_checks(path: Path, source_path: Path, regeneration_file: dict | None) -> dict:
    ...
    passed = (
        ...
        and source_specific.get("owned_pyq_headings", 11) == 11
    )
    return {
        "bytes": path.stat().st_size,
        "pages": pages,
...
def regenerate_pdfs() -> dict:
    ...
    descriptors = {
        "Revision-Guide.pdf": "Philosophy Optional · Paper I · Western Philosophy · Topic 04",
        "MCQ-Questions.pdf": "42-question closed-book bank · no answer key",
        "MCQ-Solutions.pdf": "Option-specific explanations and examiner traps",
        "Answer-Writing-Toolkit.pdf": "Eleven verified PYQs and six original solved models",
    }
    files = {}
    for pdf_name, source_name in PDF_SOURCES.items():
        ...
        unicode_markdown_pdf.build_pdf(
            ...
            footer_label=f"Kant | {source_name.removesuffix('.md')}",
        )
        files[pdf_name] = {
            "source": source_name,
