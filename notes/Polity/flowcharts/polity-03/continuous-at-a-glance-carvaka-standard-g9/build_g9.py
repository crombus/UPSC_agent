"""Build the Polity 03 continuous at-a-glance master flow, generation g9.

Reuses the Carvaka-standard rendering primitives (render_lib) proven in the
Polity 01 g9 and Polity 02 g8 packages, plus seventeen Polity-03-specific
primitives (render_p3, render_p3b). All stage structure, layout grammar and
content is authored fresh for Polity 03 — Salient Features of the Indian
Constitution.
"""
...
import render_p3                 # noqa: E402,F401  (registers new blocks, part 1)
import render_p3b                # noqa: E402,F401  (registers new blocks, part 2)
...
TOPIC_DIR = HERE.parent                      # notes/Polity/flowcharts/polity-03
PREV = HERE / "previews"
...
POSTER = HERE / f"Polity03_Continuous-At-a-Glance_g9_Poster_{DATESTAMP}.pdf"
TILED = HERE / f"Polity03_Continuous-At-a-Glance_g9_Tiled_{DATESTAMP}.pdf"
...
def build_poster(img):
    ...
    c.setTitle("Polity 03 — Salient Features of the Indian Constitution — Continuous At-a-Glance Master Flow (g9)")
    c.showPage()
    c.save()
    return Wp / 72, Hp / 72
...
def build_tiled(img):
    ...
    for i in range(n):
        ...
        c.drawString(M + 2, M - 4,
                     f"POLITY 03 — CONTINUOUS AT-A-GLANCE MASTER FLOW (g9)  ·  TILE {i+1} OF {n}  ·  "
                     f"same master canvas, overlapping crop rows {top}-{bot} of {img.height}  ·  "
                     + ("continues below" if i < n - 1 else "end of master canvas"))
        c.showPage()
    c.setTitle("Polity 03 — Continuous At-a-Glance Master Flow (g9) — tiled")
    c.save()
...
def write_spec(H, stage_heights, tile_info):
    ...
    spec = {
        "artifact": "Polity 03 — Salient Features of the Indian Constitution — continuous at-a-glance master flow",
        "generation": "polity-03:flowchart:continuous-at-a-glance-carvaka-standard:g9",
        "note_on_generation_index": (
            "g9 is simply the next free index in the polity-03 folder, which already "
            "held continuous-at-a-glance-core-first g3-g8. This is the FIRST "
            "Carvaka-standard generation for Polity 03. The index carries no quality "
            "meaning and is unrelated to the Polity 01 g9 package."
        ),
        "approved": False,
        "generated": DATESTAMP,
        "design_reference": {
            ...
            "reused_from_earlier_packages": (
                "rendering primitives only (render_lib.py, shared with Polity 01 g9 and "
                "Polity 02 g8); no stage structure, layout signature, pill set or content "
                "is reused from either package"
            ),
        },
        "new_primitives": {
            "wheel": "hub-and-four-node system definition wheel",
            "fan": "causes -> core -> consequences fan for the length architecture",
            "srcmap": "source country / adopted device / rejected-or-adapted three-column map",
            "spectrum": "amendment-difficulty spectrum with graded stops and an entrenchment ceiling",
            "quadrant": "four-quadrant federal / centralising / cooperative / asymmetric field",
            "loop": "closed parliamentary accountability loop with a central slack panel",
            "balance": "two-pan balance on a constitutional fulcrum",
            "overlap": "three-organ separation diagram with explicit constitutional overlap zones",
            "triangle": "apex-and-three-tests supremacy triangle",
            "pyramid": "court hierarchy pyramid with an adjacent review-and-remedy rail",
            "triad": "three-status column set with justiciability badges and a shared footer",
            "vs3": "three-model comparison grid with the Indian middle column highlighted",
            "bus": "integration bus line with device cards carrying function and tension",
            "constel": "central core with four constitutional-body cards around it",
            "switch": "normal-mode / exception-mode switch with trigger list and return path",
            "tension": "strength / fault line / failure-mode three-column tension matrix",
            "web": "feature-pair interaction web with outcome column",
        },
        "sources": {
        ...
        "typography": {
            ...
            "big_number_px": 78, "medium_number_px": 52,
        },
        "outputs": {
